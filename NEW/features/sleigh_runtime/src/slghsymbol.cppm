...
export module sleigh_runtime:slghsymbol;
import std;
...
public:
    ...
    virtual void encodeHeader(Encoder& encoder) const { // Save the basic attributes of a symbol
...
class SymbolTable {
    ...
    void renumber(void) { // Renumber all the scopes and symbols
...
public:
    ...
    void replaceSymbol(SleighSymbol* a, SleighSymbol* b) { // Replace symbol a with symbol b
    ...
    void decodeSymbolHeader(Decoder& decoder) { // Put the shell of a symbol in the symbol table
        symbolTableDecodeSymbolHeader(*this, decoder);
    }
    void purge(void) { // Get rid of unsavable symbols and scopes
...
public:
    PatternlessSymbol(
        void) { // The void constructor must explicitly build the ConstantValue. It is not decode (or encoded)
...
public:
    ...
    virtual VarnodeTpl* getVarnode(void) const {
        VarnodeTpl* res =
            new VarnodeTpl(ConstTpl(const_space), ConstTpl(ConstTpl::real, 0), ConstTpl(ConstTpl::real, 0));
        return res;
    }
    virtual void encode(Encoder& encoder) const {
...
class ValueMapSymbol : public ValueSymbol {
    ...
    bool tableisfilled;
    void checkTableFill(void) { // Check if all possible entries in the table have been filled
...
public:
    ...
    virtual Constructor* resolve(ParserWalker& walker) {
        if (!tableisfilled) {
            intb ind = patval->getValue(walker);
            if ((ind >= nametable.size()) || (ind < 0) ||
                ((nametable[ind].size() == 1) && (nametable[ind][0] == '\t'))) {
                ostringstream s;
                s << walker.getAddr().getShortcut();
                walker.getAddr().printRaw(s);
...
public:
    ...
    VarnodeListSymbol(const string& nm, PatternValue* pv, const vector<SleighSymbol*>& vt) : ValueSymbol(nm, pv) {
...
public:
    ...
    StartSymbol(const string& nm, AddrSpace* cspc) : SpecificSymbol(nm) {
        const_space = cspc;
    ...
    virtual VarnodeTpl* getVarnode(void) const { // Returns current instruction offset as a constant
...
public:
    ...
    EndSymbol(const string& nm, AddrSpace* cspc) : SpecificSymbol(nm) {
        const_space = cspc;
    ...
    virtual VarnodeTpl* getVarnode(void) const { // Return next instruction offset as a constant
        ConstTpl spc(const_space);
...
public:
    ...
    Next2Symbol(const string& nm, AddrSpace* cspc) : SpecificSymbol(nm) {
        const_space = cspc;
    ...
    virtual VarnodeTpl*
    getVarnode(void) const { // Return instruction offset after next instruction offset as a constant
        ConstTpl spc(const_space);
...
public:
    ...
    FlowDestSymbol(const string& nm, AddrSpace* cspc) : SpecificSymbol(nm) {
        const_space = cspc;
    }
    virtual VarnodeTpl* getVarnode(void) const {
        ConstTpl spc(const_space);
...
public:
    ...
    FlowRefSymbol(const string& nm, AddrSpace* cspc) : SpecificSymbol(nm) {
        const_space = cspc;
    }
    virtual VarnodeTpl* getVarnode(void) const {
        ConstTpl spc(const_space);
...
public:
    ...
    virtual void validate(void) const { // Throw an exception if the PatternExpression is not valid
...
public:
    ...
    void setNamedSection(ConstructTpl* tpl, int4 id) { // Add a named section to the constructor
    ...
    void markSubtableOperands(vector<int4>& check)
        const { // Adjust -check- so it has one entry for every operand, a 0 if it is a subtable, a 2 if it is not
    ...
    bool isRecursive(void) const { // Does this constructor cause recursion with its table
...
public:
    void identicalPattern(Constructor* a, Constructor* b) { // Note that -a- and -b- have identical patterns
    ...
    void conflictingPattern(Constructor* a,
                            Constructor* b) { // Note that -a- and -b- have (potentially) conflicting patterns
        if ((!a->isError()) && (!b->isError())) {
            a->setError(true);
            b->setError(true);
...
class DecisionNode {
    ...
    int4 getNumFixed(int4 low, int4 size, bool context) { // Get number of patterns that specify this field
        int4 count = 0;
        uintm mask;
    ...
    int4 getMaximumLength(bool context) { // Get maximum length of instruction pattern in bytes
        int4 max = 0;
        int4 val, i;
    ...
    void consistentValues(vector<uint4>& bins,
                          DisjointPattern* pat) { // Produce all possible values of -pat- by
...
public:
    ...
    ~DecisionNode(void) { // We own sub nodes
...
public:
    ...
    SubtableSymbol(const string& nm) : TripleSymbol(nm) {
        beingbuilt = false;
        pattern = (TokenPattern*)0;
        decisiontree = (DecisionNode*)0;
        errors = 0;
    ...
    void buildDecisionTree(DecisionProperties& props) { // Associate pattern disjoints to constructors
        if (pattern == (TokenPattern*)0)
...
void parserContextApplyCommits(ParserContext& context) {
    ...
    for (vector<ContextSet>::iterator iter = context.contextcommit.begin(); iter != context.contextcommit.end();
         ++iter) {
        ...
        if (commitAddress.isConstant()) {
            uintb newOffset =
                AddrSpace::addressToByte(commitAddress.getOffset(), context.addr.getSpace()->getWordSize());
...
bool constructorTryPrintMnemonic(const Constructor& constructor, ostream& stream, ParserWalker& walker) {
    ...
    SubtableSymbol* subtable =
        dynamic_cast<SubtableSymbol*>(constructor.operands[constructor.flowthruindex]->getDefiningSymbol());
    if (subtable == (SubtableSymbol*)0)
        return false;
