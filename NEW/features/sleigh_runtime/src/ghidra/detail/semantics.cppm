...
public:
    ...
    fillinSpace(FixedHandle& hand,
                const ParserWalker& walker) const; ///< Fill in the address space of a FixedHandle, based on \b this
    void fillinOffset(FixedHandle& hand,
                      const ParserWalker& walker) const; ///< Fill in the offset of a FixedHandle, based on \b this
...
public:
    ...
    HandleTpl(const ConstTpl& spc, const ConstTpl& sz, const VarnodeTpl* vn, AddrSpace* t_space,
              uintb t_offset); ///< Construct a dynamic HandleTpl
    const ConstTpl& getSpace(void) const {
        return space;
    } ///< Get the address space
...
public:
    ...
    int4 fillinBuild(vector<int4>& check,
                     AddrSpace* const_space); ///< Make sure there is a \b build directive for all sub-constructors
    ...
    void setInput(VarnodeTpl* vn, int4 index,
                  int4 slot);                    ///< Set the VarnodeTpl input for a particular OpTpl in the sequence
    ...
    void encode(Encoder& encoder,
                int4 sectionid) const; ///< Encode details of \b this semantic sequence to an output stream
