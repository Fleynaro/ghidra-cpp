 ...
 */
#include "detail/slghpatexpress.cppm"
    ...
    if (reversedirection) {
        ...
        for (int4 i = 0; i < minsize; ++i)
            if (tok1.toklist[i] != tok2.toklist[i]) {
                ostringstream msg;
                msg << "Mismatched tokens when combining patterns -- " << dec << tok1.toklist[i] << " != " << dec
                    << tok2.toklist[i];
                throw SleighError(msg.str());
            }
    }
...
static TokenPattern buildPattern(PatternValue* lhs, intb lhsval, vector<const PatternValue*>& semval, vector<intb>& val)
