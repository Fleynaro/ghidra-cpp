...
/// \param zerosize is \b true if the size of \b this should be disassociated from the computed \b handle and forced to
/// zero
...
VarnodeTpl::VarnodeTpl(const VarnodeTpl& vn) : space(vn.space), offset(vn.offset), size(vn.size) {
...
HandleTpl::HandleTpl(const ConstTpl& spc, const ConstTpl& sz, const VarnodeTpl* vn, AddrSpace* t_space, uintb t_offset)
