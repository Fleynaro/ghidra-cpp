...
CompressBuffer::CompressBuffer(ostream& s, int4 level)
    : outStream(s), inBuffer(std::make_unique<uint1[]>(IN_BUFFER_SIZE)),
      outBuffer(std::make_unique<uint1[]>(OUT_BUFFER_SIZE)), compressor(level) {
    setp(reinterpret_cast<char*>(inBuffer.get()), reinterpret_cast<char*>(inBuffer.get()) + IN_BUFFER_SIZE - 1);
}
...

{
    ...
    do {
        ...
        outStream.write(reinterpret_cast<char*>(outBuffer.get()), OUT_BUFFER_SIZE - outAvail);
    } while (outAvail == 0);
    pbump(-len);
}
