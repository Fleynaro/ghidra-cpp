...

{
    ...
    if (exp < -frac_size)            // Exponent is too small to represent
        return getZeroEncoding(sgn); // Values below the representable range round to zero.

    if (exp < 1) { // Must be denormalized
        if (roundToNearestEven(signif, 8 * sizeof(uintb) - frac_size - exp)) {
            // The carry path is handled by the normal encoding branch below.
