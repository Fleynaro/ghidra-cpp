...
- [`src/decompiler.cppm`](src/decompiler.cppm) defines the clean provider contract for p-code, memory, architecture, functions, and bounded decompilation results. `LOAD` and `STORE` name their target through `memory_space`; their operands are the address/value operands, with matching legacy constant-space selectors accepted for compatibility. Prototype parameters and returns retain single `Storage` locations and also accept ordered most-significant-to-least-significant storage pieces for ABI-split values.
- [`src/decompiler_impl.cppm`](src/decompiler_impl.cppm) implements `Translate`, `LoadImage`, provider-derived compiler-prototype bootstrap, namespace-aware function and data symbols, exact custom prototype storage/conventions, provider enum/bitfield/composite/typedef/array metadata, volatile memory properties, display formats, action, SSA, and C-printer integration without Java, IPC, or Sleigh internals. Dynamic constant formats optionally constrain the encoded width before attaching the native format symbol, preserving distinct x86 one-byte and four-byte uses. The legacy x86-64 GCC bootstrap also preserves the source compiler specification's four-byte `wchar_t` organization for string-copy selection. The Sleigh façade retries mapped windows down from 16 bytes and validates instruction lengths, addresses, storage ranges, p-code arities, and opcodes before entering the native engine; its LoadImage adapter preserves the native address-space name through the backward-compatible space-aware memory API.
...
- [`cli/decompiler_cli.cppm`](cli/decompiler_cli.cppm) provides the standalone `new_ghidra_decompiler.exe` console frontend. Its complete argument grammar, output examples, metadata records, and datatest workflow are documented in [`cli/README.md`](cli/README.md).
- [`tests/decompiler_datatests.cppm`](tests/decompiler_datatests.cppm) exports a narrow test-artifact capture bridge used by the CLI's `--test` mode. The bridge executes the registered C++ fixture and returns the same `DecompilationResult` objects inspected by the test assertions; it does not duplicate fixture construction.
...
The outputs are `new_ghidra_decompiler.lib`, `new_ghidra_decompiler_frontend.lib`, `new_ghidra_decompiler.exe`, and `decompiler_tests.exe`. The CLI supports direct `.sla` plus machine-code input and a `--test` bridge for every registered portable case in `tests/decompiler_datatests.cppm`; see [`cli/README.md`](cli/README.md) for commands and representative output. The tests cover provider p-code materialization, generic provider architectures, exact prototype/type/symbol metadata, bounded arithmetic/opcode validation, native flow/SSA/printer execution, and explicit machine-code-to-C end-to-end cases based on `TEST/dec_code_examples/1.md` through `11.md`. The PE fixture tests require the repository's optional `TEST/test.exe` fixture; synthetic PE tests remain independent of it.
