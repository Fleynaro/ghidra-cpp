...
- [`CMakeLists.txt`](CMakeLists.txt) separates the aggregate/module-interface units from the per-source C++23 implementation units and excludes graph-verified dormant sources.
...
- The selected implementation set follows the original `CORE` and `DECCORE` partitions in the authority [`Makefile`](../../../Ghidra/Features/Decompiler/src/decompile/cpp/Makefile), after removing the disabled rule compiler, Renoir graph exporter, signature generator, and parameter-report subsystem.
...
- [`src/decompiler_impl.cppm`](src/decompiler_impl.cppm) implements `Translate`, `LoadImage`, compiler-prototype bootstrap, `Architecture`, `Funcdata`, action, SSA, and C-printer integration without Java, IPC, or Sleigh internals.
...
The engine target intentionally excludes Sleigh compiler/parser/runtime sources, the removed Ghidra-only graph/signature/parameter/report integrations, tests and demos, and loader-specific implementations. The frontend consumes only the public `sleigh_runtime` value API; it never imports `sleigh_runtime:internal` or duplicates an instruction decoder.
