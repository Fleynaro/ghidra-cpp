...
Switch and jump-table cases are reproduced the same way: provide the complete function bytes with `--hex`, map the table using repeated `--data`, and declare the target symbols. For example:

```powershell
& $cli --sla $sla --address 0x480000 --size 0x25 `
  --hex "8b c1 83 f8 02 77 18 ff 24 c5 00 00 00 00 ..." `
  --data 0x480040:"00 00 48 00 00 00 00 00" `
  --symbol "address=0x480000,name=switch_root,kind=function" `
  --symbol "address=0x480050,name=switch_case_0,kind=function" `
  --symbol "address=0x480060,name=switch_case_1,kind=function" `
  --only high-pcode,control-flow,c
```

The ellipsis above is illustrative only; replace it with the exact bytes from the selected test. This design keeps the CLI independent and makes every input visible and reproducible in the command line.
