...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/ruleaction.cc.
...

{
    ...
    if (op->isCall())
        return 0; // Functions automatically consumed
...

{
    ...
    if ((nextop != (PcodeOp*)0) && (nextop->code() == CPUI_INT_ADD))
        return 0;
    ...
    if (nonzerocount <= 1)
        return 0; // Must sum at least two things
...

{
    ...
    if (!constvn->isConstant())
        return 0; // Must append with constant
...

{
    ...
    if (vn->getSize() > sizeof(uintb))
        return 0; // FIXME: Can't exceed uintb precision
    int4 sa;
    bool leftshift;
...

{
    ...
    uint4 list[] = {CPUI_INT_EQUAL,   CPUI_INT_NOTEQUAL,   CPUI_INT_ADD,   CPUI_INT_CARRY,
                    ...
                    CPUI_FLOAT_EQUAL, CPUI_FLOAT_NOTEQUAL, CPUI_FLOAT_ADD, CPUI_FLOAT_MULT};
...

{
    ...
    if (basevn->isInput()) {
        ...
        if (!basevn->isWritten())
            throw LowlevelError("Undefined pullsub");
...

{
    ...
    for (iter = basevn->beginDescend(); iter != basevn->endDescend(); ++iter) {
        ...
        if (prevop->code() != CPUI_SUBPIECE)
            continue; // Find previous SUBPIECE
        // Make sure output is defined in same block as vn_piece
        if (basevn->isInput() && (prevop->getParent()->getIndex() != 0))
            continue;
        if (!basevn->isWritten())
            continue;
        if (basevn->getDef()->getParent() != prevop->getParent())
            continue;
        // Make sure subpiece matches form
        if ((prevop->getIn(0) == basevn) && (prevop->getOut()->getSize() == outsize) &&
...

{
    ...
    if (indir->code() != CPUI_INDIRECT)
        return 0;
    if (indir->getIn(1)->getSpace()->getType() != IPTR_IOP)
        return 0;
    ...
    if (maxByte < minByte || (newSize >= vn->getSize()))
        return 0;
    if (!RulePullsubMulti::acceptableSize(newSize))
        return 0;
    Varnode* outvn = op->getOut();
    if (outvn->isPrecisLo() || outvn->isPrecisHi())
        return 0; // Don't pull apart double precision object

    uintb consume = calc_mask(newSize) << 8 * minByte;
    consume = ~consume;
    if ((consume & indir->getIn(0)->getConsume()) != 0)
        return 0;
...

{
    ...
    if (cvn2->isConstant()) {
        ...
        if (addop->getOut()->loneDescend() != op)
            return 0;
        for (int4 i = 0; i < 2; ++i) {
            Varnode* zerovn = addop->getIn(i);
            uintb mask2 = zerovn->getNZMask();
            if ((mask2 & val) != mask2)
                continue; // zerovn must be unaffected by the AND operation
            ...
            if (addop2->code() != CPUI_INT_ADD)
                continue;
            if (nonzerovn->loneDescend() != addop)
                continue;
            cvn2 = addop2->getIn(1);
            if (!cvn2->isConstant())
                continue;
...

{
    ...
    if (!constvn->isConstant())
        return 0;
    uintb val = constvn->getOffset();
    if ((op->code() == CPUI_INT_LESS) && (val != 1))
        return 0;
    if ((op->code() == CPUI_INT_LESSEQUAL) && (val != 0))
        return 0;
...

{
    ...
    if (baseconst == calc_mask(andvn->getSize()))
...

{
    ...
    if ((opc2 != CPUI_INT_LEFT) && (opc2 != CPUI_INT_RIGHT) && (opc2 != CPUI_INT_MULT))
        return 0;
    if (!secop->getIn(1)->isConstant())
        return 0;
    OpCode opc1 = op->code();
    int4 size = secvn->getSize();
    if (!secop->getIn(0)->isHeritageKnown())
        return 0;
    ...
    if (opc1 == opc2) { // Shifts in the same direction
        ...
        if (opc1 == CPUI_INT_LEFT) {
            ...
            if (secvn->loneDescend() == (PcodeOp*)0)
                return 0;
...

{
    ...
    if (leftshift->code() != CPUI_INT_LEFT)
        return 0;
    if (!leftshift->getIn(1)->isConstant())
        return 0;
    uintb sa = op->getIn(1)->getOffset();
    if (leftshift->getIn(1)->getOffset() != sa)
...

{
    ...
    if (mainvn->isFree())
        return 0;
    if (mainvn->getSize() > sizeof(uintb))
        return 0; // FIXME: uintb should be arbitrary precision
...

{
    ...
    if (!compvn1->isHeritageKnown())
        return 0;
    if (!compvn2->isHeritageKnown())
        return 0;
...
{ // Convert [(s)lessequal AND notequal] to (s)less
    ...
    if ((opc != CPUI_INT_LESSEQUAL) && (opc != CPUI_INT_SLESSEQUAL)) {
        ...
        op_equal = vnout2->getDef();
    if (op_equal->code() != CPUI_INT_NOTEQUAL)
        return 0;
...
{
    uint4 list[] = {CPUI_INT_NOTEQUAL,  CPUI_INT_SLESS,      CPUI_INT_LESS,   CPUI_BOOL_XOR,
                    CPUI_BOOL_AND,      CPUI_BOOL_OR,        CPUI_INT_EQUAL,  CPUI_INT_SLESSEQUAL,
                    CPUI_INT_LESSEQUAL, CPUI_INT_XOR,        CPUI_INT_AND,    CPUI_INT_OR,
...

{
    ...
    if (in0 != in1) { // Inputs must be identical
        ...
        if (!in1->isWritten())
            return 0;
        if (!in0->getDef()->isCseMatch(in1->getDef()))
            return 0; // or constructed identically
    }
    switch (op->code()) {
...
{
    uint4 list[] = {CPUI_INT_EQUAL, CPUI_INT_NOTEQUAL, CPUI_INT_LESS, CPUI_INT_LESSEQUAL};
...

{
    ...
    if (!vn2->isConstant())
        return 0;
    zext = vn1->getDef();
    if (!zext->getIn(0)->isHeritageKnown())
        return 0;
...

{
    ...
    if (opc0 != CPUI_BOOL_AND && opc0 != CPUI_BOOL_OR)
        return 0;
    PcodeOp* op1 = vn1->getDef();
    OpCode opc1 = op1->code();
    if (opc1 != CPUI_BOOL_AND && opc1 != CPUI_BOOL_OR)
        return 0;
    Varnode* ins[4];
    ins[0] = op0->getIn(0);
    ins[1] = op0->getIn(1);
    ins[2] = op1->getIn(0);
    ins[3] = op1->getIn(1);
    if (ins[0]->isFree() || ins[1]->isFree() || ins[2]->isFree() || ins[3]->isFree())
        return 0;
    bool isflipped[4];
    ...
    if (isMatch(ins[0], ins[2], isflipped[2])) {
        ...
        return 0;
    if (isflipped[leftSlot] != isflipped[rightSlot])
        return 0;
    OpCode combineOpc;
    if (centralEqual) {
...

{
    ...
    if (!subbool->isBooleanValue(data.isTypeRecoveryOn()))
        return 0;
...

{
    ...
    boolVn1 = op->getIn(0);
    if (!boolVn1->isBooleanValue(data.isTypeRecoveryOn()))
        return 0;

    multop1 = op->getOut()->loneDescend();
    if (multop1 == (PcodeOp*)0)
        return 0;
    if (multop1->code() != CPUI_INT_MULT)
        return 0;
    if (!multop1->getIn(1)->isConstant())
        return 0;
    ...
    if (multop2 == (PcodeOp*)0)
        return 0;
    if (multop2->code() != CPUI_INT_MULT)
        return 0;
    if (!multop2->getIn(1)->isConstant())
        return 0;
    coeff = multop2->getIn(1)->getOffset();
    if (coeff != calc_mask(size))
        return 0;
    zextop2 = multop2->getIn(0)->getDef();
    if (zextop2 == (PcodeOp*)0)
        return 0;
    if (zextop2->code() != CPUI_INT_ZEXT)
        return 0;
    boolVn2 = zextop2->getIn(0);
    if (!boolVn2->isBooleanValue(data.isTypeRecoveryOn()))
        return 0;
...

{
    ...
    boolVn = op->getIn(0);
    if (!boolVn->isBooleanValue(data.isTypeRecoveryOn()))
        return 0;
...

{
    ...
    while (j < matchlist.size()) {
        copyr = matchlist[j++];
        if (copyr->isMark())
            continue; // A varnode we have seen before
        ...
        for (j = 0; j < skiplist.size(); ++j) { // Collapse everything in the skiplist
            ...
            if (func_eq) { // We have only functional equality
                ...
                for (int4 i = 0; i < newop->numInput(); ++i) {
                    ...
                    if (!invn->isConstant()) {
                        substitute = Funcdata::cseFindInBlock(newop, invn, op->getParent(),
                                                              earliest); // Has newop already been copied in this block
                        break;
                    }
                }
...

{
    ...
    for (iter = svn->beginDescend(); iter != svn->endDescend(); ++iter) {
        ...
        if (!signop->getIn(0)->constantMatch(0)) {
            if (!signop->getIn(1)->constantMatch(0))
                continue;
            zside = 1;
        } else
            zside = 0;
...

{
    ...
    // Check for trivial case
    if ((bvn->isConstant() && bvn->getOffset() == 0) || (avn->isConstant() && avn->getOffset() == 0)) {
        data.opSetOpcode(op, CPUI_COPY);
        data.opSetInput(op, data.newConstant(1, 0), 0);
        data.opRemoveInput(op, 1);
        return 1;
...
{
    uint4 list[] = {CPUI_INT_ADD, CPUI_INT_XOR, CPUI_INT_OR, CPUI_BOOL_XOR, CPUI_BOOL_OR, CPUI_INT_MULT};
...
{
    OpCode opc = op->code();
    if (opc != CPUI_INT_LEFT && opc != CPUI_INT_MULT)
        return -1;
...

{
    ...
    if (!vn1->isWritten())
        return 0;
    zexthiop = vn1->getDef();
    if ((zexthiop->code() != CPUI_INT_ZEXT) && (zexthiop->code() != CPUI_INT_SEXT))
        return 0;
    ...
    if (zextloop->code() != CPUI_INT_ZEXT) {
        ...
        if (rShiftOp->code() != CPUI_INT_SRIGHT)
            return 0;
        if (!rShiftOp->getIn(1)->isConstant())
            return 0;
...

{
    ...
    if (!op->isCollapsible())
        return 0; // Expression must be collapsible
...

{
    ...
    if (!invn->isHeritageKnown())
        return 0; // Don't propagate free's away from their first use
    list<PcodeOp*>::const_iterator iter = vn->beginDescend();
    int4 count = 0;
    while (iter != vn->endDescend()) {
        PcodeOp* otherOp = *iter;
        ++iter;
        if (otherOp->isReturnCopy())
            continue;
        for (int4 slot = 0; slot < otherOp->numInput(); ++slot) {
            if (vn != otherOp->getIn(slot))
                continue;
            if (otherOp->isMarker()) {
                if (invn->isConstant())
...

{
    ...
    if (xorop == (PcodeOp*)0)
        return 0;
    if (xorop->code() != CPUI_INT_XOR)
        return 0;
    if (op->getIn(0)->loneDescend() == (PcodeOp*)0)
        return 0;
    coeff1 = op->getIn(1)->getOffset();
    Varnode* xorvn = xorop->getIn(1);
    if (xorop->getIn(0)->isFree())
    ...
    if (!c[1]->isConstant()) {
        ...
        for (int4 i = 0; i < 2; ++i) {
            ...
            if (!basevn->isSpacebase())
                continue; // Only apply this particular case if we are adding to a base pointer
            if (!basevn->isInput())
                continue; // because this adds a new add operation

            uintb val =
                op->getOpcode()->evaluateBinary(c[0]->getSize(), c[0]->getSize(), c[0]->getOffset(), c[1]->getOffset());
...

{
    ...
    if (ext0In->getSize() == ext1In->getSize()) {
        ...
        if (ext0In->isFree() && !ext0In->isConstant())
            return false;
        if (longform->getIn(1)->loneDescend() != longform)
            return false;
...

{
    ...
    switch (longform->code()) { // Determine if this op commutes with SUBPIECE
        ...
        case CPUI_INT_ADD:
            ...
            if (longform->getIn(0)->isSpacebase())
...

{
    ...
    if (highvn->isConstant() && highvn->getOffset() == 0)
        return 0;
    PcodeOp* newop = data.newOp(1, op->getAddr());
    Varnode* outvn = data.newUniqueOut(op->getOut()->getSize(), newop);
    data.opSetOpcode(newop, CPUI_INT_ZEXT);
...

{
    ...
    if (subop->code() == CPUI_SUBPIECE) {
        ...
        if (basevn->isFree())
            return 0;
        if (basevn->getSize() != op->getOut()->getSize())
            return 0; // Truncating then extending to same size
        if (basevn->getSize() > sizeof(uintb))
            return 0;
        if (subop->getIn(1)->getOffset() != 0) { // If truncating from middle
            if (subvn->loneDescend() != op)
                return 0; // and there is no other use of the truncated value
...

{
    ...
    if (b->isConstant() && c->isConstant()) {
        ...
        if ((b->getNZMask() & aMask) == 0)
            return 0; // RuleAndDistribute would reverse us
        if ((c->getNZMask() & aMask) == 0)
            return 0; // RuleAndDistribute would reverse us
...

{
    ...
    if (jt->numEntries() == 0)
        return 0;
    if (!jt->isLabelled())
        return 0; // Labels must be recovered (as this discovers multistage issues)
...

{
    ...
    if (!posvn->isHeritageKnown())
        return 0;
    if (!unnegvn->isHeritageKnown())
        return 0;
...
AddTreeState::AddTreeState(Funcdata& d, PcodeOp* op, int4 slot) : data(d) {
    baseOp = op;
    baseSlot = slot;
    biggestNonMultCoeff = 0;
    ptr = op->getIn(slot);
    ...
    if (multNode != (Varnode*)0) {
        ...
        if (ptr->getType()->needsResolution()) {
            if (((TypePointer*)ptr->getType())->getPtrTo()->getSize() == baseType->getSize())
                data.forceFacingType(ptr->getType(), -1, newop,
                                     0); // Force type not to resolve before the index is applied
            else
...

{
    ...
    for (iter = outVn->beginDescend(); iter != outVn->endDescend(); ++iter) {
        ...
        } else if ((opc == CPUI_LOAD || opc == CPUI_STORE) &&
                   decOp->getIn(1) == outVn) { // If use is as pointer for LOAD or STORE
    ...
    if (count > 1) {
        if (outVn->isSpacebase())
            return 0; // For the RESULT to be a spacebase pointer it must have only 1 descendant
                      //    res = 2;		// Uncommenting this line will not let pointers get pushed to multiple
                      //    descendants
    }
    return res;
}
...

{
    ...
    if (ct->isFormalPointerRel() && ((TypePointerRel*)ct)->evaluateThruParent(0)) {
        ...
        if (subType == (Datatype*)0)
            return 0;
        if (subType->getSize() < movesize)
            return 0; // Subtype is too small to handle LOAD/STORE
        ...
        if (newoff != 0) {
            ...
            PcodeOp* addop =
                data.newOpBefore(op, CPUI_INT_ADD, newop->getOut(), data.newConstant(ptrVn->getSize(), newoff));
...

{
    ...
    for (slot = 0; slot < op->numInput(); ++slot) { // Search for pointer type
        vni = op->getIn(slot);
        if (vni->getTypeReadFacing(op)->getMetatype() == TYPE_PTR)
            break;
    }
    if (slot == op->numInput())
        return 0;
...

{
    ...
    if (dt->getMetatype() == TYPE_PTR) { // Make sure we are still a pointer
        ...
        if (tp->getPtrTo()->getAlignSize() ==
            AddrSpace::addressToByteInt(size, tp->getWordSize())) { // of the correct size
...

{
    ...
    while (op != (PcodeOp*)0) {
        ...
        if (opc == CPUI_INT_ADD) {
            ...
            extra += getConstOffsetBack(op->getIn(1 - slot), submultiplier,
                                        DEPTH_LIMIT); // Get any constants from other input
            if (submultiplier > multiplier)
                multiplier = submultiplier;
...

{
    ...
    if (sbType->getMetatype() != TYPE_PTR)
        return 0;
    Datatype* dt = ((TypePointer*)sbType)->getPtrTo();
    if (dt->getMetatype() != TYPE_SPACEBASE)
        return 0;
...

{
    ...
    if (inVn->isAddrTied())
        return 0;
    Varnode* outVn = op->getOut();
    if (outVn->isTypeLock() || outVn->isNameLock())
        return 0;
    if (outVn->isAddrForce() || outVn->isAddrTied())
        return 0;
    list<PcodeOp*>::const_iterator iter;
...

{
    ...
    for (;;) {
        ...
        if (ct == (Datatype*)0)
            return true; // Don't know what it spans, assume multiple
        if (newOff + size > ct->getSize())
            return true; // Spans more than 1
...

{
    ...
    for (int4 i = 0; i < stack.size(); ++i) {
        ...
        if (op->code() != CPUI_INT_ZEXT)
            continue;
        if (!spanningRange(structuredType, node.getTypeOffset(), vn->getSize()))
            continue;
...

{
    ...
    if (root->isAddrTied())
        return false;
    if (!leaf->isWritten())
        return true; // Assume to be different symbols
    if (leaf->isProtoPartial())
...
{
    if (op->isPartialRoot())
        return 0; // Check if CONCAT tree already been visited
    Varnode* outvn = op->getOut();
    int4 baseOffset;
...

{
    ...
    if (subpieceop->code() != CPUI_SUBPIECE)
        return 0;
    uint4 n = subpieceop->getIn(1)->getOffset() * 8;
    if (n > 127 || n != 8 * (subpieceop->getIn(0)->getSize() - z->getSize()))
        return 0;
    ...
    for (iter = op->getOut()->beginDescend(); iter != op->getOut()->endDescend(); ++iter) {
        ...
        if (addop->code() != CPUI_INT_ADD)
            continue;
        if ((addop->getIn(0) != z) && (addop->getIn(1) != z))
            continue;
...

{
    ...
    if (shiftopc == CPUI_INT_RIGHT || shiftopc == CPUI_INT_SRIGHT) {
        ...
        if (shiftopc != CPUI_SUBPIECE)
            return (Varnode*)0; // In this case SUBPIECE is not optional
        shiftopc = CPUI_MAX;
    }
    ...
    if (curOp->code() != CPUI_INT_MULT)
        return (Varnode*)0; // There MUST be an INT_MULT
    Varnode* inVn = curOp->getIn(0);
    if (!inVn->isWritten())
        return (Varnode*)0;
...

{
    ...
    for (iter = vn->beginDescend(); iter != vn->endDescend(); ++iter) {
        ...
        if (!cvn->isConstant())
            return true; // Might be a form where constant has propagated yet
        uint4 n, xsize;
        uint8 y[2];
        OpCode extopc;
...

{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (!multout->isWritten())
            continue;
        multop = multout->getDef();
        if (multop->code() != CPUI_INT_MULT)
            continue;
        if (!multop->getIn(1)->isConstant())
            continue;
...

{
    ...
    if (opc2 == CPUI_INT_DIV && bitcount > sz * 8)
        return 0; // Unsigned overflow
    if (opc2 == CPUI_INT_SDIV && bitcount > sz * 8 - 2)
...

{
    ...
    if ((int4)constVn->getOffset() != sizeout * 8 - 1)
        return 0;
    if (!inVn->isWritten())
        return 0;
    ...
    if (c + sizeout != multSize)
        return 0; // Must be extracting high part
    if (!multOut->isWritten())
        return 0;
    ...
    if (otherVn->isConstant()) {
        if (otherVn->getOffset() > calc_mask(sizeout))
            return 0;
        if (2 * sizeout > multSize)
            return 0;
    } else if (otherVn->isWritten()) {
        PcodeOp* zextOp = otherVn->getDef();
        if (zextOp->code() != CPUI_INT_ZEXT)
            return 0;
        if (zextOp->getIn(0)->getSize() + sizeout > multSize)
            return 0;
    } else
        return 0;
    data.opSetInput(op, a, 0);
...

{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (unshiftop->code() == CPUI_INT_RIGHT) {
            if (!unshiftop->getIn(1)->isConstant())
                continue;
            break;
        }
    }
    ...
    if (sshiftop->code() != CPUI_INT_SRIGHT)
        return 0;
...

{
    ...
    for (iter1 = outvn->beginDescend(); iter1 != outvn->endDescend(); ++iter1) {
        ...
        if (div2->isConstant()) {
            ...
            if (!div2->isWritten())
                continue;
            if (div2->getDef()->code() != CPUI_INT_2COMP)
                continue;
            if (div2->getDef()->getIn(0) != div)
                continue;
        }
...

{
    ...
    for (iter = correctVn->beginDescend(); iter != correctVn->endDescend(); ++iter) {
        ...
        if (truncSize >= 0) {
            ...
            if (subOp->code() != CPUI_SUBPIECE)
                continue;
            if ((int4)subOp->getIn(1)->getOffset() != truncSize)
                continue;
            extVn = subOp->getIn(0);
        }
        if (a != extVn)
...

{
    ...
    if (base != otherBase) {
        if (!base->isWritten() || !otherBase->isWritten())
            return 0;
        ...
        if (truncAmt + base->getSize() != subOp->getIn(0)->getSize())
            return 0; // Must truncate all but high part
        base = subOp->getIn(0);
    ...
    if (trunc) {
        ...
        if (extOp == (PcodeOp*)0 || extOp->code() != CPUI_INT_ZEXT)
            return 0;
        andOut = extOp->getOut();
    }
    list<PcodeOp*>::const_iterator iter;
...

{
    ...
    for (slot = 0; slot < 2; ++slot) {
        ...
        if (!signExt->isWritten())
            continue;
        PcodeOp* shiftOp = signExt->getDef();
        if (shiftOp->code() != CPUI_INT_SRIGHT)
            continue;
        if (shiftOp->getIn(0) != base)
            continue;
        constVn = shiftOp->getIn(1);
        if (!constVn->isConstant())
            continue;
        if ((int4)constVn->getOffset() != 8 * base->getSize() - 1)
            continue;
        return base;
    }
    return (Varnode*)0;
...

{
    ...
    if (bl->getIn(1 - innerSlot) != decision)
        return (Varnode*)0;
    ...
    if (lessOp->code() != CPUI_INT_SLESS)
        return (Varnode*)0;
    if (!lessOp->getIn(1)->isConstant())
        return (Varnode*)0;
...
RulePtrFlow::RulePtrFlow(const string& g, Architecture* conf) : Rule(g, 0, "ptrflow") {
    glb = conf;
...
{
    if (!hasTruncations)
        return; // Only stick ourselves into pool if aggresiveness is turned on
    oplist.push_back(CPUI_STORE);
    oplist.push_back(CPUI_LOAD);
    oplist.push_back(CPUI_COPY);
    oplist.push_back(CPUI_MULTIEQUAL);
...

{
    ...
    if (!gatherExpression(bool0, opList0, rootblock0, inblock0))
        return 0;
    vector<PcodeOp*> opList1;
    if (!gatherExpression(bool1, opList1, rootblock0, inblock1))
        return 0;
...

{
    ...
    for (int4 i = 0; i < 2; ++i) {
        ...
        if (!def2->isBoolOutput())
            continue;
        if (!def2->getOpcode()->isFloatingPointOp())
            continue;
        if (def2->numInput() != 2)
            continue;
...
/// \brief Test if a boolean expression incorporates a floating-point comparison, and remove the NaN data-flow if it
/// does
///
...
Varnode* RuleIgnoreNan::testForComparison(Varnode* floatVar, PcodeOp* op, int4 slot, OpCode matchCode, int4& count,
                                          Funcdata& data)
...

{
    ...
    if (orop->code() != CPUI_INT_OR)
        return 0;
    if (!orop->getIn(0)->isWritten() || !orop->getIn(1)->isWritten())
        return 0;
    ...
    if (shiftop->code() != CPUI_INT_RIGHT)
        return 0;
    if (!shiftop->getIn(1)->constantMatch(1))
        return 0; // Shift to right by 1 exactly to clear high-bit
    Varnode* basevn = shiftop->getIn(0);
    if (basevn->isFree())
        return 0;
    if (andop->code() == CPUI_INT_ZEXT) {
        if (!andop->getIn(0)->isWritten())
            return 0;
        andop = andop->getIn(0)->getDef();
    }
    if (andop->code() != CPUI_INT_AND)
        return 0;
    if (!andop->getIn(1)->constantMatch(1))
        return 0; // Mask off least significant bit
    ...
    for (iter = outvn->beginDescend(); iter != outvn->endDescend(); ++iter) {
        PcodeOp* addop = *iter;
        if (addop->code() != CPUI_FLOAT_ADD)
            continue;
        if (addop->getIn(0) != outvn)
            continue;
        if (addop->getIn(1) != outvn)
            continue;
...

{
    ...
    if (zextop->code() != CPUI_INT_ZEXT)
        return 0; // Original FLOAT_INT2FLOAT must be unsigned form
    ...
    if (multiop->code() != CPUI_MULTIEQUAL)
        return 0; // Output comes together with 1 other flow
    if (multiop->numInput() != 2)
        return 0;
    int4 slot = multiop->getSlot(op->getOut());
    Varnode* otherout = multiop->getIn(1 - slot);
    if (!otherout->isWritten())
        return 0;
    PcodeOp* op2 = otherout->getDef();
    if (op2->code() != CPUI_FLOAT_INT2FLOAT)
        return 0; // The other flow must be a signed FLOAT_INT2FLOAT
    if (basevn != op2->getIn(0))
        return 0;      // taking the same input
    int4 dir2unsigned; // Control path to unsigned conversion
    FlowBlock* cond =
        FlowBlock::findCondition(multiop->getParent(), slot, multiop->getParent(), 1 - slot, dir2unsigned);
    if (cond == (FlowBlock*)0)
        return 0;
    PcodeOp* cbranch = cond->lastOp();
    if (cbranch == (PcodeOp*)0 || cbranch->code() != CPUI_CBRANCH)
        return 0;
    if (!cbranch->getIn(1)->isWritten())
        return 0;
    if (cbranch->isBooleanFlip())
        return 0;
    PcodeOp* compare = cbranch->getIn(1)->getDef();
    if (compare->code() != CPUI_INT_SLESS)
        return 0;
    if (compare->getIn(1)->constantMatch(0)) { // If condition is (basevn < 0)
        if (compare->getIn(0) != basevn)
            return 0;
        if (dir2unsigned != 1)
            return 0; // True branch must be the unsigned FLOAT_INT2FLOAT
    } else if (compare->getIn(0)->constantMatch(calc_mask(basevn->getSize()))) { // If condition is (-1 < basevn)
        if (compare->getIn(1) != basevn)
            return 0;
        if (dir2unsigned == 1)
            return 0; // True branch must be to signed FLOAT_INT2FLOAT
    } else
        return 0;
...

{
    ...
    if (vn2->isConstant()) { // Form 1 :  (z + z) - 1
        ...
        if (mask != vn2->getOffset())
            return (PcodeOp*)0; // Match the -1
        vn1 = op->getIn(0);
        if (!vn1->isWritten())
            return (PcodeOp*)0;
        addop = vn1->getDef();
        if (addop->code() != CPUI_INT_ADD)
            return (PcodeOp*)0; // Match the add
        tmpvn = addop->getIn(0);
        if (!tmpvn->isWritten())
            return (PcodeOp*)0;
        zext1 = tmpvn->getDef();
        ...
        if (!tmpvn->isWritten())
            return (PcodeOp*)0;
        zext2 = tmpvn->getDef();
        if (zext2->code() != CPUI_INT_ZEXT)
            return (PcodeOp*)0; // Match the second zext
...

{
    ...
    if (!tmpvn->isWritten())
        return 0;
    if (tmpvn->getDef()->code() != CPUI_INT_ADD)
        return 0;
    ...
    if ((!avn->isConstant()) && (avn->isFree()))
        return 0;
    if ((!bvn->isConstant()) && (bvn->isFree()))
        return 0;
    switch (form) {
...

{
    ...
    for (iter = outVn->beginDescend(); iter != outVn->endDescend(); ++iter) {
        ...
        if (tmpVn->getOffset() != 1)
            continue; // Masking 1 bit means we are checking parity of POPCOUNT input
        if (tmpVn->getSize() != 1)
        ...
        if (count == 2) {
            ...
            if (b1 == (Varnode*)0 && constRes0 != 1)
                continue;
            Varnode* b2 = getBooleanResult(inVn, pos1, constRes1);
            if (b2 == (Varnode*)0 && constRes1 != 1)
                continue;
            if (b1 == (Varnode*)0 && b2 == (Varnode*)0)
                continue;
            if (b1 == (Varnode*)0)
...

{
    ...
    if (opc == CPUI_SUBPIECE) {
        if (subOp->getIn(1)->getOffset() == 0)
            return 0;
        if (!isPathology(subOp->getIn(0), data))
            return 0;
    } else if (opc == CPUI_INDIRECT) {
        if (!subOp->isIndirectCreation())
            return 0; // Indirect concatenation
        ...
        if (addr != vn->getAddr())
            return 0; // into a contiguous register
    } else
        return 0;
...
{
    uint4 list[] = {CPUI_FLOAT_EQUAL,       CPUI_FLOAT_NOTEQUAL, CPUI_FLOAT_LESS,  CPUI_FLOAT_LESSEQUAL,
                    CPUI_FLOAT_NAN,         CPUI_FLOAT_ADD,      CPUI_FLOAT_DIV,   CPUI_FLOAT_MULT,
                    CPUI_FLOAT_SUB,         CPUI_FLOAT_NEG,      CPUI_FLOAT_ABS,   CPUI_FLOAT_SQRT,
                    CPUI_FLOAT_FLOAT2FLOAT, CPUI_FLOAT_CEIL,     CPUI_FLOAT_FLOOR, CPUI_FLOAT_ROUND,
...

{
    ...
    if (rootPtr->isWritten()) {
        ...
        if (defOp->code() == CPUI_INT_ADD && defOp->getIn(1)->isConstant()) {
            ...
            offset = off;
            if (defOp->getOut()->loneDescend() == (PcodeOp*)0)
        ...
        elType = rootPtr->getTypeReadFacing(op);
    if (elType->getMetatype() != TYPE_PTR)
        return 0;
    elType = ((TypePointer*)elType)->getPtrTo();
    if (elType->getSize() <= outSize)
        return 0; // Pointer data-type must be bigger than LOAD
