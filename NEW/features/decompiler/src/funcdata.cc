// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/funcdata.cc.
...
void Funcdata::spacebaseConstant(PcodeOp* op, int4 slot, MapEntry* entry, const Address& rampoint, uintb origval,
                                 int4 origsize)
...

{
    ...
    if (iter == endOp(CPUI_RETURN))
        return 0; // If no return statements, answer is irrelevant
...
void Funcdata::encodeVarnode(Encoder& encoder, VarnodeLocSet::const_iterator iter,
...

{
    ...
    for (int4 i = 0; i < glb->numSpaces(); ++i) {
        ...
        if (base == (AddrSpace*)0 || base->getType() == IPTR_IOP)
            continue;
...
/// \return the associated field as a ResolvedUnion or null
const ResolvedUnion* Funcdata::getAddressBasedUnionField(const Datatype* unresType, const Address& addr,
                                                         int4 slot) const
    ...
    if (op->code() == CPUI_MULTIEQUAL && slot >= 0) {
        ...
        for (int4 i = 0; i < op->numInput(); ++i) {
            if (i == slot)
                continue;
            if (op->getIn(i) != vn)
                continue; // Check that different input slot holds same Varnode
...
bool Funcdata::setAddressBasedUnionField(const Datatype* unresType, const Address& addr, int4 slot,
...

{
    ...
    if (unresType->isPointerRel()) {
        ...
        unresType =
            glb->types->getTypePointer(unresType->getSize(), baseType, ((TypePointer*)unresType)->getWordSize());
    }
...
void Funcdata::debugSetRange(const Address& pclow, const Address& pchigh, uintm uqlow, uintm uqhigh)
