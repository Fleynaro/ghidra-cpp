// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/architecture.cc.
...
void Architecture::addSpacebase(AddrSpace* basespace, const string& nm, const VarnodeData& ptrdata, int4 truncSize,
...

{
    ...
    SpacebaseSpace* spc =
        new SpacebaseSpace(this, translate, nm, ind, truncSize, basespace, ptrdata.space->getDelay() + 1, isFormal);
...

{
    ...
    if (range.getSpace()->isOverlayBase()) { // If the address space is overlayed
        ...
        for (int4 i = 0; i < num; ++i) {
            ...
            if (ospc == (AddrSpace*)0 || !ospc->isOverlay())
                continue;
...

{
    ...
    addSpacebase(basespace, "stack", point, truncSize, isreversejustify, stackGrowth,
                 true); // Create the "official" stackpointer
}
...

{
    ...
    split_datatype_config =
        OptionSplitDatatypes::option_struct | OptionSplitDatatypes::option_array | OptionSplitDatatypes::option_pointer;
