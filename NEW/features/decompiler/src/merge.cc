// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/merge.cc.
...

{
    ...
    if (high_in->isTypeLock())      // If types are locked
        if (high_out->isTypeLock()) // dont merge unless
            if (high_in->getType() != high_out->getType())
    ...
    if (high_out->isInput()) {
        if (high_in->isPersist())
            return false;
        if ((high_in->isAddrTied()) && (!high_out->isAddrTied()))
            return false;
...

{
    ...
    if (high_out->isInput()) {
        ...
        if (vn->isIllegalInput() && (!vn->isIndirectOnly()))
            return false;
    }
    if (high_in->isInput()) {
...
{
    if (!mergeTestAdjacent(high_out, high_in))
        return false;
    ...
    for (int4 i = 0; i < bblocks.getSize(); ++i) { // Do merges in linear block order
        ...
        for (iter = bl->beginOp(); iter != bl->endOp(); ++iter) {
            ...
            if (op->code() != opc)
                continue;
            vn1 = op->getOut();
            if (!mergeTestBasic(vn1))
                continue;
            for (int4 j = 0; j < op->numInput(); ++j) {
                vn2 = op->getIn(j);
                if (!mergeTestBasic(vn2))
                    continue;
...

{
    ...
    for (oiter = vn->beginDescend(); oiter != vn->endDescend(); ++oiter) {
        ...
        while (iter != enditer) {
                ...
                if (boundtype == 2) { // We have to resolve things defined at same place
                    if (vn2->getDef() == (PcodeOp*)0) {
                        if (vn->getDef() == (PcodeOp*)0) {
                            if (vn < vn2)
                                continue; // Choose an arbitrary order if both are inputs
                        } else
                            continue;
                    } else {
                        if (vn->getDef() != (PcodeOp*)0) {
                    ...
                    if (op != PcodeOp::getOpFromConst(indop->getIn(1)->getAddr()))
                        continue;
                    if (overlaptype != 1) {
                        if (vn->copyShadow(indop->getIn(0)))
                            continue; // If INDIRECT input shadows vn, don't consider as intersection
                    } else {
                        int4 off = (int4)(vn->getOffset() - vn2->getOffset());
            ...
            if (insertop)
                break; // No need to continue iterating through blocks
        }
        if (insertop)
...

{
    ...
    for (i = 0; i < max; ++i)
        if (!mergeTest(op->getIn(i)->getHigh(), testlist))
            break;
...
{
    PcodeOp* op =
        PcodeOp::getOpFromConst(indop->getIn(1)->getAddr()); // Indirect effect op
    ...
    for (oiter = data.beginOpAlive(); oiter != data.endOpAlive(); ++oiter) {
        ...
        for (i = 0; i < op->numInput(); ++i) {
            ...
            if (vn1->getSize() != vn2->getSize())
                continue;
            if ((vn2->getDef() == (PcodeOp*)0) && (!vn2->isInput()))
                continue;
            high_in = vn2->getHigh();
            if (!mergeTestAdjacent(high_out, high_in))
                continue;
...

{
    ...
    for (i = 0; i < high->numInstances(); ++i) {
        ...
        if (op->code() != CPUI_COPY)
            continue; // vn must be defineed by copy
        if (op->getIn(0)->getHigh() == high)
            continue; // From something NOT in same high
...

{
    ...
    for (int4 i = 0; i < high->numInstances(); ++i) {
        ...
        if (op->code() == CPUI_COPY) { // If the write is not a COPY
            if (op->getIn(0) == inVn)
                continue; // from the same Varnode as domOp and subOp
        }
    ...
    for (iter = data.beginOpAlive(); iter != data.endOpAlive(); ++iter) {
        ...
        switch (op->code()) {
            ...
            case CPUI_PIECE: // Check if output is built out of pieces of itself
                ...
                p2 = v2->getHigh()->piece;
                p3 = v3->getHigh()->piece;
                if (p1 == (VariablePiece*)0)
                    break;
                if (p2 == (VariablePiece*)0)
                    break;
                if (p3 == (VariablePiece*)0)
                    break;
