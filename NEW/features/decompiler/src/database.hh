// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/database.hh.
...
public:
    ...
    SymbolRange(MapEntry* e, uintb f, uintb l) {
        entry = e;
        first = f;
        last = l;
    } ///< Constructor
...
public:
    ...
    bool isPiece(void) const {
        return is_piece;
    } ///< Is \b this a proper piece of the whole Symbol
    bool isDynamic(void) const {
        return (entrytype == dynamic_entry);
    } ///< Is \b storage \e dynamic
    bool isConflict(void) const {
        return (entrytype == conflict_entry);
    } ///< Does storage have potential Symbol conflicts
...
public:
    ...
    uintm getUnique(void) const {
        return uniq;
    } ///< Get id of PcodeOp defining the conflicting Varnode
    virtual void printEntry(ostream& s) const;
    virtual void encode(Encoder& encoder) const;
    virtual void decode(Decoder& decoder);
};
...
public:
    ...
    DynamicEntry(Symbol* sym) : SymbolEntry(sym) {
        hash = 0;
...
public:
    ...
    bool hasMergeProblems(void) const {
        return ((dispflags & merge_problems) != 0);
    ...
    MapEntry*
    getMapEntry(const Address& addr) const; ///< Get first mapping of the symbol that contains the given Address
    ...
    int4 getMapEntryPosition(
        const SymbolEntry* entry) const; ///< Position of given SymbolEntry within \b this multi-entry Symbol
    int4
    getResolutionDepth(const Scope* useScope) const; ///< Get number of scope names needed to resolve \b this symbol
...
public:
    ...
    EquateSymbol(Scope* sc) : Symbol(sc) {
        value = 0;
        category = equate;
    } ///< Constructor for use with decode
...
public:
    UnionFacetSymbol(Scope* sc, const string& nm, Datatype* unionDt, int4 fldNum,
                     bool isAddr = false); ///< Constructor from components
    UnionFacetSymbol(Scope* sc) : Symbol(sc) {
        fieldNum = -1;
        category = union_facet;
        addrBased = false;
    } ///< Constructor for decode
    int4 getFieldNumber(void) const {
        return fieldNum;
    } ///< Get the particular field associate with \b this
    bool isAddrBased(void) const {
        return addrBased;
    } ///< Return \b true if facet matches any PcodeOp at the address
    virtual void encode(Encoder& encoder) const;
    virtual void decode(Decoder& decoder);
};
...
public:
    ...
    MapIterator(const vector<EntryMap*>* m, vector<EntryMap*>::const_iterator cm,
    ...
    const MapEntry* operator*(void) const {
        return (*curiter).entry;
...
protected:
    ...
    static const Scope* stackAddr(const Scope* scope1, const Scope* scope2, const Address& addr,
                                  const Address& usepoint, MapEntry** addrmatch);
    static const Scope* stackContainer(const Scope* scope1, const Scope* scope2, const Address& addr, int4 size,
                                       const Address& usepoint, MapEntry** addrmatch);
    static const Scope* stackClosestFit(const Scope* scope1, const Scope* scope2, const Address& addr, int4 size,
                                        const Address& usepoint, MapEntry** addrmatch);
    static const Scope* stackFunction(const Scope* scope1, const Scope* scope2, const Address& addr,
                                      Funcdata** addrmatch);
    static const Scope* stackExternalRef(const Scope* scope1, const Scope* scope2, const Address& addr,
                                         ExternRefSymbol** addrmatch);
    static const Scope* stackCodeLabel(const Scope* scope1, const Scope* scope2, const Address& addr,
                                       LabSymbol** addrmatch);
    ...
    virtual void addRange(AddrSpace* spc, uintb first,
                          uintb last); ///< Add a memory range to the ownership of \b this Scope
    virtual void removeRange(AddrSpace* spc, uintb first,
                             uintb last); ///< Remove a memory range from the ownership of \b this Scope
...
public:
    ...
    void turnOnDebug(void) const {
        debugon = true;
    }
    void turnOffDebug(void) const {
        debugon = false;
    }
#endif
    ...
    virtual list<DynamicEntry*>::const_iterator
    beginDynamic(void) const = 0; ///< Beginning iterator to dynamic SymbolEntrys
    ...
    virtual void
    clearUnlockedCategory(int4 cat) = 0; ///< Clear unlocked symbols of the given category from \b this scope
    ...
    virtual string buildVariableName(const Address& addr, const Address& pc, Datatype* ct, int4& index,
                                     uint4 flags) const = 0;
    ...
    /// \param ind is, for the function_parameter category, the index position to set, and is unused for other
    /// categories
    ...
    MapEntry* queryProperties(const Address& addr, int4 size, const Address& usepoint,
                              uint4& flags) const;      ///< Find a Symbol or properties at the given address
    Funcdata* queryFunction(const Address& addr) const; ///< Look-up a function by address
    Funcdata*
    queryExternalRefFunction(const Address& addr) const;  ///< Look-up a function thru an \e external \e reference
    ...
    Scope* discoverScope(const Address& addr, int4 sz,
                         const Address& usepoint); ///< Find the owning Scope of a given memory range
    ...
    const Scope*
    findDistinguishingScope(const Scope* op2) const; ///< Find first ancestor of \b this not shared by given scope
    ...
    MapEntry* addMapPoint(Symbol* sym, const Address& addr,
...
protected:
    virtual Scope*
    buildSubScope(uint8 id, const string& nm); ///< Build an unattached Scope to be associated as a sub-scope of \b this
...
public:
    ...
    void addRange(Scope* scope, AddrSpace* spc, uintb first,
                  uintb last); ///< Add an address range to the \e ownership of a Scope
    void removeRange(Scope* scope, AddrSpace* spc, uintb first,
                     uintb last); ///< Remove an address range from \e ownership of a Scope
    ...
    Scope* findCreateScope(uint8, const string& nm,
                           Scope* parent); /// Find (and if not found create) a specific subscope
    ...
    void decodeScope(Decoder& decoder,
                     Scope* newScope);        ///< Register and fill out a single Scope from an XML \<scope> tag
