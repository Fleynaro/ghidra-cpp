// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/printlanguage.hh.
...
public:
    ...
    struct Atom {
        ...
        Atom(const string& nm, tagtype t, EmitMarkup::syntax_highlight hl) : name(nm) {
            type = t;
            highlight = hl;
        }

        /// \brief Construct a token for a data-type name
        Atom(const string& nm, tagtype t, EmitMarkup::syntax_highlight hl, const Datatype* c) : name(nm) {
            type = t;
            highlight = hl;
            ptr_second.ct = c;
        }
        ...
        Atom(const string& nm, tagtype t, EmitMarkup::syntax_highlight hl, const Datatype* c, int4 off,
             const PcodeOp* o)
            : name(nm) {
            type = t;
            highlight = hl;
        ...
        Atom(const string& nm, tagtype t, EmitMarkup::syntax_highlight hl, const PcodeOp* o) : name(nm) {
            type = t;
            highlight = hl;
            op = o;
        }
...
protected:
    ...
    } ///< Return \b true if the RPN stack is empty
    bool isModStackEmpty(void) const {
        return modstack.empty();
    } ///< Return \b true if the printing modification stack is empty
#endif
    ...
    void pushVn(const Varnode* vn, const PcodeOp* op,
                uint4 m); ///< Push an expression rooted at a Varnode onto the RPN stack
    void pushVnExplicit(const Varnode* vn, const PcodeOp* op); ///< Push an explicit variable onto the RPN stack
    void pushSymbolDetail(const Varnode* vn, const PcodeOp* op,
                          bool isRead); ///< Push symbol name with adornments matching given Varnode

    bool parentheses(
        const OpToken* op2); ///< Determine if the given token should be emitted in its own parenthetic expression
