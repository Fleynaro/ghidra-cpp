...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/database.cc.
...
SymbolEntry::SymbolEntry(Symbol* sym) : symbol(sym) {
    extraflags = 0;
    offset = 0;
    size = -1;
    entrytype = map_entry;
...
MapEntryConflict::MapEntryConflict(Symbol* sym, Varnode* vn) : MapEntry(sym) {
    addr = vn->getAddr();
    size = vn->getSize();
...

{
    for (int4 i = 0; i < mapentry.size(); ++i) {
        ...
        if (addr.getSpace() != entryaddr.getSpace())
            continue;
        if (addr.getOffset() < entryaddr.getOffset())
            continue;
...
FunctionSymbol::FunctionSymbol(Scope* sc, const string& nm, int4 size) : Symbol(sc) {
    fd = (Funcdata*)0;
    consumeSize = size;
    buildType();
    name = nm;
...
FunctionSymbol::FunctionSymbol(Scope* sc, int4 size) : Symbol(sc) {
    fd = (Funcdata*)0;
    consumeSize = size;
    buildType();
}
...
EquateSymbol::EquateSymbol(Scope* sc, const string& nm, uint4 format, uintb val) : Symbol(sc, nm, (Datatype*)0) {
    value = val;
    category = equate;
...
/// \param sc is the Scope that will contain the new Symbol
/// \param nm is the name of the new Symbol
LabSymbol::LabSymbol(Scope* sc, const string& nm) : Symbol(sc) {
    buildType();
    name = nm;
    displayName = nm;
}
...
/// \param sc is the Scope that will contain the new Symbol
LabSymbol::LabSymbol(Scope* sc) : Symbol(sc) {
    buildType();
}
...
/// \param nm is the name of the Symbol
ExternRefSymbol::ExternRefSymbol(Scope* sc, const Address& ref, const string& nm) : Symbol(sc, nm, (Datatype*)0) {
    refaddr = ref;
    buildNameType();
}
...
const Scope* Scope::stackAddr(const Scope* scope1, const Scope* scope2, const Address& addr, const Address& usepoint,
                              MapEntry** addrmatch) {
    MapEntry* entry;
    if (addr.isConstant())
...
/// \param addrmatch is used to pass-back any matching SymbolEntry
/// \return the Scope owning the address or NULL if none found
const Scope* Scope::stackContainer(const Scope* scope1, const Scope* scope2, const Address& addr, int4 size,
...
/// \param addrmatch is used to pass-back any matching SymbolEntry
/// \return the Scope owning the address or NULL if none found
const Scope* Scope::stackClosestFit(const Scope* scope1, const Scope* scope2, const Address& addr, int4 size,
                                    const Address& usepoint, MapEntry** addrmatch) {
    MapEntry* entry;
    if (addr.isConstant())
        return (const Scope*)0;
...
/// \return the Scope owning the address or NULL if none found
const Scope* Scope::stackFunction(const Scope* scope1, const Scope* scope2, const Address& addr, Funcdata** addrmatch) {
    Funcdata* fd;
    if (addr.isConstant())
        return (const Scope*)0;
    while ((scope1 != (const Scope*)0) && (scope1 != scope2)) {
...
/// \return the Scope owning the address or NULL if none found
const Scope* Scope::stackExternalRef(const Scope* scope1, const Scope* scope2, const Address& addr,
...
/// \param addrmatch is used to pass-back any matching Symbol
/// \return the Scope owning the address or NULL if none found
const Scope* Scope::stackCodeLabel(const Scope* scope1, const Scope* scope2, const Address& addr,
                                   LabSymbol** addrmatch) {
    LabSymbol* sym;
    if (addr.isConstant())
        return (const Scope*)0;
...

{
    if (onlyGlobal && (!isGlobal()))
        return; // Only save global scopes
    encode(encoder);
...
{
    if (sym->type->getMetatype() == TYPE_UNKNOWN)
...
{
    if (this == op2)
        return (const Scope*)0; // Quickly check most common cases
    if (parent == op2)
        return this;
    if (op2->parent == this)
        return (const Scope*)0;
    if (parent == op2->parent)
        return this;
...
ScopeInternal::ScopeInternal(uint8 id, const string& nm, Architecture* g) : Scope(id, nm, g, this) {
    nextUniqueId = 0;
...

{
    ...
    if (rangemap != (EntryMap*)0) {
        ...
        if (usepoint.isInvalid())
            res = rangemap->find(addr.getOffset(), EntryMap::subsorttype(false), EntryMap::subsorttype(true));
        else
            res = rangemap->find(addr.getOffset(), EntryMap::subsorttype(false), EntryMap::subsorttype(usepoint));
...

{
    ...
    if (rangemap != (EntryMap*)0) {
        pair<EntryMap::const_iterator, EntryMap::const_iterator> res;
        res = rangemap->find(addr.getOffset(), EntryMap::subsorttype(false), EntryMap::subsorttype(addr));
        while (res.first != res.second) {
            --res.second;
            MapEntry* entry = (*res.second).entry;
            if (entry->getAddr().getOffset() == addr.getOffset()) {
...
string ScopeInternal::buildVariableName(const Address& addr, const Address& pc, Datatype* ct, int4& index,
                                        uint4 flags) const {
    ostringstream s;
...

{
    SymbolNameTree::const_iterator iter = findFirstByName(nm);
    if (iter == nametree.end())
        return nm; // nm is already unique
...

{
    if (scope == globalscope)
        return; // Does not apply to the global scope
    if (scope->fd != (Funcdata*)0)
        return; // Does not apply to functional scopes
...
const Scope* Database::mapScope(const Scope* qpoint, const Address& addr, const Address& usepoint) const {
...
/// \param usepoint is a point in code where the address is being accessed (may be \e invalid)
/// \return a Scope to act as a starting point for a hierarchical search
Scope* Database::mapScope(Scope* qpoint, const Address& addr, const Address& usepoint) {
    if (resolvemap.empty()) // If there are no namespace scopes
        return qpoint;      // Start querying from scope placing query
    pair<ScopeResolve::const_iterator, ScopeResolve::const_iterator> res;
    res = resolvemap.find(addr);
...

{
    ...
    for (;;) {
        uint4 subId = decoder.peekElement();
        if (subId != ELEM_PROPERTY_CHANGEPOINT)
            break;
        decoder.openElement();
