// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/unify.cc.
...

{
    ...
    if (!vn->isConstant())
        return false;
    if (vn->getOffset() != (val & calc_mask(vn->getSize())))
        return false;
    return true;
}
...
{
    printstate.printIndent(s);
    s << "list<PcodeOp *>::const_iterator iter" << dec << printstate.getDepth() << ",enditer" << printstate.getDepth()
      << ';' << endl;
    printstate.printIndent(s);
    ...
    do {
        ...
        if (stateint == 0) { // Attempt a step at current constraint
            ...
            if (subconstraint->step(state)) {
                ...
                subindex -= 1;
                if (subindex < 0)
                    return false; // Popped off the top
                traverse->setCurrentIndex(subindex);
