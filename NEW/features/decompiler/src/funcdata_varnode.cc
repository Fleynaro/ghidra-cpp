// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/funcdata_varnode.cc.
...

{
    ...
    vflags &= (Varnode::annotation | Varnode::externref | Varnode::readonly | Varnode::persist | Varnode::addrtied |
               Varnode::addrforce | Varnode::indirect_creation | Varnode::incidental_copy | Varnode::volatil |
               Varnode::mapped);
...

{
    ...
    while (iter != vn->endDescend()) {
        ...
        if (op->getParent()->isDead())
            continue;
        if (op->getParent()->sizeIn() != 0)
            res = true;
        i = op->getSlot(vn);
    ...
    while (iter != vn->endDescend()) {
        ...
        if (op->isMarker()) { // Must be careful putting constants in here
            ...
            if (outvn->getAddr() == vn->getAddr())
                continue; // Ignore indirect to itself
...

{
    ...
    if (vn->isWritten()) { // A written value
        ...
        if (!vn->hasNoDescend())
            throw LowlevelError("Volatile memory was propagated");
        PcodeOp* defop = vn->getDef();
...

{
    ...
    for (; iter != enditer; ++iter) { // Loop over all inputs
        ...
        if (!vn->isIllegalInput())
            continue; // Only check illegal inputs
...

{
    ...
    if (indop->code() != CPUI_INDIRECT)
        return;
    if (indop->getIn(1)->getSpace()->getType() != IPTR_IOP)
        return;
...

{
    ...
    while (iter != enditer) {
        ...
        if (!otherVn->isWritten())
            break;
        if (vn->def->getSeqNum().getAddr() != otherVn->def->getSeqNum().getAddr())
            break;
...

{
    ...
    if (sym != (Symbol*)0)
        return sym; // Symbol already assigned
...

{
    ...
    if (vn->getSymbolEntry() != (SymbolEntry*)0)
        return false;                 // Varnode is already labeled
...

{
    ...
    if (vn == (Varnode*)0)
        return false;
    if (vn->getSymbolEntry() != (SymbolEntry*)0)
        return false;                           // Symbol already applied
...

{
    ...
    if (iter == vn->descend.end())
        return; // No descendants at all
    useop = *iter++;
    if (iter == vn->descend.end())
        return; // Only one descendant
    for (;;) {
...

{
    ...
    while (iter != enditer) {
        ...
        if (entry == (MapEntry*)0) {
            ...
            int4 index = 0;
            string symbolname =
                discover->buildVariableName(addr, usepoint, ct, index, Varnode::addrtied | Varnode::persist);
...
bool Funcdata::checkCallDoubleUse(const PcodeOp* opmatch, const PcodeOp* op, const Varnode* vn, uint4 fl,
                                  const ParamTrial& trial) const

{
    int4 j = op->getSlot(vn);
    if (j <= 0)
        return false; // Flow traces to indirect call variable, definitely not a param
    ...
    for (i = 0; i < varlist.size(); ++i) {
        ...
        for (iter = vn->descend.begin(); iter != vn->descend.end(); ++iter) {
            ...
            switch (op->code()) {
                ...
                case CPUI_CALL:
                case CPUI_CALLIND:
                    if (checkCallDoubleUse(opmatch, op, vn, curFlags, trial))
                        continue;
                    res = false;
                    break;
                case CPUI_INDIRECT:
                    ...
                    break;
                case CPUI_COPY:
                    if ((op->getOut()->getSpace()->getType() != IPTR_INTERNAL) && !op->isIncidentalCopy() &&
                        !vn->isIncidentalCopy()) {
...
bool Funcdata::ancestorOpUse(int4 maxlevel, const Varnode* invn, const PcodeOp* op, ParamTrial& trial, int4 offset,
...

{
    ...
    switch (def->code()) {
        ...
        case CPUI_PIECE:
            ...
            if (offset == 0)
                return ancestorOpUse(maxlevel - 1, def->getIn(1), op, trial, 0,
                                     mainFlags); // Follow into least sig piece
            if (offset == def->getIn(1)->getSize())
                return ancestorOpUse(maxlevel - 1, def->getIn(0), op, trial, 0,
                                     mainFlags); // Follow into most sig piece
            return false;
        case CPUI_SUBPIECE: {
            ...
            if (invn->getSpace()->getType() == IPTR_INTERNAL || def->isIncidentalCopy() ||
                def->getIn(0)->isIncidentalCopy() || invn->overlap(*def->getIn(0)) == newOff) {
...
/// \return the command indicating the next traversal step: push (enter_node), or pop (pop_success, pop_fail,
/// pop_solid...)
...

{
    ...
    if (!stateVn->isWritten()) {
        if (stateVn->isInput()) {
            ...
            if (stateVn->isPersist())
                return pop_success; // A global input, not active movement, but a valid possibility
    ...
    switch (op->code()) {
        case CPUI_INDIRECT:
            ...
            if (!op->isIndirectStore()) { // If flow goes THROUGH a call
                if (op->getOut()->isReturnAddress())
                    return pop_fail; // Storage address location is completely invalid
                if (trial->isKilledByCall())
                    return pop_fail; // "Likely" killedbycall is invalid
            }
        ...
        case CPUI_COPY: {
            ...
            // are viewed as just another node on the path to traverse
            if (op->getOut()->getSpace()->getType() == IPTR_INTERNAL || op->isIncidentalCopy() ||
                op->getIn(0)->isIncidentalCopy() || (op->getOut()->getAddr() == op->getIn(0)->getAddr())) {
                stateStack.push_back(State(op, 0));
                return enter_node; // Push into the new node
            }
