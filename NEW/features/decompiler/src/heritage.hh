// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/heritage.hh.
...
struct MemRange {
    ...
    MemRange(const Address& ad, int4 sz, uint4 fl) : addr(ad) {
        size = sz;
        flags = fl;
    } ///< Constructor
    bool newAddresses(void) const {
        return ((flags & new_addresses) != 0);
    } ///< Does \b this range cover new addresses?
    bool oldAddresses(void) const {
        return ((flags & old_addresses) != 0);
    } ///< Does \b this range cover old addresses?
...
public:
    ...
    iterator erase(iterator iter) {
        return tasklist.erase(iter);
    } ///< Remove a particular range
    iterator begin(void) {
        return tasklist.begin();
    } ///< Get iterator to beginning of \b this list
    iterator end(void) {
        return tasklist.end();
    } ///< Get iterator to end of \b this list
    void clear(void) {
        tasklist.clear();
...
class Heritage {
    ...
    int4 collect(MemRange& memrange, vector<Varnode*>& read, vector<Varnode*>& write, vector<Varnode*>& input,
    ...
    void splitPieces(const vector<Varnode*>& vnlist, PcodeOp* insertop, const Address& addr, int4 size,
                     Varnode* startvn);
    ...
    void guard(const Address& addr, int4 size, bool guardPerformed, vector<Varnode*>& read, vector<Varnode*>& write,
    ...
    void guardOutputOverlapStack(PcodeOp* callOp, const Address& addr, int4 size, const Address& retAddr, int4 retSize,
                                 vector<Varnode*>& write);
