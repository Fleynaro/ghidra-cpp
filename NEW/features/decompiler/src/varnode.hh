// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/varnode.hh.
...
public:
    ...
    Datatype*
    getTypeReadFacing(const PcodeOp* op) const; ///< Get the data-type of \b this when it is read by the given PcodeOp
    Datatype* getHighTypeDefFacing(void) const; ///< Return the data-type of the HighVariable when \b this is written to
    Datatype* getHighTypeReadFacing(
        const PcodeOp* op) const; ///< Return data-type of the HighVariable when read by the given PcodeOp
    ...
    bool isConsumeList(void) const {
        return ((addlflags & Varnode::lisconsume) != 0);
    } ///< Get marker used by dead-code algorithm
    bool isConsumeVacuous(void) const {
        return ((addlflags & Varnode::vacconsume) != 0);
    } ///< Get marker used by dead-code algorithm
    ...
    void
    printRaw(ostream& s) const; ///< Print a simple identifier plus additional info identifying Varnode with SSA form
    ...
    int4 characterizeOverlap(
        const Varnode& op) const; ///< Return 0, 1, or 2 for "no overlap", "partial overlap", "identical storage"
    int4 overlap(const Varnode& op) const; ///< Return relative point of overlap between two Varnodes
    int4 overlapJoin(const Varnode& op)
        const; ///< Return relative point of overlap, where the given Varnode may be in the \e join space
    ...
    bool isAnnotation(void) const {
        return ((flags & Varnode::annotation) != 0);
    ...
    bool isDirectWrite(void) const {
        return ((flags & Varnode::directwrite) != 0);
    ...
    bool isAddrTied(void) const {
        return ((flags & (Varnode::addrtied | Varnode::insert)) == (Varnode::addrtied | Varnode::insert));
    }
    bool isAddrForce(void) const {
        return ((flags & Varnode::addrforce) != 0);
    } ///< Is \b this value forced into a particular storage location?
    bool isAutoLive(void) const {
        return ((flags & (Varnode::addrforce | Varnode::autolive_hold)) != 0);
    } ///< Is \b this varnode exempt from dead-code removal?
    bool isAutoLiveHold(void) const {
        return ((flags & Varnode::autolive_hold) != 0);
    ...
    bool isReturnAddress(void) const {
        return ((flags & Varnode::return_address) != 0);
    ...
    bool isIndirectZero(void) const {
        return ((flags & (Varnode::indirect_creation | Varnode::constant)) ==
    ...
    bool isEventualConstant(int4 maxBinary,
                            int4 maxLoad) const; ///< Will \b this Varnode ultimately collapse to a constant

    /// Return \b true if this Varnode is linked into the SSA tree
    bool isHeritageKnown(void) const {
        return ((flags & (Varnode::insert | Varnode::constant | Varnode::annotation)) != 0);
    }
    ...
    void setSymbolCheck(uint4 val) {
        addlflags = (addlflags & ~symcheck_complete) | (val & symcheck_complete);
    } ///< Set state of checks for a constant symbol reference
    ...
    bool partialCopyShadow(const Varnode* op2,
                           int4 relOff) const; ///< Is one of \b this or \b op2 a partial copy of the other?
...
public:
    ...
    Varnode* createDef(int4 s, const Address& m, Datatype* ct,
                       PcodeOp* op);                             ///< Create a Varnode as the output of a PcodeOp
    ...
    bool hasInputIntersection(int4 s,
                              const Address& loc) const; ///< Check for input Varnode that overlaps the given range
    uint4 getCreateIndex(void) const {
        return create_index;
