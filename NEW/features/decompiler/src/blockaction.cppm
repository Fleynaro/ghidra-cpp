...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/blockaction.cc.
...
{
    if (trace->pathout != 0)
        return false; // Only check, if this is the first sibling
...
TraceDAG::TraceDAG(list<FloatingEdge>& lg) : likelygoto(lg) {
    activecount = 0;
...

{
    ...
    if ((bl->sizeIn() == 1) && (bl->getIn(0)->sizeOut() == 1))
        return false; // Must be start of chain
    ...
    if (!bl->isDecisionOut(0))
        return false; // Not a goto or a loopbottom
...

{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (i == 0) {
            ...
            if (otherblock->sizeIn() == 1 && otherblock->sizeOut() == 0 && !otherblock->isSwitchOut() &&
                bl->isDecisionOut(1)) {
    ...
    if (exitblock == (FlowBlock*)0) {
        ...
        for (int4 i = 0; i < sizeout; ++i) {
            ...
            if (curbl->isGotoIn(0))
                return false; // In cannot be a goto
            if (curbl->sizeOut() > 1)
                return false; // There can be at most 1 exit from a case
            if (curbl->sizeOut() == 1) {
                if (curbl->isGotoOut(0))
                    return false; // Out cannot be goto
...

{
    ...
    if (fallthru.empty())
        return false; // No fall thru candidates
...
CollapseStructure::CollapseStructure(BlockGraph& g) : graph(g) {
...
{
    joinblock =
        data.nodeJoinCreateBlock(block1, block2, exita, exitb, (a_in1 > a_in2), (b_in1 > b_in2), cbranch1->getAddr());
    setupMultiequals();
    moveCbranch();
