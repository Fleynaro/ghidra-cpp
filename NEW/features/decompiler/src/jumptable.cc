// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/jumptable.cc.
...
EmulateFunction::EmulateFunction(Funcdata* f) : EmulatePcodeOp(f->getArch()) {
    fd = f;
...
uintb EmulateFunction::emulatePath(uintb val, const PathMeld& pathMeld, PcodeOp* startop, Varnode* startvn) {
    uint4 i;
    for (i = 0; i < pathMeld.numOps(); ++i)
        if (pathMeld.getOp(i) == startop)
            break;
    if (startop->code() == CPUI_MULTIEQUAL) { // If we start on a MULTIEQUAL
        ...
        startvn =
            startop->getOut(); // So the output of the MULTIEQUAL is the new startvn (as if a COPY from old startvn)
...
void JumpModelTrivial::buildLabels(Funcdata* fd, vector<Address>& addresstable, vector<uintb>& label,
                                   const JumpModel* orig) const
...

{
    ...
    while (curvn != invn) {
        ...
        if (op->getEvalType() == PcodeOp::binary) {
            ...
            output =
                top->recoverInputBinary(slot, op->getOut()->getSize(), output, op->getIn(slot)->getSize(), otherval);
            curvn = op->getIn(slot);
...

{
    ...
    if (jumptable->isPartial())
        rng.setRange(0, 0, vn->getSize(),
                     step); // Don't restrict with non-zero mask if we restricted too far previously
    else
...

{
    ...
    if (!constvn2->isConstant())
        return 0;
    if (addop->getIn(0) != addop2->getIn(0))
        return 0;
    ...
    for (i = 0; i < maxbranch; ++i) {
        if ((pathout >= 0) && (bl->sizeOut() == 2)) {
            ...
            for (;;) {
                ...
                if (prevbl->sizeOut() != 1)
                    break;   // Is it possible to deviate from switch path in this block
...

{
    ...
    for (iter = selectguards.begin(); iter != selectguards.end(); ++iter) {
        ...
        if (matchval == 0)
            continue;
        if (rng.intersect(guard.getRange()) != 0)
            continue;
    }
...
void JumpBasic::findNormalized(Funcdata* fd, BlockBasic* rootbl, int4 pathout, const JumpModel* previous,
                               uint4 maxtablesize)
...
/// \param maxpullback is the maximum number of times to pull back from the guard CBRANCH to the putative switch
/// variable
...

{
    ...
    for (int4 j = 0; j < maxpullback; ++j) {
        ...
        if (rng.isEmpty())
            break;
        if (!BlockBasic::liftVerifyUnroll(varArray, readOp->getSlot(vn)))
            break;
    }
}
...

{
    ...
    if (cbranchblock->sizeOut() != 2)
        return false;                // In which case, we can't fold it in
    ...
    for (pos = 0; pos < switchbl->sizeOut(); ++pos)
        if (switchbl->getOut(pos) == guardtarget)
            break;
...

{
    ...
    while (i < pathMeld.numCommonVarnode()) {
        if (!flowsOnlyToModel(switchvn, normop))
...
void JumpBasic::buildLabels(Funcdata* fd, vector<Address>& addresstable, vector<uintb>& label,
                            const JumpModel* orig) const

{
    ...
    while (notdone) {
        ...
        if (origrange->isReversible()) { // If the current value is reversible
            ...
            try {
                switchval =
                    backup2Switch(fd, val, normalvn, switchvn); // Do reverse emulation to get original switch value
        ...
        if (label.size() >= addresstable.size())
            break;
...
bool JumpBasic::sanityCheck(Funcdata* fd, PcodeOp* indop, vector<Address>& addresstable, vector<LoadTable>& loadpoints,
    ...
    if (addr.getOffset() != 0) {
        for (i = 1; i < addresstable.size(); ++i) {
            if (addresstable[i].getOffset() == 0)
                break;
            diff = (addr.getOffset() < addresstable[i].getOffset()) ? (addresstable[i].getOffset() - addr.getOffset())
...
{ // Try to recover a jumptable using the second model
    ...
    if (multiop->numInput() != 2)
        return false; // Must be exactly 2 paths
...

{
    ...
    if (checkNormalDominance()) {                 // If the normal switch variable dominates the switch itself
        JumpBasic::findUnnormalized(maxaddsub, maxleftright,
                                    maxext); // We can use the basic form of calculating the unnormalized
        return;
    }
...
JumpBasicOverride::JumpBasicOverride(JumpTable* jt) : JumpBasic(jt) {
    startingvalue = 0;
    hash = 0;
    istrivial = false;
}
...
void JumpBasicOverride::buildLabels(Funcdata* fd, vector<Address>& addresstable, vector<uintb>& label,
                                    const JumpModel* orig) const
...

{
    ...
    if (assistOp == (PcodeOp*)0)
        return false;
    if (assistOp->code() != CPUI_CALLOTHER)
        return false;
    if (assistOp->numInput() < 3)
        return false;
    ...
    if (userop->getCalcSize() == -1) // If no size script, first param after switch var is size
        ...
        ExecutablePcode* pcodeScript =
            (ExecutablePcode*)fd->getArch()->pcodeinjectlib->getPayload(userop->getCalcSize());
        vector<uintb> inputs;
    ...
    if (previous != (const JumpModel*)0 &&
        previous->getTableSize() - 1 != sizeIndices) // table size has 1 added to it for the default case
...
void JumpAssisted::buildAddresses(Funcdata* fd, PcodeOp* indop, vector<Address>& addresstable,
    ...
    ExecutablePcode* defaultScript =
        (ExecutablePcode*)fd->getArch()->pcodeinjectlib->getPayload(userop->getDefaultAddr());
...
void JumpAssisted::buildLabels(Funcdata* fd, vector<Address>& addresstable, vector<uintb>& label,
                               const JumpModel* orig) const
...
JumpTable::JumpTable(Address ad) : opaddress(ad) {
    jmodel = (JumpModel*)0;
    origmodel = (JumpModel*)0;
    indirect = (PcodeOp*)0;
...

{
    IndexPair val(block2Position(bl), 0);
    int4 count = 0;
    vector<IndexPair>::const_iterator iter =
        lower_bound(block2addr.begin(), block2addr.end(), val, IndexPair::compareByPosition);
