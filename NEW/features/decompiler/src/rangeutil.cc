// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/rangeutil.cc.
    ...
    switch (opc) {
        ...
        case CPUI_INT_SRIGHT: {
            if (step == 1) {
                ...
                if (((left >= leftb) && (left <= rightb) && (right >= leftb) && (right <= rightb) && (left >= right)) ||
                    (left == right)) {
                    // covers everything in range of shift
                    left = 0; // So domain is everything
                    right = 0;
...
/// \return \b true if the result is known and forms a range
bool CircleRange::pushForwardBinary(OpCode opc, const CircleRange& in1, const CircleRange& in2, int4 inSize,
                                    int4 outSize, int4 maxStep)
    ...
    switch (opc) {
        ...
        case CPUI_INT_ADD:
            ...
            if (in1.left == in1.right || in2.left == in2.right) {
                ...
                step = (in1.step < in2.step) ? in1.step : in2.step; // Smaller step
                uintb size1 =
                    (in1.left < in1.right) ? (in1.right - in1.left) : (in1.mask - (in1.left - in1.right) + in1.step);
                left = (in1.left + in2.left) & mask;
...

{
    ...
    if (opCode == CPUI_MULTIEQUAL) {
        ...
        if (equations.size() == 0) {
            if (!res.pushForwardBinary(opCode, inSet1->range, inSet2->range, inSet1->vn->getSize(), vn->getSize(),
                                       MAX_STEP)) {
                setFull();
                return true;
            }
        ...
        if (!res.pushForwardTrinary(opCode, range1, range2, inSet3->range, inSet1->vn->getSize(), vn->getSize(),
                                    MAX_STEP)) {
            setFull();
            return true;
        }
...
void ValueSetSolver::constraintsFromPath(int4 type, CircleRange& lift, Varnode* startVn, Varnode* endVn,
                                         PcodeOp* cbranch)
