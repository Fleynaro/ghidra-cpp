// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/translate.hh.
...
class SpacebaseSpace : public AddrSpace {
    ...
    void setBaseRegister(const VarnodeData& data, int4 origSize,
                         bool stackGrowth); ///< Set the base register at time space is created
public:
    SpacebaseSpace(AddrSpaceManager* m, const Translate* t, const string& nm, int4 ind, int4 sz, AddrSpace* base,
...
public:
    ...
    Address getEquivalentAddress(uintb offset,
                                 int4& pos) const; ///< Given offset in \e join space, get equivalent address of piece
    bool operator<(const JoinRecord& op2) const;   ///< Compare records lexigraphically by pieces
    static void mergeSequence(vector<VarnodeData>& seq,
                              const Translate* trans); ///< Merge any contiguous ranges in a sequence
};
...
protected:
    ...
    void addSpacebasePointer(SpacebaseSpace* basespace, const VarnodeData& ptrdata, int4 truncSize,
                             bool stackGrowth);                  ///< Set the base register of a spacebase space
...
public:
    ...
    JoinRecord* findAddJoin(const vector<VarnodeData>& pieces,
                            uint4 logicalsize) const;       ///< Get (or create) JoinRecord for \e pieces
    ...
    Address constructJoinAddress(const Translate* translate, const Address& hiaddr, int4 hisz, const Address& loaddr,
                                 int4 losz) const;
