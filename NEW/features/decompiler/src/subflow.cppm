...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/subflow.cc.
    ...
    if (sextrestrictions) {
        if (vn->getSize() != flowsize) {
            if ((!aggressive) && vn->isInput())
                return (ReplaceVarnode*)0; // Cannot assume input is sign extended
            if (vn->isPersist())
                return (ReplaceVarnode*)0;
        }
        ...
        if (bitsize >= 8) { // Not a flag
            ...
            if ((!aggressive) &&
                ((vn->getConsume() & ~mask) != 0)) // If there is any use of value outside of the logical variable
        ...
        if (vn->isInput()) { // Must be careful with inputs
            ...
            if (bitsize < 8)
                return (ReplaceVarnode*)0; // Dont create input flag
            if ((mask & 1) == 0)
                return (ReplaceVarnode*)0; // Dont create unique input
...
SubvariableFlow::ReplaceOp* SubvariableFlow::createOpDown(OpCode opc, int4 numparam, PcodeOp* op, ReplaceVarnode* inrvn,
                                                          int4 slot)
...

{
    ...
    if (fd->getFuncProto().isOutputLocked())
        return false;
    if (!aggressive) {
        if ((rvn->vn->getConsume() & ~rvn->mask) != 0) // If there's something outside the mask being consumed
            return false;                              // Don't truncate
...

{
    ...
    if (fc->isOutputActive())
        return false; // Don't trim while in the middle of figuring out return value
    ...
    for (iter = rvn->vn->beginDescend(); iter != enditer; ++iter) {
        ...
        switch (op->code()) {
            ...
            case CPUI_INT_XOR:
                ...
                if (!createLink(rop, rvn->mask, -1, outvn))
                    return false;
                hcount += 1; // Dealt with this descendant
                break;
            case CPUI_INT_OR:
                if (doesOrSet(op, rvn->mask) != -1)
                    break; // Subvar set to 1s, truncate flow
            ...
            case CPUI_INT_AND:
                ...
                if (doesAndClear(op, rvn->mask) != -1)
                    break; // Subvar set to zero, truncate flow
            ...
            case CPUI_INT_MULT:
                ...
                if (bitsize + sa > 8 * rvn->vn->getSize())
                    return false;
                rop = createOpDown(CPUI_INT_MULT, 2, op, rvn, slot);
                if (!createLink(rop, rvn->mask << sa, -1, outvn))
                    return false;
                hcount += 1;
                break;
            case CPUI_INT_DIV:
            case CPUI_INT_REM:
                ...
                if (!op->getIn(0)->isZeroExtended(flowsize))
                    return false;
                if (!op->getIn(1)->isZeroExtended(flowsize))
                    return false;
            ...
            case CPUI_INT_LEFT:
                if (slot == 1) { // Logical flow is into shift amount
                    if ((rvn->mask & 1) == 0)
                        return false; // Cannot account for effect of extraneous bits
                    if (bitsize < 8)
                        return false;
                    // Its possible that truncating to the logical value could have an effect, if there were non-zero
                    // bits being truncated.  Non-zero bits here would mean the shift-amount was very large (>255),
                    // indicating the the result was undefined
                ...
                if (newmask == 0)
                    break; // Subvar is cleared, truncate flow
                ...
                rop = createOpDown(CPUI_COPY, 1, op, rvn, 0);
                if (!createLink(rop, newmask, -1, outvn))
                    return false;
                hcount += 1; // Dealt with this descendant
                break;
            case CPUI_INT_RIGHT:
            case CPUI_INT_SRIGHT:
                ...
                if (newmask == 0) {
                    if (op->code() == CPUI_INT_RIGHT)
                        break; // subvar does not pass thru, truncate flow
                    return false;
                }
                if (rvn->mask != (newmask << sa))
                    return false;
            ...
            case CPUI_SUBPIECE:
                ...
                newmask = (rvn->mask >> sa) & calc_mask(outvn->getSize());
                if (newmask == 0)
                    break;                          // subvar is set to zero, truncate flow
            ...
            case CPUI_INT_LESSEQUAL:
                ...
                if (outvn->isConstant()) {
                    ...
                    if ((!aggressive) &&
                        (((rvn->mask | outvn->getNZMask()) != rvn->mask))) // unused bits of otherside must be zero
                        return false;
                }
            ...
            case CPUI_INT_EQUAL:
                ...
                if (bitsize != 1) {
                    if ((!aggressive) && (((rvn->vn->getNZMask() | rvn->mask) != rvn->mask)))
                        return false; // Everything but logical variable must definitely be zero (unless we are
                                      // aggressive)
                    if (outvn->isConstant()) {
                        if ((rvn->mask | outvn->getOffset()) != rvn->mask)
                        ...
                        if ((!aggressive) &&
                            (((rvn->mask | outvn->getNZMask()) != rvn->mask))) // unused bits must be zero
                            return false;
                    }
                    if (!createCompareBridge(op, rvn, slot, outvn))
                        return false;
            ...
            case CPUI_CALLIND:
                ...
                if (!tryCallPull(op, rvn, slot))
                    return false;
                hcount += 1; // Dealt with this descendant
                break;
            case CPUI_RETURN:
                if (!tryReturnPull(op, rvn, slot))
                    return false;
                hcount += 1;
                break;
            case CPUI_BRANCHIND:
                if (!trySwitchPull(op, rvn))
                    return false;
                hcount += 1;
                break;
    ...
    switch (op->code()) {
        ...
        case CPUI_INT_AND:
            ...
            if (sa != -1) {
                ...
                if (!createLink(rop, rvn->mask, 0, op->getIn(0)))
                    return false;
                if (!createLink(rop, rvn->mask, 1, op->getIn(1)))
                    return false;
            }
            return true;
        case CPUI_INT_OR:
        ...
        case CPUI_INT_LEFT:
            ...
            if ((newmask << sa) == rvn->mask) {
                rop = createOp(CPUI_COPY, 1, rvn);
                if (!createLink(rop, newmask, 0, op->getIn(0)))
                    return false;
                return true;
            }
            if ((rvn->mask & 1) == 0)
        ...
        case CPUI_PIECE:
            if ((rvn->mask & calc_mask(op->getIn(1)->getSize())) == rvn->mask) {
                rop = createOp(CPUI_COPY, 1, rvn);
                if (!createLink(rop, rvn->mask, 0, op->getIn(1)))
                    return false;
                return true;
            }
        ...
        case CPUI_FLOAT_NAN:
            ...
            if ((rvn->mask & 1) == 1)
                break; // Not normal variable flow
...

{
    ...
    for (iter = rvn->vn->beginDescend(); iter != enditer; ++iter) {
        ...
        switch (op->code()) {
            ...
            case CPUI_INT_SRIGHT:
                if (!op->getIn(1)->isConstant())
                    return false; // Right now we only deal with constant shifts
...

{
    ...
    switch (op->code()) {
        ...
        case CPUI_INT_ZEXT:
            ...
            break;
        case CPUI_INT_SEXT:
            if (flowsize != op->getIn(0)->getSize())
                return false;
            rop = createOp(CPUI_COPY, 1, rvn);
            if (!createLink(rop, rvn->mask, 0, op->getIn(0)))
                return false;
...
SubvariableFlow::ReplaceVarnode* SubvariableFlow::addConstant(ReplaceOp* rop, uintb mask, uint4 slot,
                                                              Varnode* constvn) {
...

{
    if (rvn->vn->isInput())
        return true;
    // If we trim an addrtied varnode, because of required merges, we increase chance of conflicting forms for one
    // variable
...

{
    ...
    for (piter = patchlist.begin(); piter != patchlist.end(); ++piter) {
        if ((*piter).type != PatchRecord::push_patch)
            break;
...

{
    ...
    if ((op->getIn(1)->getOffset() != mask) && (op->getIn(1)->getOffset() != 0))
        return 0;
    ...
    while (iter != enditer) {
        ...
        switch (op->code()) {
            ...
            case CPUI_INT_LEFT: {
                ...
                opSetInput(rop, newConstant(op->getIn(1)->getSize(), 0, op->getIn(1)->getOffset()),
                           1); // Original shift amount
                break;
            }
            case CPUI_INT_SRIGHT:
            case CPUI_INT_RIGHT: {
                ...
                if (val == laneDescription.getSize(0) * 8) { // Shift of exactly loSize bytes
                    ...
                    opSetInput(rop, newConstant(op->getIn(1)->getSize(), 0, remainShift),
                               1); // Shift any remaining bits
                }
                break;
            }
            default:
...

{
    ...
    switch (op->code()) {
        ...
        case CPUI_INT_LEFT: {
            ...
            if (!cvn->isConstant())
                return false;
            if (cvn->getOffset() != laneDescription.getSize(0) * 8)
                return false;
            Varnode* invn = op->getIn(0);
            if (!invn->isWritten())
                return false;
            ...
            if (zextOp->code() != CPUI_INT_ZEXT)
                return false;
            invn = zextOp->getIn(0);
            if (invn->getSize() != laneDescription.getSize(1))
                return false;
            if (invn->isFree())
                return false;
            TransformOp* loOp = newOpReplace(1, CPUI_COPY, op);
...

{
    ...
    for (int4 i = 0; i < 3; ++i) {
        if (pointer->isAddrTied() || pointer->loneDescend() == (PcodeOp*)0)
            break;
...

{
    ...
    if (isLoadStore && inCategory == 2 && !inConstant && outCategory == 1)
        return false; // Don't split primitive into an array pointer, TODO: We could check if primitive is defined by
                      // PIECE
...
SplitDatatype::SplitDatatype(Funcdata& func) : data(func), resolver(func) {
...

{
    ...
    if (resType->getAlignSize() < size) {
        if (metain == TYPE_INT || metain == TYPE_UINT || metain == TYPE_BOOL || metain == TYPE_FLOAT ||
            metain == TYPE_PTR) {
...

{
    ...
    if (metain != TYPE_PARTIALSTRUCT && metaout != TYPE_PARTIALSTRUCT && metain != TYPE_ARRAY &&
        metaout != TYPE_ARRAY && metain != TYPE_STRUCT && metaout != TYPE_STRUCT)
        return false;
        ...
        switch (op->code()) {
            ...
            case CPUI_FLOAT_FLOAT2FLOAT: {
                if (outvn->getSize() < precision)
                    return false;
                TransformOp* rop =
                    newPreexistingOp(1, (outvn->getSize() == precision) ? CPUI_COPY : CPUI_FLOAT_FLOAT2FLOAT, op);
                opSetInput(rop, rvn, 0);
                terminatorCount += 1;
                break;
            }
...
SubfloatFlow::SubfloatFlow(Funcdata* f, Varnode* root, int4 prec) : TransformManager(f) {
    precision = prec;
...

{
    ...
    if (origPtr->isFree()) {
        if (!origPtr->isConstant())
            return false;
    }
    TransformVar* basePtr = getPreexistingVarnode(origPtr);
    int4 ptrSize = origPtr->getSize();
    // Order lanes by pointer offset.  Least significant to most for little endian, or most significant to least for big
    // endian.
            ...
            case CPUI_SUBPIECE: {
                ...
                if (!description.restriction(numLanes, skipLanes, bytePos, outvn->getSize(), outLanes, outSkip)) {
                    if (allowSubpieceTerminator) {
                        ...
                        if (laneIndex < 0 ||
                            laneIndex >= description.getNumLanes()) // Does piece start on lane boundary?
                            return false;
...

{
    ...
    if (!traceBackward(rvn, numLanes, skipLanes))
        return false;
