// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/block.hh.
...
public:
    ...
    enum block_flags {
        ...
        f_whiledo_overflow =
            0x8000,            ///< Set if the conditional block of a whiledo is too big to print as while(cond) { ...
...
protected:
    ...
    void clearAllFlags(void) {
        flags = 0;
    } ///< Clear all properties of \b this block
public:
    ...
    virtual bool preferComplement(Funcdata& data,
                                  bool allowOpRemoval); ///< Rearrange \b this hierarchy to simplify boolean expressions
    ...
    void setImmedCopyEdge(int4 i) {
        setOutEdgeFlag(i, f_immed_copy);
    } ///< Mark that an immediate COPY has propagated across the edge
    bool hasImmedCopyEdge(int4 i) const {
        return ((outofthis[i].label & f_immed_copy) != 0);
    } ///< Has an immediate COPY propagated across the edge
    ...
    const FlowBlock* getOut(int4 i) const {
        return (const FlowBlock*)outofthis[i].point;
    ...
    const FlowBlock* getIn(int4 i) const {
        return (const FlowBlock*)intothis[i].point;
    } ///< Get the i-th input FlowBlock
    ...
    int4 getBasicCount(void) const {
        return leafCount;
    } ///< Get the number of basic blocks under the structure at \b this point
    int4 getStructureDepth(void) const {
        return structureDepth;
    }                                        ///< Get the nesting depth of the structure rooted at \b this point
    ...
    static FlowBlock*
    findCommonBlock(const vector<FlowBlock*>& blockSet); ///< Find common dominator of multiple FlowBlocks
...
public:
    ...
    void switchEdge(FlowBlock* in, FlowBlock* outbefore,
                    FlowBlock* outafter);                            ///< Move an edge from one out FlowBlock to another
    ...
    void removeFromFlowSplit(FlowBlock* bl,
                             bool flipflow); ///< Remove FlowBlock splitting flow between input and output edges
    ...
    void orderBlocks(void) { ///< Sort blocks using the final ordering
        if (list.size() != 1)
            sort(list.begin(), list.end(), compareFinalOrder);
    }
    ...
    void collectReachable(vector<FlowBlock*>& res, FlowBlock* bl,
                          bool un) const; ///< Collect reachable/unreachable FlowBlocks from a given start FlowBlock
    void structureLoops(vector<FlowBlock*>& rootlist); ///< Label loop edges
    bool hasFinalTransform(void) const {
        return ((getFlags() & f_final_transform) != 0);
    } ///< Return \b true if finalTransform() has been run on \b this
#ifdef BLOCKCONSISTENT_DEBUG
...
public:
    ...
    virtual void finalTransform(Funcdata& data, bool allowOpMoves);
    bool
    unblockedMulti(int4 outslot) const; ///< Check if \b this block can be removed without introducing inconsistencies
    ...
    void setDelayedDonothing(void) {
        setFlag(f_delayed_donothing);
    } ///< Mark as \e do \e nothing block with delayed removal
    void clearDelayedDonothing(void) {
        clearFlag(f_delayed_donothing);
    } ///< Clear mark for delayed removal
    bool isDelayedDonothing(void) const {
        return ((getFlags() & f_delayed_donothing) != 0);
    } ///< Is block marked for delayed removal
    static bool liftVerifyUnroll(vector<Varnode*>& varArray,
                                 int4 slot); ///< Verify given Varnodes are defined with same PcodeOp
};
...
public:
    ...
    virtual void printRawImpliedGoto(ostream& s, const FlowBlock* nextBlock) const {
        copy->printRawImpliedGoto(s, nextBlock);
    }
    virtual void emit(PrintLanguage* lng) const {
    ...
    virtual bool negateCondition(bool toporbottom) {
        ...
        FlowBlock::negateCondition(toporbottom);
        return res;
    }
    virtual FlowBlock* getSplitPoint(void) {
        return copy->getSplitPoint();
    }
    virtual bool isComplex(void) const {
        return copy->isComplex();
    }
    virtual void finalTransform(Funcdata& data, bool allowOpMoves) {
        return copy->finalTransform(data, allowOpMoves);
    }
...
class BlockWhileDo : public BlockGraph {
    ...
    void findLoopVariable(PcodeOp* cbranch, BlockBasic* head, BlockBasic* tail,
                          PcodeOp* lastOp);                      ///< Find a \e loop \e variable
