// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/funcdata.hh.
...
class Funcdata {
    ...
    map<ResolveEdge, ResolvedUnion>
        unionMap; ///< A map from data-flow edges to the resolved field of TypeUnion being accessed
    ...
    static void
    markIndirectAliasUpdate(Varnode* vn); ///< Inform defining INDIRECT of update in alias status for given Varnode
    ...
    Varnode* createReplaceVarnode(Varnode* origvn,
                                  bool makeUnique); ///< Create a replacement for a Varnode that may be merged
    ...
    static void encodeVarnode(Encoder& encoder, VarnodeLocSet::const_iterator iter,
                              VarnodeLocSet::const_iterator enditer);
    static bool checkIndirectUse(Varnode* vn);

public:
    Funcdata(const string& nm, const string& disp, Scope* conf, const Address& addr, FunctionSymbol* sym,
             int4 sz = 0); ///< Constructor
    ...
    void setJumptableRecovery(bool val) {
        if (val)
            flags &= ~jumptablerecovery_dont;
        else
            flags |= jumptablerecovery_dont;
    }

    bool isJumptableRecoveryOn(void) const {
        return ((flags & jumptablerecovery_on) != 0);
    ...
    void setTypeRecoveryExceeded(void) {
        flags |= typerecovery_exceeded;
    ...
    void setNormalization(bool val) {
        flags = val ? (flags | normalization_on) : (flags & ~normalization_on);
    }
    ...
    void encode(Encoder& encoder, uint8 id, bool saveTree,
                bool saveOverrides) const;        ///< Encode a description of \b this function to stream
    ...
    bool hasRestartPending(void) const {
        return ((flags & restart_pending) != 0);
    }
    ...
    bool hasUnimplemented(void) const {
        return ((flags & unimplemented_present) != 0);
    }

    bool hasBadData(void) const {
        return ((flags & baddata_present) != 0);
    ...
    void spacebaseConstant(PcodeOp* op, int4 slot, MapEntry* entry, const Address& rampoint, uintb origval,
                           int4 origsize);

    int4 getHeritagePass(void) const {
        return heritage.getPass();
    ...
    int4 numHeritagePasses(AddrSpace* spc) {
        return heritage.numHeritagePasses(spc);
    }
    ...
    bool deadRemovalAllowedSeen(AddrSpace* spc) {
        return heritage.deadRemovalAllowedSeen(spc);
    }
    ...
    Address findDisjointCover(Varnode* vn,
                              int4& sz); ///< Find range covering given Varnode and any intersecting Varnodes
    ...
    bool hasInputIntersection(int4 s, const Address& loc) const {
        return vbank.hasInputIntersection(s, loc);
    }
    ...
    VarnodeLocSet::const_iterator beginLoc(int4 s, const Address& addr, uint4 fl) const {
        return vbank.beginLoc(s, addr, fl);
    }
    ...
    VarnodeLocSet::const_iterator endLoc(int4 s, const Address& addr, uint4 fl) const {
        return vbank.endLoc(s, addr, fl);
    }

    /// \brief Start of Varnodes matching storage and definition address
    VarnodeLocSet::const_iterator beginLoc(int4 s, const Address& addr, const Address& pc,
                                           uintm uniq = ~((uintm)0)) const {
        return vbank.beginLoc(s, addr, pc, uniq);
    }

    /// \brief End of Varnodes matching storage and definition address
    VarnodeLocSet::const_iterator endLoc(int4 s, const Address& addr, const Address& pc,
                                         uintm uniq = ~((uintm)0)) const {
    ...
    bool checkCallDoubleUse(const PcodeOp* opmatch, const PcodeOp* op, const Varnode* vn, uint4 fl,
                            const ParamTrial& trial) const;
    bool onlyOpUse(const Varnode* invn, const PcodeOp* opmatch, const ParamTrial& trial, uint4 mainFlags) const;
    bool ancestorOpUse(int4 maxlevel, const Varnode* invn, const PcodeOp* op, ParamTrial& trial, int4 offset,
                       uint4 mainFlags) const;
    ...
    void findLinkedVarnodes(SymbolEntry* entry,
                            vector<Varnode*>& res) const; ///< Find Varnodes that map to the given SymbolEntry
    ...
    void markIndirectCreation(PcodeOp* indop,
                              bool possibleOutput); ///< Convert CPUI_INDIRECT into an \e indirect \e creation
    void markReturnCopy(PcodeOp* op) {
    ...
    void
    opDestroyRecursive(PcodeOp* op,
                       vector<PcodeOp*>& scratch); ///< Remove a PcodeOp and recursively remove ops producing its inputs
    ...
    void opSetAllInput(PcodeOp* op,
                       const vector<Varnode*>& vvec); ///< Set all input Varnodes for the given PcodeOp simultaneously
    void opRemoveInput(PcodeOp* op, int4 slot);       ///< Remove a specific input slot for the given PcodeOp
    void opInsertInput(PcodeOp* op, Varnode* vn,
                       int4 slot); ///< Insert a new Varnode into the operand list for the given PcodeOp
    ...
    PcodeOpTree::const_iterator endOp(const Address& addr) const {
        return obank.end(addr);
    }

    PcodeOp* findPrimaryBranch(const Address& addr, bool findBranch, bool findCall, bool findCallother,
                               bool findReturn);

    bool moveRespectingCover(PcodeOp* op,
                             PcodeOp* lastOp); ///< Move given op past \e lastOp respecting covers if possible
    ...
    bool setAddressBasedUnionField(const Datatype* unresType, const Address& addr, int4 slot,
    ...
    JumpTable::RecoveryMode
    earlyJumpTableFail(PcodeOp* op); ///< Try to determine, early, if jump-table analysis will fail
    ...
    void enableJTCallback(void (*jtcb)(Funcdata& orig, Funcdata& fd)) {
        jtcallback = jtcb;
    } ///< Enable a debug callback
    void disableJTCallback(void) {
        jtcallback = (void (*)(Funcdata& orig, Funcdata& fd))0;
    } ///< Disable debug callback
    void debugActivate(void) {
        if (opactdbg_on)
            opactdbg_active = true;
    } ///< Turn on recording
    void debugDeactivate(void) {
    ...
    void
    debugModPrint(const string& actionname); ///< Print before and after strings for PcodeOps modified by given action
    bool debugBreak(void) const {
        return opactdbg_on && opactdbg_breakon;
    } ///< Has a breakpoint been hit
    int4 debugSize(void) const {
    ...
    void debugSetRange(const Address& pclow, const Address& pchigh, uintm uqlow = ~((uintm)0),
                       uintm uqhigh = ~((uintm)0)); ///< Add a new memory range to the debug trace
    void debugHandleBreak(void) {
        opactdbg_breakon = false;
    } ///< Mark a breakpoint as handled
    void debugSetBreak(int4 count) {
...
class CloneBlockOps {
    ...
    void buildVarnodeOutput(PcodeOp* origOp,
                            PcodeOp* cloneOp); ///< Clone the output Varnode of the given op onto its clone
    void patchInputs(int4 inedge);             ///< Set the input Varnodes of all cloned ops
public:
    CloneBlockOps(Funcdata& fd) : data(fd) {} ///< Constructor
    void cloneBlock(BlockBasic* b, BlockBasic* bprime,
                    int4 inedge); ///< Clone all p-code ops from a block into its copy
...
class AncestorRealistic {
    ...
    public:
        enum {
            seen_solid0 =
                1, ///< Indicates a \e solid movement into the Varnode occurred on at least one path to MULTIEQUAL
        ...
        ///
        /// Data ultimately in SUBPIECE output is copied from a non-zero offset within the input Varnode. Note this
        /// offset
