// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/blockaction.hh.
...
public:
    ActionStructureTransform(const string& g, bool allowMoves) : Action(0, "structuretransform", g) {
        allowOpMoves = allowMoves;
    } ///< Constructor
...
public:
    ActionPreferComplement(const string& g, bool allowMods) : Action(0, "prefercomplement", g) {
        allowOpMods = allowMods;
    } ///< Constructor
    virtual Action* clone(const ActionGroupList& grouplist) const {
        if (!grouplist.contains(getGroup()))
            return (Action*)0;
