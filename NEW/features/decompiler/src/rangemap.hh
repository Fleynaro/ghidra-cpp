// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/rangemap.hh.
...
template <typename _recordtype> class rangemap {
public:
...
private:
    ...
    class AddrRange {
        ...
        AddrRange(linetype l) : subsort(false) {
            last = l;
        } ///< (Partial) constructor
        AddrRange(linetype l, const subsorttype& s) : subsort(s) {
            last = l;
    ...
    public:
        ...
        typename std::list<_recordtype>::iterator getValue(void) const {
            return value;
...
public:
    ...
    typename std::list<_recordtype>::const_iterator begin_list(void) const {
        return record.begin();
    } ///< Beginning of records
    typename std::list<_recordtype>::const_iterator end_list(void) const {
        return record.end();
    } ///< End of records
    typename std::list<_recordtype>::iterator begin_list(void) {
        return record.begin();
    } ///< Beginning of records
    typename std::list<_recordtype>::iterator end_list(void) {
        return record.end();
    } ///< End of records
    ...
    std::pair<const_iterator, const_iterator> find(linetype a, const subsorttype& subsort1,
                                                   const subsorttype& subsort2) const;
...
template <typename _recordtype>
typename std::list<_recordtype>::iterator rangemap<_recordtype>::insert(const inittype& data, linetype a, linetype b)
    ...
    while ((low != tree.end()) && ((*low).first <= b)) {
        if (f <= (*low).last) { // Do we overlap at all
            ...
            if ((*low).last <= b) { // Insert as much of interval as we can
                ...
                tree.insert(low, addrrange);
                if ((*low).last == b)
                    break; // Did we manage to insert it all
                f = (*low).last + 1;
...
template <typename _recordtype>
typename rangemap<_recordtype>::const_iterator rangemap<_recordtype>::find_begin(linetype point) const
...
template <typename _recordtype>
typename rangemap<_recordtype>::const_iterator rangemap<_recordtype>::find_end(linetype point) const
...
template <typename _recordtype>
typename rangemap<_recordtype>::const_iterator rangemap<_recordtype>::find_overlap(linetype point, linetype end) const
...
template <typename _recordtype>
typename rangemap<_recordtype>::const_iterator rangemap<_recordtype>::find_first_after(linetype point) const
...
template <typename _recordtype>
typename rangemap<_recordtype>::const_iterator rangemap<_recordtype>::find_last_before(linetype point) const
