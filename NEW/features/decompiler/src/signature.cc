// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/signature.cc.
...
/// \param sigMap is the map from Varnode to its SignatureEntry overlay
void SignatureEntry::noisePostOrder(const vector<SignatureEntry*>& rootlist, vector<SignatureEntry*>& postOrder,
...

{
    ...
    for (iter = blockmap.begin(); iter != blockmap.end(); ++iter) {
        ...
        while (oiter != enditer) {
            ...
            if ((stopind == 0) && (outvn == (Varnode*)0 || !outvn->hasNoDescend()))
                continue;
            if (outvn != (Varnode*)0) {
...

{
    ...
    uint4 mask =
        SIG_COLLAPSE_SIZE | SIG_DONOTUSE_CONST | SIG_DONOTUSE_INPUT | SIG_DONOTUSE_PERSIST | SIG_COLLAPSE_INDNOISE;
    mask = (mask << 2) | 1;      // Add the check bit
    return ((val & ~mask) == 0); // Do not allow any other bit to be set
}

GraphSigManager::GraphSigManager(void)
    : SigManager()
