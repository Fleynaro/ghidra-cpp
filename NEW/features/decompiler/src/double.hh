// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/double.hh.
...
public:
    ...
    bool
    inHandLoNoHi(Varnode* l); ///< Try to initialize given just the least significant piece (other piece may be zero)
    ...
    bool hasBothPieces(void) const {
        return ((hi != (Varnode*)0) && (lo != (Varnode*)0));
    ...
    bool isWholeFeasible(
        PcodeOp* existop); ///< Does a whole Varnode already exist or can it be created before the given PcodeOp
    bool isWholePhiFeasible(
        FlowBlock* bl); ///< Does a whole Varnode already exist or can it be created before the given basic block
    ...
    static bool testContiguousPointers(PcodeOp* most, PcodeOp* least, PcodeOp*& first, PcodeOp*& second,
                                       AddrSpace*& spc);
    ...
    static void createShiftOp(Funcdata& data, SplitVarnode& out, SplitVarnode& in, Varnode* sa, PcodeOp* existop,
                              OpCode opc);
    ...
    static void replaceCopyForce(Funcdata& data, const Address& addr, SplitVarnode& in, PcodeOp* copylo,
                                 PcodeOp* copyhi);
