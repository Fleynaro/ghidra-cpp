// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/fspec.hh.
...
public:
    enum {
        ...
        smallsize_inttype =
            0x20, ///< Assume values that are below the max \b size are sign OR zero extended based on integer type
...
private:
    ...
    bool isLeftJustified(void) const {
        return (((flags & force_left_justify) != 0) || (!spaceid->isBigEndian()));
    }
...
public:
    ...
    bool isFirstInClass(void) const {
        return ((flags & first_storage) != 0);
    } ///< Return \b true if \b this is the first entry in the storage class
    bool
    subsumesDefinition(const ParamEntry& op2) const; ///< Does \b this subsume the definition of the given ParamEntry
    ...
    bool isParamCheckHigh(void) const {
        return ((flags & extracheck_high) != 0);
    } ///< Return \b true if there is a high overlap
    bool isParamCheckLow(void) const {
        return ((flags & extracheck_low) != 0);
    } ///< Return \b true if there is a low overlap
    static void orderWithinGroup(const ParamEntry& entry1,
                                 const ParamEntry& entry2); ///< Enforce ParamEntry group ordering rules
};
...
class ParamEntryRange {
    ...
    public:
        InitData(int4 pos, ParamEntry* e) {
            position = pos;
            entry = e;
        } ///< Constructor
    };
    ...
    public:
        ...
        SubsortPosition(int4 pos) {
            position = pos;
        } ///< Construct given position
        SubsortPosition(bool val) {
            position = val ? 1000000 : 0;
        } ///< Constructor minimal/maximal subsort
        bool operator<(const SubsortPosition& op2) {
...
public:
    ...
    bool isIndCreateFormed(void) const {
        return ((flags & indcreate_formed) != 0);
    ...
    bool hasAncestorRealistic(void) const {
        return ((flags & ancestor_realistic) != 0);
    ...
    static bool fixedPositionCompare(const ParamTrial& a,
                                     const ParamTrial& b); ///< Sort by fixed position; stable for fixedPosition = -1
};
...
public:
    ...
    bool isJoinReverse(void) const {
        return joinReverse;
    } ///< Do Varnodes need to be joined in reverse order
    void setJoinReverse(void) {
        joinReverse = true;
    } ///< Mark that varnodes need to be joined in reverse order
...
public:
    ...
    Address getAddress(void) const {
        return Address(range.space, range.offset);
...
public:
    ...
    virtual void assignMap(const PrototypePieces& proto, TypeFactory& typefactory,
...
protected:
    ...
    const ParamEntry* findEntry(const Address& loc, int4 size,
                                bool just) const; ///< Given storage location find matching ParamEntry
    const ParamEntry* selectUnreferenceEntry(int4 grp,
                                             type_class prefType) const; ///< Select entry to fill an unreferenced param
    ...
    void parsePentry(Decoder& decoder, vector<EffectRecord>& effectlist, int4 groupid, bool normalstack,
                     bool splitFloat, bool grouped);
    void parseGroup(Decoder& decoder, vector<EffectRecord>& effectlist, int4 groupid, bool normalstack,
                    bool splitFloat);
...
public:
    ...
    const AddrSpaceManager* getManager(void) const {
        return glb;
    } ///< Get the owner
    bool isBigEndian(void) const {
        return entry.front().getSpace()->isBigEndian();
    } ///< Return \b true if resources are big endian
    void extractTiles(vector<const ParamEntry*>& tiles,
                      type_class type) const;    ///< Get registers of given storage class
    ...
    uint4 assignAddress(Datatype* dt, const PrototypePieces& proto, int4 pos, TypeFactory& tlst, vector<int4>& status,
                        ParameterPieces& res) const;
...
public:
    ...
    ParamListStandardOut(const ParamListStandardOut& op2) : ParamListStandard(op2) {
        useFillinFallback = op2.useFillinFallback;
    } ///< Copy constructor
...
public:
    ...
    uint4 hasEffect(const Address& addr,
                    int4 size) const; ///< Determine side-effect of \b this on the given memory range
    ...
    bool
    isCompatible(const ProtoModel* op2) const; ///< Return \b true if other given model can be substituted for \b this
    ...
    } ///< Get an iterator to the first \e internalstorage
    vector<VarnodeData>::const_iterator internalEnd(void) const {
        return internalstorage.end();
    } ///< Get an iterator to the last \e internalstorage
    ...
    AddrSpace* getSpacebase(void) const {
        return input->getSpacebase();
    } ///< Get the stack space associated with \b this model
    bool isStackGrowsNegative(void) const {
        return stackgrowsnegative;
    } ///< Return \b true if the stack \e grows toward smaller addresses
    bool hasThisPointer(void) const {
        return hasThis;
    } ///< Is \b this a model for (non-static) class methods
    bool isConstructor(void) const {
        return isConstruct;
    } ///< Is \b this model for class constructors
    ...
    bool isAutoKilledByCall(void) const {
        return output->isAutoKilledByCall();
    }
...
public:
    ...
    virtual bool isTypeLocked(void) const {
        return ((flags & ParameterPieces::typelock) != 0);
    }
    virtual bool isNameLocked(void) const {
        return ((flags & ParameterPieces::namelock) != 0);
    }
    virtual bool isSizeTypeLocked(void) const {
        return ((flags & ParameterPieces::sizelock) != 0);
    }
    virtual bool isThisPointer(void) const {
        return ((flags & ParameterPieces::isthis) != 0);
    }
    virtual bool isIndirectStorage(void) const {
        return ((flags & ParameterPieces::indirectstorage) != 0);
    }
    virtual bool isHiddenReturn(void) const {
        return ((flags & ParameterPieces::hiddenretparm) != 0);
    }
    ...
    virtual Symbol* getSymbol(void) const {
        throw LowlevelError("Parameter is not a real symbol");
    }
};
...
protected:
    ...
    bool isParamshiftApplied(void) const {
        return ((flags & paramshift_applied) != 0);
...
public:
    ...
    bool isOutputLocked(void) const {
        return store->getOutput()->isTypeLocked();
    ...
    void setCustomStorage(bool val) {
        flags = val ? (flags | custom_storage) : (flags & ~((uint4)custom_storage));
    }
    /// Clear ABI placement errors after an external provider has supplied storage.
    void clearProviderErrors(void) {
    ...
    vector<VarnodeData>::const_iterator internalBegin(void) const {
        return model->internalBegin();
    } ///< Get iterator to front of \e internalstorage list
    vector<VarnodeData>::const_iterator internalEnd(void) const {
        return model->internalEnd();
    } ///< Get iterator to end of \e internalstorage list
    ...
    int4 getMaxOutputDelay(void) const {
        return model->getMaxOutputDelay();
    }
...
public:
    ...
    } ///< Return \b true if return value recovery analysis is active
    ...
    void setStackOutputLock(bool val) {
        isstackoutputlock = val;
    } ///< Toggle whether output is locked and on the stack
    bool isStackOutputLock(void) const {
        return isstackoutputlock;
    ...
    static FuncCallSpecs* getFspecFromConst(const Address& addr) {
        return (FuncCallSpecs*)(uintp)addr.getOffset();
    }
