// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/type.cc.
...
sub_metatype Datatype::base2sub[18] = {
    SUB_PARTIALUNION, SUB_PARTIALSTRUCT, SUB_UINT_PARTIALENUM, SUB_UNION,   SUB_STRUCT,    SUB_INT_ENUM,
    SUB_UINT_ENUM,    SUB_ARRAY,         SUB_PTRREL,           SUB_PTR,     SUB_FLOAT,     SUB_CODE,
...
{
    if (submeta != op.submeta)
        return (submeta < op.submeta) ? -1 : 1;
    if (size != op.size)
        return (op.size - size);
    return 0;
}
...

{
    ...
    if (arrayHint != 1 &&
        before.elSize != after.elSize) { // element sizes are different, try to distinguish by arrayHint
...

{
    ...
    for (;;) {
        ...
        if (next == (Datatype*)0)
            break; // No smaller component
        if (curOff + sz > next->getSize())
            break; // Next component down does not contain
        res = next;
        off = curOff;
    }
    *newoff = off;
...
TypeUnicode::TypeUnicode(const string& nm, int4 sz, type_metatype m) : TypeBase(sz, m, nm) {
...

{
    ...
    if (wordsize != tp->wordsize)
        return (wordsize < tp->wordsize) ? -1 : 1;
    if (spaceid != tp->spaceid) {
        if (spaceid == (AddrSpace*)0)
            return 1; // Pointers with address space come earlier
        if (tp->spaceid == (AddrSpace*)0)
            return -1;
...
{
    if (submeta != op.getSubMeta())
        return (submeta < op.getSubMeta()) ? -1 : 1;
    TypePointer* tp = (TypePointer*)&op; // Both must be pointers
    if (ptrto != tp->ptrto)
        return (ptrto < tp->ptrto) ? -1 : 1; // Compare absolute pointers
    if (wordsize != tp->wordsize)
        return (wordsize < tp->wordsize) ? -1 : 1;
    if (spaceid != tp->spaceid) {
        if (spaceid == (AddrSpace*)0)
...
TypePointer* TypePointer::downChain(int8& off, TypePointer*& par, int8& parOff, bool allowArrayWrap,
                                    TypeFactory& typegrp)
...

{
    if (submeta != op.getSubMeta())
        return (submeta < op.getSubMeta()) ? -1 : 1;
    TypeArray* ta = (TypeArray*)&op; // Both must be arrays
    if (arrayof != ta->arrayof)
        return (arrayof < ta->arrayof) ? -1 : 1; // Compare absolute pointers
    return (op.getSize() - size);
}
...
{
    if (off > 0)
        return false; // Skip if we are in the middle of array
    res.offset = off;
...
{
    if (off < 0)
        return false; // Skip if we are before array
    res.offset = off;
    res.elSize = arrayof->getAlignSize();
    if (off < size)
...
TypeEnum::TypeEnum(const TypeEnum& op)
    : TypeBase(op)
...
TypeStruct::TypeStruct(const TypeStruct& op) : Datatype(op) {
...
{
    vector<TypeBitField>::const_iterator iter =
    ...
    vector<TypeField>::const_iterator fiter =
        upper_bound(field.begin(), field.end(), offset, TypeField::compareMaxByte);
    for (; fiter != field.end(); ++fiter) {
        const TypeField& curField(*fiter);
        if (curField.offset >= offset + sz)
            break;
        if (curField.type->getMetatype() != TYPE_STRUCT)
            continue;
        if (!curField.type->hasBitfields())
            continue;
        ((TypeStruct*)curField.type)
            ->collectBitFields(baseOffset + curField.offset, res, offset - curField.offset,
                               sz); // Recurse into nested structure
    }
}
...

{
    ...
    for (; fiter != field.end(); ++fiter) {
        ...
        if (!curField.type->hasBitfields())
            continue;
        if (((const TypeStruct*)curField.type)
                ->hasBitFieldsInRange(offset - curField.offset, sz)) // Recurse into nested structure
            return true;
    }
    return false;
}
...
int4 TypeStruct::compare(const Datatype& op, int4 level) const {
    ...
    if (field.size() != ts->field.size())
        return (ts->field.size() - field.size());
    iter1 = field.begin();
    ...
    if (bitfield.size() != ts->bitfield.size())
        return (ts->bitfield.size() - bitfield.size());
...

{
    ...
    if (curBitField.type == (Datatype*)0)
        throw LowlevelError("Bad bitfield data-type for bitfield \"" + bitfield.back().name +
    ...
    if (meta != TYPE_INT && meta != TYPE_UINT && meta != TYPE_BOOL && meta != TYPE_ENUM_INT && meta != TYPE_ENUM_UINT)
        throw LowlevelError("Non integer data-type for bitfield \"" + bitfield.back().name +
                            "\" in structure: " + getName());
...
void TypeStruct::assignFieldOffsets(vector<TypeField>& list, vector<TypeBitField>& bitlist, int4& newSize,
...
TypeUnion::TypeUnion(const TypeUnion& op) : Datatype(op) {
...

{
    ...
    if (field.size() != tu->field.size())
        return (tu->field.size() - field.size());
    iter1 = field.begin();
...

{
    if (!ct->needsResolution()) {
        ...
        for (int4 i = 0; i < field.size(); ++i) {
            ...
            if (fieldType->getSize() != ct->getSize())
                continue;
            if (fieldType->needsResolution())
                continue;
            if (ct->findCompatibleResolve(fieldType) >= 0)
                return i;
        }
...
TypePartialEnum::TypePartialEnum(const TypePartialEnum& op) : TypeEnum(op) {
    stripped = op.stripped;
    parent = op.parent;
    offset = op.offset;
}

TypePartialEnum::TypePartialEnum(TypeEnum* par, int4 off, int4 sz, Datatype* strip) : TypeEnum(sz, TYPE_PARTIALENUM) {
...

{
    ...
    if (parent != tp->parent)
        return (parent < tp->parent) ? -1 : 1; // Compare absolute pointers
    if (offset != tp->offset)
        return (offset < tp->offset) ? -1 : 1;
    return (op.getSize() - size);
}
...
TypePartialStruct::TypePartialStruct(const TypePartialStruct& op) : Datatype(op) {
    stripped = op.stripped;
...

{
    ...
    if (container != tp->container)
        return (container < tp->container) ? -1 : 1; // Compare absolute pointers
    if (offset != tp->offset)
        return (offset < tp->offset) ? -1 : 1;
    return (op.getSize() - size);
}

TypePartialUnion::TypePartialUnion(const TypePartialUnion& op) : Datatype(op) {
    stripped = op.stripped;
    container = op.container;
    offset = op.offset;
}
...
TypePointer* TypePointerRel::downChain(int8& off, TypePointer*& par, int8& parOff, bool allowArrayWrap,
    ...
    if (relOff == 0 &&
        offset !=
            0) // Recovering the start of the parent is still downchaining, even though the parent may be the container
...
TypeCode::TypeCode(const TypeCode& op)
    : Datatype(op)
...

{
    ...
    if (spaceid != tsb->spaceid)
        return (spaceid < tsb->spaceid) ? -1 : 1;
    if (scopeId != tsb->scopeId)
        return (scopeId < tsb->scopeId) ? -1 : 1;
    return 0;
}
...
void TypeFactory::setCoreType(const string& name, int4 size, type_metatype meta, bool chartp) {
    Datatype* ct;
    if (chartp) {
        if (size == 1)
            ct = getTypeChar(name);
...

{
    ...
    for (iter = tree.begin(); iter != tree.end(); ++iter) {
        ...
        switch (ct->getMetatype()) {
            ...
            case TYPE_UINT:
                if (ct->isEnumType())
                    break; // Conceivably an enumeration
                if (ct->isCharPrint()) {
                    if (ct->getSize() < 5)
...
{ // Get type of given name
    ...
    if (id != 0) { // Search for an exact type
        ...
        if (iter == nametree.end())
            return (Datatype*)0; // Didn't find it
        ...
        if (iter == nametree.end())
            return (Datatype*)0; // Didn't find it
        if ((*iter)->getName() != n)
            return (Datatype*)0; // Found at least one datatype with this name
    }
    return *iter;
}
...
void TypeFactory::setFields(const vector<TypeField>& fd, const vector<TypeBitField>& bit, TypeStruct* ot, int4 newSize,
    ...
    while (iter != incompleteTypedef.end()) {
        ...
        if (!defedType->isIncomplete()) {
            if (dt->getMetatype() == TYPE_STRUCT) {
                ...
                setFields(defedStruct->field, defedStruct->bitfield, prevStruct, defedStruct->size,
                          defedStruct->alignment, defedStruct->flags);
...
/// \return the new/matching pointer
TypePointerRel* TypeFactory::getTypePointerRel(int4 sz, Datatype* parent, Datatype* ptrTo, int4 ws, int4 off,
                                               const string& nm)
...

{
    ...
    if (!ct->isIncomplete()) { // Structure of this name was already present
        ...
        setFields(ts.field, ts.bitfield, (TypeStruct*)ct, ts.size, ts.alignment,
                  ts.flags); // Define structure now by copying fields
    }
    if (!warning.empty())
