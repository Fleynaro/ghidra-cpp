// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/varmap.hh.
...
public:
    TypeRecommend(const Address& ad, Datatype* dt) : addr(ad), dataType(dt) {} ///< Constructor
    const Address& getAddress(void) const {
        return addr;
    } ///< Get the storage address
...
public:
    ...
    bool isTypeLock(void) const {
        return ((flags & typelock) != 0);
    } ///< Is the data-type for \b this range locked
    bool isBackfill(void) const {
        return ((flags & backfill) != 0);
    }                                                 ///< Does \b this hint want to back fill its data-type
    ...
    bool merge(RangeHint* b, AddrSpace* space,
               TypeFactory* typeFactory);     ///< Try to form the union of \b this with another RangeHint
...
public:
    ...
    bool
    hasLocalAlias(Varnode* vn) const; ///< Return \b true if it looks like the given Varnode is aliased by a pointer
...
class MapState {
    ...
    void addGuard(const LoadGuard& guard, OpCode opc,
                  TypeFactory* typeFactory);                    ///< Add LoadGuard record as a hint to the collection
    ...
    void addFixedType(uintb start, Datatype* ct, uint4 flags,
                      TypeFactory* types); ///< Add a fixed reference to a specific data-type
...
public:
    ...
    void turnOnDebug(Architecture* g) const {
        debugon = true;
        glb = g;
    }
    ...
    bool hasParamRangeHit(void) const {
        return paramRangeHit;
    } ///< Return \b true if reference into parameter stack region has been seen
    void sortAlias(void) {
        checker.sortAlias();
...
public:
    ...
    bool hasOverlapProbems(void) const {
    ...
    bool isUnaffectedStorage(Varnode* vn) const {
        return (vn->getSpace() == space);
    }

    bool isUnmappedUnaliased(Varnode* vn) const; ///< Check if a given unmapped Varnode should be treated as unaliased.

    bool hasOpenParamRefs(void) const {
        return openParamRefs;
    } ///< Return \b true if we have seen refs into parameter region

    void markNotMapped(AddrSpace* spc, uintb first, int4 sz,
                       bool param); ///< Mark a specific address range is not mapped
