// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/op.hh.
...
public:
    ...
    uint4 getEvalType(void) const {
        return (flags & (PcodeOp::unary | PcodeOp::binary | PcodeOp::special | PcodeOp::ternary));
    }
    /// \brief Get type which indicates unusual halt in control-flow
    uint4 getHaltType(void) const {
        return (flags & (PcodeOp::halt | PcodeOp::badinstruction | PcodeOp::unimplemented | PcodeOp::noreturn |
                         PcodeOp::missing));
    }
    bool isDead(void) const {
        return ((flags & PcodeOp::dead) != 0);
    } ///< Return \b true if this op is dead
    bool isAssignment(void) const {
        return (output != (Varnode*)0);
    } ///< Return \b true is this op has an output
    bool isCall(void) const {
        return ((flags & PcodeOp::call) != 0);
    ...
    bool isIndirectSource(void) const {
        return ((flags & PcodeOp::indirect_source) != 0);
    ...
    bool isPtrFlow(void) const {
        return ((flags & PcodeOp::ptrflow) != 0);
    ...
    bool isStoreUnmapped(void) const {
        return ((addlflags & store_unmapped) != 0);
    } ///< Is STORE location supposed to be unmapped
    void setStoreUnmapped(void) const {
        addlflags |= store_unmapped;
    ...
    bool
    hasCopyImmed(int4 slot) const; ///< Return \b true if a COPY propagation from an immediate input block has happened
    bool hasAliasUpdate(void) const {
        return ((addlflags & store_aliasupdate) != 0);
    } ///< Has alias information for \b this been updated
    void setAliasUpdate(void) {
        addlflags |= store_aliasupdate;
    } ///< Mark that there is new alias information for \b this op
    /// \brief Return \b true if this LOADs or STOREs from a dynamic \e spacebase pointer
    bool usesSpacebasePtr(void) const {
        return ((flags & PcodeOp::spacebase_ptr) != 0);
    }
    ...
    bool
    isMoveable(const PcodeOp* point) const; ///< Can \b this be moved to after \e point, without disturbing data-flow
    ...
    bool isCommutative(void) const {
        return ((flags & PcodeOp::commutative) != 0);
    ...
    static PcodeOp* getOpFromConst(const Address& addr) {
        return (PcodeOp*)(uintp)addr.getOffset();
    }

    Datatype* outputTypeLocal(void) const {
        return opcode->getOutputLocal(this);
    } ///< Calculate the local output type
    Datatype* inputTypeLocal(int4 slot) const {
        return opcode->getInputLocal(this, slot);
...
public:
    ...
    void markIncidentalCopy(PcodeOp* firstop,
                            PcodeOp* lastop); ///< Mark any COPY ops in the given range as \e incidental
