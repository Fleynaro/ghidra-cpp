// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/constseq.cc.
...
ArraySequence::ArraySequence(Funcdata& fdata, Datatype* ct, PcodeOp* root) : data(fdata) {
    rootOp = root;
    charType = ct;
...

{
    ...
    while (startOp != endOp) {
        if (startOp->getEvalType() == PcodeOp::special) {
            ...
            if (opc != CPUI_INDIRECT && opc != CPUI_CALLOTHER && opc != CPUI_SEGMENTOP && opc != CPUI_CPOOLREF &&
                opc != CPUI_NEW)
                return false;
        }
        startOp = startOp->nextOp();
    ...
    do {
        ...
        if (parentType->needsResolution()) {
            const TypeField* field =
                parentType->resolveTruncation(off, root, -1, off); // Resolve thru the specific COPY
...

{
    ...
    if (startAddr != rootAddr) {
        beginAddr =
            rootAddr - charType->getAlignSize(); // or the first address before the root address (whichever is later)
    }
...

{
    ...
    Datatype* charPtrType =
        types->getTypePointer(types->getSizeOfPointer(), charType, rootAddr.getSpace()->getWordSize());
    ...
    while (pos < ptradds.size()) {
        ...
        for (iter = vn->beginDescend(); iter != vn->endDescend(); ++iter) {
            ...
            if (opc == CPUI_PTRADD) {
                ...
                if (op->getIn(2)->getOffset() != ptrAddMult)
                    continue;
...
/// \brief Calculate the  byte offset and any non-constant additive elements between the given Varnode and the \b
/// basePointer
///
...

{
    ...
    for (int4 i = 0; i < indirectPairs.size(); ++i) {
        if (indirectPairs[i].isDuplicate())
            continue;
...
HeapSequence::HeapSequence(Funcdata& fdata, Datatype* ct, PcodeOp* root) : ArraySequence(fdata, ct, root) {
    baseOffset = 0;
...

{
    ...
    if (!ct->isCharPrint())
        return 0; // Copied to a "char" data-type Varnode
    if (ct->isOpaqueString())
        return 0;
    if (!outvn->isAddrTied())
        return 0;
