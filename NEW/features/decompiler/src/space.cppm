...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/space.cc.
...
AddrSpace::AddrSpace(AddrSpaceManager* m, const Translate* t, spacetype tp, const string& nm, bool bigEnd, uint4 size,
...
OtherSpace::OtherSpace(AddrSpaceManager* m, const Translate* t) : AddrSpace(m, t, IPTR_PROCESSOR) {
    clearFlags(heritaged | does_deadcode);
    setFlags(is_otherspace);
}
...
UniqueSpace::UniqueSpace(AddrSpaceManager* m, const Translate* t) : AddrSpace(m, t, IPTR_INTERNAL) {
    setFlags(hasphysical);
}
...
/// \param t is the processor translator
OverlaySpace::OverlaySpace(AddrSpaceManager* m, const Translate* t) : AddrSpace(m, t, IPTR_PROCESSOR) {
