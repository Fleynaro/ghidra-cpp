// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/flow.cc.
...
FlowInfo::FlowInfo(Funcdata& d, PcodeOpBank& o, BlockGraph& b, vector<FuncCallSpecs*>& q)
    : data(d), obank(o), bblocks(b), qlst(q), baddr(d.getAddress().getSpace(), 0),
      eaddr(d.getAddress().getSpace(), ~((uintb)0)), minaddr(d.getAddress()), maxaddr(d.getAddress())
...
FlowInfo::FlowInfo(Funcdata& d, PcodeOpBank& o, BlockGraph& b, vector<FuncCallSpecs*>& q, const FlowInfo* op2)
    : data(d), obank(o), bblocks(b), qlst(q), baddr(op2->baddr), eaddr(op2->eaddr), minaddr(d.getAddress()),
...
PcodeOp* FlowInfo::xrefControlFlow(list<PcodeOp*>::const_iterator oiter, bool& startbasic, bool& isfallthru,
                                   FuncCallSpecs* fc)
...
/// \param startbasic indicates of the instruction starts a basic block and passes back whether the next instruction
/// does
