// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/coreaction.cc.
    ...
    for (;;) {
        if (iter == begiter) {
            if (curblock->sizeIn() != 1)
                return 0; // Can trace back to next basic block if only one path
        ...
        PcodeOp* curop = *iter;
        if (curop->isCall())
            return 0; // Don't try to trace aliasing through a call
    ...
    while (begiter != enditer) {
        ...
        if (!isStackRelative(spcbasein, x, constx)) { // If x is not stack relative
            ...
            if (!isStackRelative(spcbasein, x, constx))
                continue; // Now maybe the new x is stack relative
        }
    ...
    while (step < 2) {
        ...
        if (step == 0) {
            ...
            if (op->code() != CPUI_SUBPIECE)
                continue; // Is the big register split into pieces
            ...
            if (op->code() != CPUI_PIECE)
                continue; // Is the big register formed from smaller pieces
...

{
    ...
    for (int4 i = 0; i < bblocks.getSize(); ++i) {
        ...
        while (iter != bl->endOp()) {
            op = *iter++;
            if (op->getAddr().getOffset() != startoffset)
                break;
...

{
    for (int4 i = 0; i < 3; ++i) {
        ...
        if (dt->getMetatype() == TYPE_PTR) {
            ...
            if (spc != (AddrSpace*)0 &&
                spc->getAddrSize() == vn->getSize()) // If provided a pointer with space attribute
                return spc;                          // use that
        }
        switch (op->code()) {
            case CPUI_INT_ADD:
...
MapEntry* ActionConstantPtr::isPointer(AddrSpace* spc, Varnode* vn, PcodeOp* op, int4 slot, Address& rampoint,
                                       uintb& fullEncoding, Funcdata& data) {
    bool needexacthit;
    ...
    if (vn->getTypeReadFacing(op)->getMetatype() == TYPE_PTR) { // Are we explicitly marked as a pointer
        ...
        if (vn->isTypeLock())
            return (MapEntry*)0; // Locked as NOT a pointer
        needexacthit = true;
...

{
    ...
    while (begiter != enditer) {
        vn = *begiter++;
        if (!vn->isConstant())
            break; // New varnodes may get inserted between begiter and enditer
        if (vn->getOffset() == 0)
            continue; // Never make constant 0 into spacebase
        uint4 check = vn->getSymbolCheck();
        if (check == Varnode::symcheck_complete)
            continue; // Have we checked this variable before
        if (check == Varnode::symcheck_incomplete && vn->getType()->getMetatype() != TYPE_PTR)
            continue;
        if (vn->hasNoDescend())
            continue;
        if (vn->isSpacebase())
        ...
        OpCode opc = op->code();
        if (opc == CPUI_INT_ADD) {
            if (op->getIn(1 - slot)->isSpacebase())
                continue; // Make sure other side is not a spacebase already
...

{
    ...
    for (int4 i = 0; i < data.numCalls(); ++i) {
        ...
        if (data.hasTypeRecoveryStarted()) {
            ...
            if ((ct->getMetatype() == TYPE_PTR) && (((TypePointer*)ct)->getPtrTo()->getMetatype() == TYPE_CODE)) {
...

{
    ...
    if (zext->code() != CPUI_INT_ZEXT)
        return;
    if (!zext->getIn(0)->constantMatch(0))
        return;
    list<PcodeOp*>::const_iterator iter;
    ...
    for (iter = data.beginLoc(); iter != data.endLoc(); ++iter) {
        ...
        if (vn->isInput()) {
            ...
            if (!op->isMarker()) {
                ...
                if (op->getIn(0)->getAddr() !=
                    outvn->getAddr())        // Check if storage address changes from input to output
    ...
    if (fc->isOutputLocked()) {
        ...
        if (outtype->getMetatype() != TYPE_VOID) {
            ...
            if (addr.getSpace()->getType() == IPTR_SPACEBASE) {
                // Delay creating output Varnode until heritage of the stack, when we know relative value of the stack
                // pointer
...

{
    ...
    if (newparam.size() <= 2) // Easy zero or one return varnode case
        ...
        Address joinaddr =
            data.getArch()->constructJoinAddress(data.getArch()->translate, trialhi.getAddress(), trialhi.getSize(),
    ...
    if (active != (ParamActive*)0) {
        ...
        for (iter = data.beginOp(CPUI_RETURN); iter != iterend; ++iter) {
            ...
            if (op->isDead())
                continue;
            if (op->getHaltType() != 0)
                continue; // Don't evaluate special halts
...

{
    ...
    for (; eiter != endeiter; ++eiter) { // Iterate through saved registers
        if ((*eiter).getType() == EffectRecord::killedbycall)
            continue; // Not saved
            ...
            switch (op->code()) {
                ...
                case CPUI_INT_AND:
                    ...
                    if (op->getIn(1)->isConstant()) {
                        ...
                        if ((val == ((mask << 8) & mask)) || (val == ((mask << 16) & mask)) ||
                            (val == ((mask << 32) & mask))) {
                            indlist.push_back(op);
                            break;
                        }
...

{
    ...
    if (opc != CPUI_INT_ADD)
        return false;
    if (!isCopyConstant(op->getIn(1)))
        return false;
    if (!isCopyConstant(op->getIn(0)))
        return false;
    return true;
}
...

{
    ...
    while (curVn->isWritten()) {
        ...
        if ((evalType & (PcodeOp::binary | PcodeOp::ternary)) != 0) {
            ...
            // We test for INDIRECTs coming into the MULTIEQUAL.  If there is at least one, we prevent it from
            // collapsing, otherwise we assume the MULTIEQUAL itself is unlikely to collapse.
...

{
    ...
    if (slot < 0 && curType->getMetatype() == TYPE_PTR)
        return false; // Cannot take field of pointer during assignment
...

{
    ...
    if (!inType->needsResolution() && !outType->needsResolution())
        return false;
    int4 inResolve = -1;
    int4 outResolve = -1;
        ...
        for (iter = bb->beginOp(); iter != bb->endOp(); ++iter) {
            ...
            if (opc == CPUI_PTRADD) { // Check for PTRADD that no longer fits its pointer
                ...
                if (ct->getMetatype() != TYPE_PTR ||
                    ((TypePointer*)ct)->getPtrTo()->getAlignSize() !=
                        AddrSpace::addressToByteInt(sz, ((TypePointer*)ct)->getWordSize()))
    ...
    for (int4 i = 0; i < numfunc; ++i) {
        ...
        if (fc->isBadJumpTable()) {
            ...
            if (sym == (Symbol*)0)
                continue;
            if (sym->isNameLocked())
                continue; // Override any unlocked name
            if (sym->getScope() != localmap)
                continue; // Only name this in the local scope
...

{
    ...
    for (uint4 i = 0; i < varlist.size(); ++i) { // Do the actual naming in the original (address based) order
        ...
        if (vn->isFree())
            continue;
        if (vn->isInput())
            continue; // Don't override unaffected or input naming strategy
        HighVariable* high = vn->getHigh();
        if (high->getNumMergeClasses() > 1)
            continue; // Don't inherit a name if speculatively merged
...

{
    ...
    HighVariable* high = vn->getHigh();
    if ((high != (HighVariable*)0) && (high->numInstances() > 1))
...

{
    ...
    if (!firstuse->isCall())
        return;
    if (firstuse->getOut() != (Varnode*)0)
        return;
    if (firstuse->numInput() < 2)
        return; // Must have at least 1 parameter (plus destination varnode)
    if (firstuse->getIn(1) != vn)
        return;                        // First parameter must result of new
...

{
    ...
    for (int4 i = 0; i < 2; ++i) {
        ...
        PcodeOp* op = vncur->getDef();
        OpCode opc = op->code();
        if ((opc != CPUI_INT_ADD) && (opc != CPUI_PTRSUB) && (opc != CPUI_PTRADD) && (opc != CPUI_INT_XOR))
            continue;
        if (var[1 - i] != op->getIn(0))
            continue;
        if (op->getIn(1)->isConstant())
            return false;
    }
...

{
    ...
    if ((!vn1->isWritten()) || (!vn2->isWritten())) {
        if (vn1->isConstant() && vn2->isConstant())
            return (vn1->getOffset() ==
                    vn2->getOffset()); // FIXME: these could be NEAR each other and still have an alias
...

{
    ...
    if (op->isCall() || (op->code() == CPUI_LOAD)) { // loads crossing calls
        for (i = 0; i < data.numCalls(); ++i) {
            ...
            if (vn->getCover()->contain(callop, 2))
                return false;
        }
    }
...

{
    ...
    for (int4 i = 0; i < graph.getSize(); ++i) {
        BlockBasic* bb = (BlockBasic*)graph.getBlock(i);
        if (!bb->isDelayedDonothing())
            continue;
        if (bb->isDoNothing()) {
            if (removingCreatesRedundancy(bb))
                continue;
            if ((bb->sizeOut() == 1) && (bb->getOut(0) == bb)) { // Infinite loop
                if (!bb->isDonothingLoop()) {
                    bb->setDonothingLoop();
...

{
    ...
    if ((newval == vn->getConsume()) && vn->isConsumeVacuous())
        return;
            ...
            if (op->getIn(1)->getSpace()->getType() == IPTR_IOP) {
                ...
                if (!indop->isDead()) {
                    if (indop->code() == CPUI_COPY) {
                        ...
                        // If we reach here, there isn't a true block of INDIRECT (RuleIndirectCollapse will convert it
                        // to COPY)
                    } else
                        indop->setIndirectSource();
                }
...

{
    ...
    for (i = 0; i < manage->numSpaces(); ++i) {
        spc = manage->getSpace(i);
        if (spc == (AddrSpace*)0 || !spc->doesDeadcode())
            continue;
        if (data.deadRemovalAllowed(spc))
            continue; // Mark consumed if we have NOT heritaged
...
void ActionConditionalConst::collectReachable(Varnode* vn, vector<PcodeOpNode>& phiNodeEdges,
            ...
            if (opc == CPUI_MULTIEQUAL) {
                ...
                for (tmpOp.slot = 0; tmpOp.slot < op->numInput(); ++tmpOp.slot) {
                    if (op->getIn(tmpOp.slot) != vn)
                        continue; // Find incoming slot for current Varnode
                    // Don't count as flow if coming thru excised edge
                    if (!binary_search(phiNodeEdges.begin(), phiNodeEdges.end(), tmpOp))
                        break;
                }
...
void ActionConditionalConst::placeMultipleConstants(vector<PcodeOpNode>& phiNodeEdges, vector<int4>& marks,
    ...
    for (int4 i = 0; i < phiNodeEdges.size(); ++i) {
        if (marks[i] != 2)
            continue; // Check that the MULTIQUAL is marked as flowing together
...

{
    if ((op->getEvalType() & PcodeOp::special) != 0)
        return;
    if (op->getOpcode()->isFloatingPointOp())
        return;
    Varnode* outvn = op->getOut();
    if (outvn->getSize() > sizeof(uintb))
        return;
...
void ActionConditionalConst::handlePhiNodes(Varnode* varVn, Varnode* constVn, vector<PcodeOpNode>& phiNodeEdges,
                                            Funcdata& data)

{
    ...
    for (int4 i = 0; i < phiNodeEdges.size(); ++i) {
        if (results[i] != 1)
            continue; // Check for disconnected path that does not flow into another path
    ...
    if (hasFlowTogether) {
        placeMultipleConstants(phiNodeEdges, results, constVn,
                               data); // Add COPY assignment for edges that flow together
        count += 1;
    }
}
            ...
            if (opc == CPUI_INDIRECT) // Don't propagate constant into these
                ...
                if (bl == constBlock) { // The immediate edge from the conditional block, coming into a MULTIEQUAL
                    if (op->getIn(point.inSlot) == varVn) {
                        ...
                        if (point.value > 1)
                            continue;
                        if (op->getOut()->isAddrTied())
                            continue;
                        if (testAlternatePath(varVn, op, point.inSlot, 2))
                            continue;
...
void ActionConditionalConst::findConstCompare(list<ConstPoint>& points, Varnode* boolVn, FlowBlock* bl, bool* blockDom,
                                              bool flipEdge) {
    if (!boolVn->isWritten())
        return;
    ...
    if (varVn->loneDescend() != (PcodeOp*)0)
        return;
    if (flipEdge)
...

{
    ...
    if (spctype->getMetatype() != TYPE_SPACEBASE)
        return;
            ...
            if (!data.getFuncProto().possibleInputParam(addr, dt->getSize())) {
                ...
                if (data.getFuncProto().unjustifiedInputParam(addr, dt->getSize(), vData)) {
                    if (vData.getAddr() == addr && vData.size > dt->getSize()) {
                        // If the address range is unjustified but can be justified by just changing the size (big
                        // endian case) Change the data-type to be the justified size
    ...
    while (iter != enditer) {
        ...
        do {
            ...
            while (iter2 != begiter) {
                --iter2;
                vn = *iter2;
                if (vn->getSpace() != vdata.space)
                    continue;
                uintb offset = vn->getOffset() + vn->getSize() - 1; // Last offset in varnode
                if ((offset >= vdata.offset) &&
                    (vn->getOffset() < vdata.offset)) { // If there is overlap that extends size
                    overlaps = true;
            ...
            if (!overlaps)
                break; // Found no additional overlaps, go with current justified container
...
void ActionInferTypes::propagationDebug(Architecture* glb, Varnode* vn, const Datatype* newtype, PcodeOp* op, int4 slot,
                                        Varnode* ptralias)
...

{
    ...
    if (outslot < 0)
        ...
        if (outvn->isAnnotation())
            return false;
    }
    if (outvn->isTypeLock())
        return false; // Can't propagate through typelock
    if (outvn->stopsUpPropagation() && outslot >= 0)
...

{
    ...
    while (iter != enditer) {
        ...
        if (curvn->isAnnotation())
            continue;
        if ((!curvn->isWritten()) && curvn->hasNoDescend())
            continue;
        if (curvn->isTypeLock())
            continue;
        if (curvn->getSymbolEntry() != (SymbolEntry*)0)
    ...
    for (iter = data.beginOp(CPUI_RETURN); iter != iterend; ++iter) {
        ...
        if (retop->numInput() > 1) {
            ...
            if (vn->getSize() != baseSize)
                continue;
            if (isBool && vn->getNZMask() > 1)
                continue; // Don't propagate bool if value is not necessarily 0 or 1
            if (vn->getTempType() == ct)
...

{
    if (isDefaultGroups)
        return;
    groupmap.clear();
    const char* members[] = {"base",           "protorecovery",   "protorecovery_a",
                             "deindirect",     "localrecovery",   "deadcode",
                             "typerecovery",   "stackptrflow",    "blockrecovery",
                             "stackvars",      "deadcontrolflow", "switchnorm",
                             "cleanup",        "splitcopy",       "splitpointer",
                             "merge",          "dynamic",         "casts",
                             "analysis",       "fixateglobals",   "fixateproto",
                             "constsequence",  "bitfields",       "segment",
                             "returnsplit",    "nodejoin",        "doubleload",
                             "doubleprecis",   "unreachable",     "subvar",
                             "floatprecision", "conditionalexe",  ""};
    setGroup("decompile", members);

    const char* jumptab[] = {"base",     "noproto", "localrecovery", "deadcode",          "stackptrflow",   "stackvars",
                             "analysis", "segment", "subvar",        "normalizebranches", "conditionalexe", ""};
    setGroup("jumptable", jumptab);

    const char* normali[] = {"base",
                             "protorecovery",
                             "protorecovery_b",
                             "deindirect",
                             "localrecovery",
    ...
    const char* paramid[] = {
        "base",         "protorecovery", "protorecovery_b", "deindirect",     "localrecovery",   "deadcode",
        "typerecovery", "stackptrflow",  "siganalysis",     "stackvars",      "deadcontrolflow", "analysis",
        "fixateproto",  "unreachable",   "subvar",          "floatprecision", "conditionalexe",  ""};
...

{
    ...
    act->addAction(new ActionFuncLinkOutOnly("noproto"));
    {
        actfullloop = new ActionGroup(Action::rule_repeatapply, "fullloop");
        {
            ...
            actmainloop->addAction(
                new ActionDynamicMapping("dynamic"));            // Must come before restructurevarnode and infertypes
