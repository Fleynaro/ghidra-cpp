// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/userop.hh.
...
public:
    ...
    uint4 getDisplay(void) const {
        return (flags & (annotation_assignment | no_operator | display_string));
...
public:
    DatatypeUserOp(const string& nm, Architecture* g, int4 ind, Datatype* out, Datatype* in0 = (Datatype*)0,
                   Datatype* in1 = (Datatype*)0, Datatype* in2 = (Datatype*)0, Datatype* in3 = (Datatype*)0);
    virtual Datatype* getOutputLocal(const PcodeOp* op) const;
    virtual Datatype* getInputLocal(const PcodeOp* op, int4 slot) const;
    virtual void decode(Decoder& decoder) {}
};
...
public:
    InjectedUserOp(const string& nm, Architecture* g, int4 ind, int4 injid) : UserPcodeOp(nm, g, injected, ind) {
        injectid = injid;
    } ///< Constructor
...
public:
    VolatileOp(const string& nm, Architecture* g, uint4 tp, int4 ind) : UserPcodeOp(nm, g, tp, ind) {} ///< Constructor
...
public:
    VolatileReadOp(const string& nm, Architecture* g, bool functional)
        : VolatileOp(nm, g, volatile_read, BUILTIN_VOLATILE_READ) {
...
public:
    VolatileWriteOp(const string& nm, Architecture* g, bool functional)
        : VolatileOp(nm, g, volatile_write, BUILTIN_VOLATILE_WRITE) {
...
public:
    ...
    void manualCallOtherFixup(const string& useropname, const string& outname, const vector<string>& inname,
