// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/funcdata_op.cc.
...

{
    ...
    flag &= (PcodeOp::halt | PcodeOp::badinstruction | PcodeOp::unimplemented | PcodeOp::noreturn | PcodeOp::missing);
    if (flag == 0)
...

{
    ...
    if (multSize == 1)
        return; // If no multiplier, we are done
...
PcodeOp* Funcdata::findPrimaryBranch(const Address& addr, bool findBranch, bool findCall, bool findCallother,
                                     bool findReturn) {
...

{
    ...
    if (op->code() == CPUI_INT_SLESSEQUAL) {
        ...
        if ((diff == -1) && (val == calc_int_min(vn->getSize())))
            return false;
        if ((diff == 1) && (val == calc_int_max(vn->getSize())))
            return false;
        opSetOpcode(op, CPUI_INT_SLESS);
    } else { // Check for unsigned overflow
        if ((diff == -1) && (val == 0))
            return false;
        if ((diff == 1) && (val == calc_uint_max(vn->getSize())))
            return false;
...
/// \return 0 if the change normalizes, 1 if the change denormalizes or is ambivalent, 2 if flip in place is not
/// possible
...

{
    ...
    for (iter = vn->beginDescend(); iter != vn->endDescend(); ++iter) {
        ...
        if (earliest != (PcodeOp*)0) {
            if (earliest->getSeqNum().getOrder() < res->getSeqNum().getOrder())
...

{
    ...
    if (op->code() == CPUI_CAST) {
        ...
        if (!vn->isExplicit()) { // If CAST is part of expression, we need to move the previous op as well
            ...
            if (prevOp->isCall())
                return false;
            if (op->previousOp() != prevOp)
                return false; // Previous op must exist and feed into the CAST
        }
    }
    ...
    do {
        ...
        if (opc != CPUI_COPY && opc != CPUI_CAST)
            break; // Limit ourselves to only crossing COPY and CAST ops
        if (rootvn == nextOp->getIn(0))
