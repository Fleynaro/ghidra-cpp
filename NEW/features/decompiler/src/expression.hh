// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/expression.hh.
...
public:
    ...
    bool getFlip(void) const {
        return matchflip;
    } ///< Return \b true if the expressions are anti-correlated
};
...
public:
    ...
    void gatherTwoTermsSubtract(Varnode* a,
                                Varnode* b); ///< Walk expression given two roots being subtracted from one another
...
protected:
    void getStructures(Datatype* dt, int4 initByteOff, int4 leastBitOff,
                       bool isBigEndian); ///< Recover the structure(s) holding the bitfield
public:
    ...
    const Varnode*
    recoverStructurePointer(const Varnode* vn,
                            int4 offset); ///< Recover the Varnode holding the pointer to the parent structure
    bool isValid(void) const {
        return (bitfield != (const TypeBitField*)0);
    } ///< Is \b this a valid bitfield expression
    static const TypeBitField*
    getPullField(const PcodeOp* pull); ///< Get field description corresponding to given ZPULL or SPULL
};
