// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/variable.cc.
...

{
    ...
    for (iter = group->pieceSet.begin(); iter != group->pieceSet.end(); ++iter) {
        ...
        if (otherPiece == this)
            continue;
        if (endOffset <= otherPiece->groupOffset)
            continue;
        int4 otherEndOffset = otherPiece->groupOffset + otherPiece->size;
        if (groupOffset >= otherEndOffset)
            continue;
...
{
    if ((high->highflags & (HighVariable::coverdirty | HighVariable::extendcoverdirty)) == 0)
        return;
...

{
    ...
    else if (symbol->getType()->getSize() == vn->getSize() && ((MapEntry*)entry)->getAddr() == vn->getAddr() &&
             !entry->isPiece())
        symboloffset = -1; // A matching entry
    else {
        symboloffset = vn->getAddr().overlapJoin(0, ((MapEntry*)entry)->getAddr(), symbol->getType()->getSize()) +
                       entry->getOffset();
    }
...

{
    ...
    // Prefer NOT internal
    if ((vn1->getSpace()->getType() != IPTR_INTERNAL) && (vn2->getSpace()->getType() == IPTR_INTERNAL))
        return false;
    if ((vn1->getSpace()->getType() == IPTR_INTERNAL) && (vn2->getSpace()->getType() != IPTR_INTERNAL))
        return true;
...
bool HighIntersectTest::testBlockIntersection(HighVariable* a, int4 blk, const Cover& cover, int4 relOff,
    ...
    for (int4 i = 0; i < a->numInstances(); ++i) {
        Varnode* vn = a->getInstance(i);
        if (2 > vn->getCover()->intersectByBlock(blk, cover))
            continue;
        ...
        for (int4 i = 0; i < b->piece->numIntersection(); ++i) {
            ...
            if (a->piece != (VariablePiece*)0) {
                ...
                for (int4 j = 0; j < a->piece->numIntersection(); ++j) {
                    ...
                    if (off > 0 && off >= bPiece->getSize())
                        continue; // Do a piece and b piece intersect at all
                    if (off < 0 && -off >= interPiece->getSize())
                        continue;
                    if (testBlockIntersection(interPiece->getHigh(), blk, bCover, off, blist))
                        return true;
                }
...

{
    ...
    if (vn->hasNoLocalAlias())
        return false; // A local variable is only in scope if it has aliases
