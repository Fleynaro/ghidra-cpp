// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/jumptable.hh.
...
public:
    ...
    void setRange(const CircleRange& rng, Varnode* vn, PcodeOp* op) {
        range = rng;
        normqvn = vn;
        startop = op;
    }
...
public:
    ...
    virtual void buildLabels(Funcdata* fd, vector<Address>& addresstable, vector<uintb>& label,
                             const JumpModel* orig) const = 0;
    ...
    virtual bool sanityCheck(Funcdata* fd, PcodeOp* indop, vector<Address>& addresstable, vector<LoadTable>& loadpoints,
                             vector<int4>* loadcounts) = 0;
...
public:
    JumpModelTrivial(JumpTable* jt) : JumpModel(jt) {
        size = 0;
...
protected:
    ...
    static bool
    isprune(Varnode* vn); ///< Do we prune in here in our depth-first search for the normalized switch variable
    ...
    static bool
    duplicateVarnodes(const vector<Varnode*>& arr); ///< Return \b true if all array elements are the same Varnode
    void getInitialRange(Varnode* vn,
                         CircleRange& rng) const; ///< Get initial range of values the given Varnode can take
    ...
    bool flowsOnlyToModel(Varnode* vn,
                          PcodeOp* trailOp); ///< Check if the given Varnode flows to anything other than \b this model
    bool checkCommonCbranch(vector<Varnode*>& varArray,
                            BlockBasic* bl); ///< Check that all incoming blocks end with a CBRANCH
...
public:
    JumpBasic(JumpTable* jt) : JumpModel(jt) {
...
///   - C -> S and S -> A   A -> S or "out of loop", i.e. S is in a loop, and the guard block doubles as the loop
///   condition
///
...
public:
    JumpAssisted(JumpTable* jt) : JumpModel(jt) {
        assistOp = (PcodeOp*)0;
        switchvn = (Varnode*)0;
        sizeIndices = 0;
    } ///< Constructor
...
public:
    ...
    } ///< Get the display format for integer cases
    void setDisplayFormat(uint4 format) {
        displayFormat = format;
    } ///< Set the display format to use for integer case values
    ...
    int4 getRecoverCount(void) const {
        return recoverCount;
    } ///< Get number of times a recovery has been attempted
    void incrementRecoveryCount(void) {
        recoverCount += 1;
    ...
    void setFoldedDefault(void) {
        defaultIsFolded = true;
    } ///< Mark that the \e default block is a folded CBRANCH target
    bool hasFoldedDefault(void) const {
    ...
    bool foldInGuards(Funcdata* fd) {
        return jmodel->foldInGuards(fd, this);
...
{
    if (blockPosition != op2.blockPosition)
