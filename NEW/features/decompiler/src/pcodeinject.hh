// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/pcodeinject.hh.
...
public:
    InjectParameter(const string& nm, uint4 sz) : name(nm) {
        index = 0;
        size = sz;
    } ///< Constructor
...
public:
    ...
    virtual void
    printTemplate(ostream& s) const = 0; ///< Print the p-code ops of the injection to a stream (for debugging)
    ...
    virtual string getSource(
        void) const = 0; ///< Return a string describing the \e source of the injection (.cspec, prototype model, etc.)
};
...
public:
    ...
    virtual ~ExecutablePcode(void) {
        if (emitter != (PcodeEmit*)0)
            delete emitter;
    }
