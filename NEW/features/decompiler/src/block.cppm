...
{
    if (!toporbottom)
        return false; // No change was made to data-flow
    swapEdges();
    return false;
}
...
{
    if (bl1->getIndex() == 0)
        return true; // Make sure the entry point comes first
    if (bl2->getIndex() == 0)
        return false;
...

{
    ...
    for (int4 repeat = 0; repeat < 2; ++repeat) {
        ...
        int4 rootindex = 0;
        clearEdgeFlags(f_irreducible | f_tree_edge | f_forward_edge | f_cross_edge | f_back_edge | f_loop_edge |
                       f_loop_exit_edge); // Clear spanning tree
...

{
    ...
    while (xi >= 0) { // For each vertex in reverse pre-order
        ...
        for (int4 i = 0; i < sizein; ++i) {
            if (!x->isBackEdgeIn(i))
                continue; // For each back-edge into x
...

{
    ...
    if (ret->outofthis.size() > 2)
        ret->flags |= f_switch_out; // Make sure switch is marked (even if not produced by INDIRECT) as it is needed for
                                    // structuring
    addBlock(ret);
    return ret;
}
  ...
  // to prevent looping
    ...
    while (!path.empty()) {
        ...
        if (i >= bl->sizeOut()) {   // Visited everything below this node, POP
            ...
            state.back() += 1;
            if (bl->isLoopOut(i))
                continue; // Previously marked loop-edge (act as if it doesn't exist)
            nextbl = bl->getOut(i);
...

{
    ...
    do {
        ...
        if (needrebuild) {
            clearEdgeFlags(f_tree_edge | f_forward_edge | f_cross_edge | f_back_edge |
                           f_loop_edge); // Clear the spanning tree
            preorder.clear();
            rootlist.clear();
        }
    } while (needrebuild);
...
{
    if (sizeOut() != 1)
        return false; // A block that does nothing useful
                      // has exactly one out, (no return or cbranch)
    if (sizeIn() == 0)
        return false; // A block that does nothing but
...

{
    ...
    if (nextBlock->getType() != t_basic) {
        ...
        if (nextBlock == (const FlowBlock*)0)
            return;
        nextBlock = nextBlock->subBlock(0);
    }
    if (getOut(0) == nextBlock)
        return;
    if (!op.empty() && op.back()->isBranch())
        return;
    getStop().printRaw(s);
    s << ":   \t[ goto ";
...

{
    ...
    if (initialBlock->sizeOut() != 1)
        return (PcodeOp*)0; // Initializer block must flow only to for loop
    if (lastOp->isBranch()) {
