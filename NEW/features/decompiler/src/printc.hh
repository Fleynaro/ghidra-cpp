// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/printc.hh.
...
public:
    static void ensureRegistration(void);
    virtual PrintLanguage* buildLanguage(Architecture* glb);
};

/// Forces the C printer translation unit into static-library links so its
/// capability registration is available to standalone clients.
...
protected:
    ...
    void buildTypeStack(const Datatype* ct,
                        vector<const Datatype*>& typestack); ///< Prepare to push components of a data-type declaration
    ...
    pushTypeStart(const Datatype* ct,
                  bool noident); ///< Push part of a data-type declaration onto the RPN stack, up to the identifier
    virtual void pushTypeEnd(const Datatype* ct); ///< Push the tail ends of a data-type declaration onto the RPN stack
    void pushBoolConstant(uintb val, const TypeBase* ct, tagtype tag, const Varnode* vn, const PcodeOp* op);
    void pushCharConstant(uintb val, const Datatype* ct, tagtype tag, const Varnode* vn, const PcodeOp* op);
    void pushEnumConstant(uintb val, const TypeEnum* ct, tagtype tag, const Varnode* vn, const PcodeOp* op);
    ...
    bool
    checkArrayDeref(const Varnode* vn) const; ///< Determine whether a LOAD/STORE expression requires pointer '*' syntax
    bool checkBitFieldMember(const Varnode* vn, const TypeBitField* field)
        const; ///< Determine whether a ZPULL/SPULL/INSERT should use '->' or '.' notation
    ...
    void
    emitAnyLabelStatement(const FlowBlock* bl); ///< Emit any required label statement for a given control-flow block
    ...
    virtual void pushConstant(uintb val, const Datatype* ct, tagtype tag, const Varnode* vn, const PcodeOp* op,
                              uint4 displayFormat);
    ...
    virtual void pushPartialSymbol(const Symbol* sym, int4 off, int4 sz, const Varnode* vn, const PcodeOp* op,
    ...
    virtual void push_integer(uintb val, int4 sz, bool sign, tagtype tag, const Varnode* vn, const PcodeOp* op,
                              uint4 displayFormat);
    virtual void push_float(uintb val, int4 sz, tagtype tag, const Varnode* vn, const PcodeOp* op);
...
public:
    ...
    void setBraceFormatFunction(Emit::brace_style style) {
        option_brace_func = style;
    } ///< Set how function declarations are formatted
    void setBraceFormatIfElse(Emit::brace_style style) {
        option_brace_ifelse = style;
    } ///< Set how if/else blocks are formatted
    void setBraceFormatLoop(Emit::brace_style style) {
        option_brace_loop = style;
    } ///< Set how loop blocks are formatted
    void setBraceFormatSwitch(Emit::brace_style style) {
        option_brace_switch = style;
    ...
    virtual string getScopeDelimiter(void) const {
        return scope.print1;
    }
};
