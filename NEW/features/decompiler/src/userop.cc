// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/userop.cc.
...
DatatypeUserOp::DatatypeUserOp(const string& nm, Architecture* g, int4 ind, Datatype* out, Datatype* in0, Datatype* in1,
                               Datatype* in2, Datatype* in3)
...

{
    ...
    if (dynamic_cast<UnspecializedPcodeOp*>(base) ==
        (UnspecializedPcodeOp*)0) // Make sure the userop isn't used for some other purpose
...
SegmentOp::SegmentOp(const string& nm, Architecture* g, int4 ind) : TermPatternOp(nm, g, segment, ind) {
    constresolve.space = (AddrSpace*)0;
}

bool SegmentOp::unify(Funcdata& data, PcodeOp* op, vector<Varnode*>& bindlist) const {
    ...
    if (op->code() != CPUI_CALLOTHER)
        return false;
    if (op->getIn(0)->getOffset() != useropindex)
        return false;
    if (op->numInput() != 3)
        return false;
    innervn = op->getIn(1);
...
JumpAssistOp::JumpAssistOp(Architecture* g) : UserPcodeOp("", g, jumpassist, 0) {
    index2case = -1;
    index2addr = -1;
    defaultaddr = -1;
    calcsize = -1;
...
InternalStringOp::InternalStringOp(Architecture* g) : UserPcodeOp("stringdata", g, string_data, BUILTIN_STRINGDATA) {
...

{
    ...
    switch (i) {
        ...
        case UserPcodeOp::BUILTIN_MEMCPY: {
            ...
            res = new DatatypeUserOp("builtin_memcpy", glb, UserPcodeOp::BUILTIN_MEMCPY, ptrType, ptrType, ptrType,
                                     intType);
            break;
        }
        case UserPcodeOp::BUILTIN_STRNCPY: // Copy "char" elements
        {
            ...
            Datatype* intType = glb->types->getBase(4, TYPE_INT);
            res = new DatatypeUserOp("builtin_strncpy", glb, UserPcodeOp::BUILTIN_STRNCPY, ptrType, ptrType, ptrType,
                                     intType);
            break;
        }
        case UserPcodeOp::BUILTIN_WCSNCPY: // Copy "wchar_t" elements
        {
            ...
            Datatype* ptrType = glb->types->getTypePointer(ptrSize, cType, wordSize);
            Datatype* intType = glb->types->getBase(4, TYPE_INT);
            res = new DatatypeUserOp("builtin_wcsncpy", glb, UserPcodeOp::BUILTIN_WCSNCPY, ptrType, ptrType, ptrType,
                                     intType);
            break;
        }
        default:
...

{
    ...
    if (ind < 0)
        throw LowlevelError("UserOp not assigned an index");
...
/// \param glb is the owning Architecture
void UserOpManage::manualCallOtherFixup(const string& useropname, const string& outname, const vector<string>& inname,
                                        const string& snippet, Architecture* glb)
