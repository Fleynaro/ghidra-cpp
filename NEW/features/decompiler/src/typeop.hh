// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/typeop.hh.
...
public:
    ...
    bool inheritsSign(void) const {
        return ((addlflags & inherits_sign) != 0);
    }
    ...
    bool isFloatingPointOp(void) const {
        return ((addlflags & floatingpoint_op) != 0);
    }
    ...
    virtual Datatype* propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                    int4 outslot);
...
public:
    TypeOpFunc(TypeFactory* t, OpCode opc, const string& n, type_metatype mout, type_metatype min) : TypeOp(t, opc, n) {
        metaout = mout;
        metain = min;
    } ///< Constructor
    virtual Datatype* getOutputLocal(const PcodeOp* op) const;
