// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/bitfield.hh.
...
public:
    ...
    BitFieldNodeState(const BitFieldNodeState& copy, const BitRange& newField, Varnode* vn,
                      bool sgnExt); ///< Copy constructor with new \b bitsField
    /// \brief Can the current Varnode be treated as the isolated bitfield
    bool isFieldAligned(void) const {
        return (bitsField.leastSigBit == 0 && bitsField.numBits == bitsUsed.numBits);
    }
    ...
    bool doesSignExtensionMatch(void) const {
        return isSignExtended == (field->type->getMetatype() == TYPE_INT);
    }
};
...
protected:
    ...
    void establishFields(Varnode* vn,
                         bool followHoles); ///< Build worklist for each bitfield overlapped by given Varnode
...
public:
    BitFieldTransform(Funcdata* f, Datatype* dt,
                      int4 off); ///< Constructor setting up basic info about bitfield data-type
};
...
class BitFieldInsertTransform : public BitFieldTransform {
    ...
    bool verifyLoadStoreOriginalValue(
        uintb mask) const; ///< Test for other STORE ops interfering with the \e original \e value
    bool verifyMappedOriginalValue(
        uintb mask) const; ///< Test for other ops interfering with the mapped \e original \e value
    uintb
    constructOriginalValueMask(void) const; ///< Calculate mask where 1 bits represent all the bits being preserved
    bool
    verifyOriginalValueBits(void) const; ///< Do final check that unINSERTed bits come from the \e original \e value
    bool
    isOverwrittenPartial(const BitFieldNodeState& state); ///< Is given state a partial field that is overwritten later
    ...
    bool isOriginalValue(
        BitFieldNodeState& state); ///< Is the given Varnode a (partial) copy of the original value being INSERTed into
    ...
    bool handleRightBack(BitFieldNodeState& state,
                         PcodeOp* op); ///< Follow bitfield back through INT_SRIGHT by a constant
    ...
    bool testCallOriginal(BitFieldNodeState& state,
                          PcodeOp* op);             ///< Test if a call is producing the \e original \e value
    bool processBackward(BitFieldNodeState& state); ///< Follow field back, creating an InsertRecord if possible
    PcodeOp* setInsertInputs(PcodeOp* op,
                             const InsertRecord& rec); ///< Fill-in INSERT inputs based on given InsertRecord
    void addFieldShift(PcodeOp* insertOp,
                       const InsertRecord& rec);   ///< Create any shift p-code op specified by given InsertRecord
...
class BitFieldPullTransform : public BitFieldTransform {
    ...
    public:
        ...
        PullRecord(const BitFieldNodeState& state, PcodeOp* op,
                   uintb val);                       ///< Construct record for a pull into an INT_EQUAL or INT_NOTEQUAL
    ...
    static bool testConsumed(Varnode* vn,
                             const BitRange& bitField); ///< Test if all consumed bits are in the given bitfield
    ...
    void handleSubpieceForward(const BitFieldNodeState& state,
                               PcodeOp* op);                               ///< Follow bitfield forward through SUBPIECE
    void handleInsertForward(const BitFieldNodeState& state, PcodeOp* op); ///< Follow bitfield forward into INSERT
    void handleLessForward(const BitFieldNodeState& state,
                           PcodeOp* op); ///< Follow bitfield forward through INT_LESS, INT_SLESS
    void handleLeastSigOp(const BitFieldNodeState& state,
                          PcodeOp* op); ///< Follow bitfield into INT_ADD, INT_MULT, INT_OR, INT_XOR
    void handleEqualForward(const BitFieldNodeState& state,
                            PcodeOp* op);          ///< Follow bitfield into INT_EQUAL or INT_NOTEQUAL
    ...
    void applyRecord(PullRecord& rec,
                     TransformState& state);        ///< Perform transform corresponding to the given PullRecord
...
public:
    BitFieldPullTransform(Funcdata* f, Varnode* r, Datatype* dt,
                          int4 off); ///< Construct from Varnode containing bitfields
