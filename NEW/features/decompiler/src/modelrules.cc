// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/modelrules.cc.
...
MetaTypeFilter::MetaTypeFilter(type_metatype meta, int4 min, int4 max) : SizeRestrictedFilter(min, max) {
    metaType = meta;
}

MetaTypeFilter::MetaTypeFilter(const MetaTypeFilter& op2) : SizeRestrictedFilter(op2) {
...
HomogeneousAggregate::HomogeneousAggregate(const HomogeneousAggregate& op2) : SizeRestrictedFilter(op2) {
    metaType = op2.metaType;
...

{
    ...
    if (!primitives.isValid() || primitives.size() == 0 || primitives.containsUnknown() || !primitives.isAligned() ||
        primitives.containsHoles())
        return false;
...
void AssignAction::justifyPieces(vector<VarnodeData>& pieces, int4 offset, bool isBigEndian, bool consumeMostSig,
                                 bool justifyRight) {
...
GotoStack::GotoStack(const ParamListStandard* res, int4 val) : AssignAction(res) {
    stackEntry = (const ParamEntry*)0;
    fillinOutputActive = true;
}

GotoStack::GotoStack(const ParamListStandard* res) : AssignAction(res) {
    stackEntry = (const ParamEntry*)0;
    fillinOutputActive = true;
    initializeEntry();
}
...
ConvertToPointer::ConvertToPointer(const ParamListStandard* res) : AssignAction(res) {
...
/// \param res is the new resource set to associate with \b this action
MultiSlotAssign::MultiSlotAssign(const ParamListStandard* res) : AssignAction(res) {
...
MultiSlotAssign::MultiSlotAssign(type_class store, bool stack, bool mostSig, bool align, bool justRight,
...
int4 MultiSlotDualAssign::getFirstUnused(int4 iter, const vector<const ParamEntry*>& tiles,
...
/// Set default configuration
/// \param res is the new resource set to associate with \b this action
MultiSlotDualAssign::MultiSlotDualAssign(const ParamListStandard* res) : AssignAction(res) {
    isBigEndian = res->isBigEndian();
    fillinOutputActive = true;
...
MultiSlotDualAssign::MultiSlotDualAssign(type_class baseStore, type_class altStore, bool stack, bool mostSig,
                                         bool justRight, bool fillAlt, const ParamListStandard* res)
    : AssignAction(res) {
    isBigEndian = res->isBigEndian();
    fillinOutputActive = true;
    baseType = baseStore;
...
ConsumeAs::ConsumeAs(type_class store, const ParamListStandard* res) : AssignAction(res) {
    resourceType = store;
    fillinOutputActive = true;
}
...
HiddenReturnAssign::HiddenReturnAssign(const ParamListStandard* res, uint4 code) : AssignAction(res) {
    retCode = code;
}
...
ConsumeExtra::ConsumeExtra(const ParamListStandard* res) : AssignAction(res) {
    resourceType = TYPECLASS_GENERAL;
    matchSize = true;
}

ConsumeExtra::ConsumeExtra(type_class store, bool match, const ParamListStandard* res) : AssignAction(res) {
    resourceType = store;
    matchSize = match;
    initializeEntries();
}
...
ConsumeRemaining::ConsumeRemaining(const ParamListStandard* res) : AssignAction(res) {
    resourceType = TYPECLASS_GENERAL;
}

ConsumeRemaining::ConsumeRemaining(type_class store, const ParamListStandard* res) : AssignAction(res) {
    resourceType = store;
    initializeEntries();
}
...
/// \param res is the new resource set to associate with \b this action
ExtraStack::ExtraStack(const ParamListStandard* res) : AssignAction(res) {
    afterBytes = -1;
    afterStorage = TYPECLASS_GENERAL;
    stackEntry = (const ParamEntry*)0;
}

ExtraStack::ExtraStack(type_class storage, int4 offset, const ParamListStandard* res) : AssignAction(res) {
