...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/heritage.cc.
...
int4 Heritage::collect(MemRange& memrange, vector<Varnode*>& read, vector<Varnode*>& write, vector<Varnode*>& input,
                       vector<Varnode*>& remove) const {
    read.clear();
    write.clear();
    input.clear();
    remove.clear();
...

{
    ...
    if (mostsigsize != 0) {
        ...
        if (op->isCall() && callOpIndirectEffect(pieceaddr, mostsigsize, op)) { // Does CALL have an effect on piece
            newop = fd->newIndirectCreation(op, pieceaddr, mostsigsize,
                                            false); // Don't create a new big read if write is from a CALL
            mostvn = newop->getOut();
        } else {
            newop = fd->newOp(2, op->getAddr());
    ...
    if (overlap != 0) {
        ...
        if (addr.isBigEndian())
            ...
            pieceaddr = addr;
        if (op->isCall() &&
            callOpIndirectEffect(pieceaddr, overlap, op)) { // Unless CALL definitely has no effect on piece
            newop = fd->newIndirectCreation(op, pieceaddr, overlap,
                                            false); // Don't create a new big read if write is from a CALL
...
void Heritage::splitPieces(const vector<Varnode*>& vnlist, PcodeOp* insertop, const Address& addr, int4 size,
                           Varnode* startvn)
    ...
    for (int4 i = 0; i < spc->numSpacebase(); ++i) {
        ...
        while (!path.empty()) {
            ...
            switch (op->code()) {
                ...
                case CPUI_SEGMENTOP: {
                    if (op->getIn(2) != curNode.vn)
                        break; // Check that stackpointer comes in as inner pointer
...
void Heritage::guard(const Address& addr, int4 size, bool addIndirects, vector<Varnode*>& read, vector<Varnode*>& write,
...
void Heritage::guardOutputOverlapStack(PcodeOp* callOp, const Address& addr, int4 size, const Address& retAddr,
                                       int4 retSize, vector<Varnode*>& write) {
    int4 sizeFront = (int4)(retAddr.getOffset() - addr.getOffset());
    int4 sizeBack = size - retSize - sizeFront;
    PcodeOp* insertPoint = callOp;
...

{
    ...
    for (int4 i = 0; i < fd->numCalls(); ++i) {
        ...
        if (fc->isInputActive() && tryregister) {
            ...
            if (inputCharacter ==
                ParamEntry::contains_justified) { // Call could be using this range as an input parameter
                ParamActive* active = fc->getActiveInput();
                if (active->whichTrial(transAddr, size) < 0) { // If not already a trial
                    PcodeOp* op = fc->getOp();
                    active->registerTrial(transAddr, size);
                    ...
                    fd->opInsertInput(op, vn, op->numInput());
                       ...
                       ParamEntry::contained_by) // Call may be using part of this range as an input parameter
...
TaskList::iterator Heritage::refinement(TaskList::iterator memiter, const vector<Varnode*>& readvars,
    ...
    if (lastpos == 0)
        return disjoint.end(); // No non-trivial refinements
        ...
        while (iter != enditer) {
            ...
            if (prev == 0) // All new location being heritaged, or intersecting with something new
                ...
                if ((!needwarning) && (info->deadremoved > 0) && !fd->isJumptableRecoveryOn()) {
                    ...
                    // In most cases, a truly new overlapping read will hit the bumpDeadcodeDelay either here or in
                    // prev==2
                    needwarning = true;
                    bumpDeadcodeDelay(vn->getSpace());
                    warnvn = vn;
