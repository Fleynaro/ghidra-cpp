// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/signature.hh.
...
public:
    ...
    void
    print(ostream& s) const; ///< Print the feature hash and a brief description of \b this feature to the given stream
...
class SignatureEntry {
    ...
    enum SignatureFlags {
        ...
        SIG_NODE_NOT_EMITTED =
            0x4, ///< Varnode is not emitted as a formal feature (it might be hashed with other features)
    ...
    } ///< Return \b true if \b this node has been visited before
    void setVisited(void) {
        flags |= VISITED;
    } ///< Mark that \b this node has been visited
...
public:
    ...
    } ///< Return \b true if \b this node has no inputs
    bool isNotEmitted(void) const {
        return ((flags & SIG_NODE_NOT_EMITTED) != 0);
    } ///< Return \b true if \b this is not emitted as a feature
    bool isCommutative(void) const {
        return ((flags & SIG_NODE_COMMUTATIVE) != 0);
    } ///< Return \b true if inputs to \b this are unordered
    bool isStandaloneCopy(void) const {
        return ((flags & SIG_NODE_STANDALONE) != 0);
    } ///< Return \b true if \b this is a stand-alone COPY
    int4 numInputs(void) const {
        return inSize;
    } ///< Return the number incoming edges to \b this node
    ...
    Varnode* getVarnode(void) const {
        return vn;
    } ///< Get the underlying Varnode which \b this overlays
    ...
    void
    verifyNoiseRemoval(map<int4, SignatureEntry*>& sigMap) const; ///< Verify \b shadow is set correctly for \b this
    static void
    verifyAllNoiseRemoval(map<int4, SignatureEntry*>& sigMap); ///< Verify all nodes have \b shadow set correctly
#endif
};
...
public:
    ...
    BlockBasic* getBlock(void) const {
        return bl;
    } ///< Get the underlying basic block which \b this overlays
    hashword getHash(void) const {
        return hash[0];
    } ///< Get the current hash value
};
...
public:
    VarnodeSignature(const Varnode* v, hashword h) : Signature(h) {
        vn = v;
    } ///< Constructor
...
public:
    BlockSignature(const BlockBasic* b, hashword h, const PcodeOp* o1, const PcodeOp* o2) : Signature(h) {
        bl = b;
        op1 = o1;
        op2 = o2;
    } ///< Constructor
...
public:
    CopySignature(const BlockBasic* b, hashword h) : Signature(h) {
        bl = b;
    } ///< Constructor
    virtual void encode(Encoder& encoder) const;
    virtual void printOrigin(ostream& s) const;
...
private:
    ...
    void
    collectBlockSigs(void); ///< Generate the final feature(s) for each basic block from its BlockSignatureEntry overlay
...
extern void debugSignature(Funcdata* fd,
                           Encoder& encoder); ///< Generate features (with debug info) for a single function
