// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/expression.cc.
...

{
    ...
    if (depth != 0 && (opc1 == CPUI_BOOL_AND || opc1 == CPUI_BOOL_OR || opc1 == CPUI_BOOL_XOR)) {
        if (opc2 == CPUI_BOOL_AND || opc2 == CPUI_BOOL_OR || opc2 == CPUI_BOOL_XOR) {
            if (opc1 == opc2 || (opc1 == CPUI_BOOL_AND && opc2 == CPUI_BOOL_OR) ||
                (opc1 == CPUI_BOOL_OR && opc2 == CPUI_BOOL_AND)) {
                int4 pair1 = evaluate(op1->getIn(0), op2->getIn(0), depth - 1);
                int4 pair2;
                if (pair1 == uncorrelated) {
                    pair1 = evaluate(op1->getIn(0), op2->getIn(1),
                                     depth - 1); // Try other possible pairing (commutative op)
                    if (pair1 == uncorrelated)
                        return uncorrelated;
...

{
    ...
    if (dt->getMetatype() == TYPE_PARTIALSTRUCT) {
        ...
        if (structTmp->getMetatype() != TYPE_STRUCT)
            return;
    ...
    for (;;) {
        ...
        if (tmpDt == (Datatype*)0)
            break;
        if (tmpDt->getMetatype() != TYPE_STRUCT)
            break;
...

{
    ...
    if (num >= 3) {
        if (opc != CPUI_PTRADD)
            return -1; // If this is a PTRADD
        if (op1->getIn(2)->getOffset() != op2->getIn(2)->getOffset())
            return -1; // Make sure the elsize constant is equal
...

{
    ...
    if ((!vn1->isWritten()) || (!vn2->isWritten())) {
        ...
        if (vn1->isInput() && vn2->isInput())
            return false; // Might be the same
        if (vn1->isFree() || vn2->isFree())
            return false; // Might be the same
        return true;
    }
