...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/marshal.cc.
...
AttributeId::AttributeId(const string& nm, uint4 i, int4 scope) : name(nm) {
    id = i;
    if (scope == 0)
...
ElementId::ElementId(const string& nm, uint4 i, int4 scope) : name(nm) {
    id = i;
    if (scope == 0)
        getList().push_back(this);
}
    ...
    switch (spc->getType()) {
        ...
        case IPTR_SPACEBASE:
            if (spc->isFormalStackSpace())
                ...
                outStream.put((TYPECODE_SPECIALSPACE << TYPECODE_SHIFT) |
                              SPECIALSPACE_SPACEBASE); // A secondary register offset space
            break;
        default:
