// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/type.hh.
...
public:
    ...
    bool isOpaqueString(void) const {
        return ((flags & opaque_string) != 0);
    } ///< Is \b this an opaquely encoded string
    bool isPointerToArray(void) const {
        return ((flags & pointer_to_array) != 0);
    } ///< Is \b this a pointer to an array
    bool isPointerRel(void) const {
        return ((flags & is_ptrrel) != 0);
    } ///< Is \b this a TypePointerRel
    bool isFormalPointerRel(void) const {
        return (flags & (is_ptrrel | has_stripped)) == is_ptrrel;
    } ///< Is \b this a non-ephemeral TypePointerRel
    bool hasStripped(void) const {
        return (flags & has_stripped) != 0;
    } ///< Return \b true if \b this has a stripped form
    bool isIncomplete(void) const {
        return (flags & type_incomplete) != 0;
    } ///< Is \b this an incompletely defined data-type
    bool needsResolution(void) const {
        return (flags & needs_resolution) != 0;
    } ///< Is \b this a union or a pointer to union
    bool hasWarning(void) const {
        return (flags & warning_issued) != 0;
    } ///< Has a \e warning been issued about \b this data-type
    bool hasBitfields(void) const {
        return (flags & has_bitfields) != 0;
    } ///< Return \b true if \b this contains/overlaps bitfields
    uint4 inheritForPointer(void) const {
        return (flags & (coretype | warning_issued));
    } ///< Get properties pointers inherit
    uint4 inheritForPartial(void) const {
        return (flags & warning_issued);
    }                                   ///< Get properties \e partial data-types inherit
    ...
    /// at is returned.  For a relative pointer, the innermost structured data-type is returned and the offset passed
    /// back.
    ...
    virtual bool isOffsetValid(int8 off, int8 extra,
                               int8 multiplier) const; ///< Is a given offset into \b this valid, given sub-offset info
    virtual Datatype*
    getStripped(void) const; ///< Get a stripped version of \b this for formal use in formal declarations
    ...
    virtual Datatype* getPartialBase(void) const {
        return (Datatype*)0;
    }
    ...
    int4
    typeOrderFormal(const Datatype& op) const; ///< Order \b this with \b op for selecting a formal high-level data-type
    ...
    Datatype* findSmallestContainer(int8 off, int8 sz,
                                    int8* newoff); ///< Find the smallest component containing the given range
...
public:
    ...
    int4 compareDependency(
        const TypeField& op2) const;     ///< Compare \b this with another TypeField for functional equivalence
...
public:
    ...
    TypeBitField(int4 id, int4 numBits, bool isBigEndian, const string& nm,
                 Datatype* ct); ///< Construct from components
    int4 compare(const TypeBitField& op2)
        const; ///< Compare definition of \b this with another TypeBitField for propagation ordering
    int4 compareDependency(
        const TypeBitField& op2) const;  ///< Compare \b this with another TypeBitField for functional equivalence
    void encode(Encoder& encoder) const; ///< Encode \b this bitfield to a stream
    static bool compareMaxByte(int4 off,
                               const TypeBitField& bitfield) { ///< Compare byte container end-point to the given offset
...
public:
    ...
    TypeBase(int4 s, type_metatype m, const string& n) : Datatype(s, -1, m) {
        name = n;
        displayName = n;
    }
...
public:
    ...
    TypeChar(const TypeChar& op) : TypeBase(op) {
        flags |= Datatype::chartype;
    }
    /// Construct a char (always 1-byte) given a name
    TypeChar(const string& n) : TypeBase(1, TYPE_INT, n) {
        flags |= Datatype::chartype;
...
public:
    ...
    TypeVoid(const TypeVoid& op) : Datatype(op) {
        flags |= Datatype::coretype;
    }
    /// Constructor
    TypeVoid(void) : Datatype(0, 1, TYPE_VOID) {
        name = "void";
        displayName = name;
        flags |= Datatype::coretype;
    }
...
protected:
    ...
    TypePointer(void) : Datatype(0, -1, TYPE_PTR) {
        ptrto = (Datatype*)0;
        wordsize = 1;
        spaceid = (AddrSpace*)0;
        truncate = (TypePointer*)0;
    }

public:
    /// Construct from another TypePointer
    TypePointer(const TypePointer& op) : Datatype(op) {
        ptrto = op.ptrto;
    ...
    virtual void encode(Encoder& encoder) const;
    virtual TypePointer* downChain(int8& off, TypePointer*& par, int8& parOff, bool allowArrayWrap,
                                   TypeFactory& typegrp);
    virtual bool isPtrsubMatching(int8 off, int8 extra, int8 multiplier) const;
...
protected:
    ...
    TypeArray(void) : Datatype(0, -1, TYPE_ARRAY) {
        arraysize = 0;
        arrayof = (Datatype*)0;
    }
...
protected:
    ...
    void setFields(const vector<TypeField>& fd, const vector<TypeBitField>& bit, int4 fixedSize,
                   int4 fixedAlign);         ///< Establish fields for \b this
...
public:
    ...
    TypeStruct(void) : Datatype(0, -1, TYPE_STRUCT) {
    ...
    int4 numBitFields(void) const {
        return bitfield.size();
    } ///< Return the number of bitfields contained by \b this
    const TypeBitField& getBitField(int4 i) const {
        return bitfield[i];
    } ///< Return the i-th bitfield
    const TypeBitField*
    findMatchingBitField(const BitRange& range) const; ///< Return bitfield matching the given bit range
    void collectBitFields(int4 baseOffset, vector<BitFieldTriple>& res, int4 offset,
                          int4 sz) const; ///< Collect bitfield records that overlap given range
    bool hasBitFieldsInRange(int4 offset, int4 sz)
        const; ///< Return \b true if \b this structure has 1 or more bitfields in the given byte range
    virtual const TypeField* findTruncation(int8 off, int4 sz, const PcodeOp* op, int4 slot, int8& newoff) const;
    virtual Datatype* getSubType(int8 off, int8* newoff) const;
    virtual bool nearestArrayedComponentForward(int8 off, Nearest& res) const;
    virtual bool nearestArrayedComponentBackward(int8 off, Nearest& res) const;
    ...
    virtual Datatype* findResolve(const PcodeOp* op, int4 slot);
    virtual int4 findCompatibleResolve(Datatype* ct) const;
    static void assignFieldOffsets(vector<TypeField>& list, vector<TypeBitField>& bitlist, int4& newSize,
                                   int4& newAlign, uint4& flags, const TypeStruct* st);
    static int4 scoreSingleComponent(Datatype* parent, PcodeOp* op,
                                     int4 slot); ///< Determine best type fit for given PcodeOp use
};
...
public:
    ...
    TypeUnion(void) : Datatype(0, -1, TYPE_UNION) {
...
public:
    ...
    Datatype* getParent(void) const {
        return parent;
    } ///< Get the enumeration containing \b this piece
    virtual void printRaw(ostream& s) const;
    ...
    virtual int4 compareDependency(const Datatype& op) const;
    virtual Datatype* clone(void) const {
        return new TypePartialEnum(*this);
    }
    virtual void encode(Encoder& encoder) const;
...
public:
    ...
    virtual int4 compareDependency(const Datatype& op) const;
    virtual Datatype* clone(void) const {
        return new TypePartialStruct(*this);
    }
    virtual Datatype* getStripped(void) const {
        return stripped;
    }
...
public:
    ...
    TypeUnion* getParentUnion(void) const {
        return container;
    ...
    virtual int4 compareDependency(const Datatype& op) const;
    virtual Datatype* clone(void) const {
        return new TypePartialUnion(*this);
    }
    virtual void encode(Encoder& encoder) const;
    virtual Datatype* getStripped(void) const {
        return stripped;
...
protected:
    ...
    /// Internal constructor for decode
    TypePointerRel(void) : TypePointer() {
        offset = 0;
        parent = (Datatype*)0;
...
public:
    ...
    bool
    evaluateThruParent(uintb addrOff) const; ///< Do we display given address offset as coming from the parent data-type
    ...
    int4 getAddressOffset(void) const {
        return AddrSpace::byteToAddressInt(offset, wordsize);
    }
    ...
    virtual int4 compareDependency(const Datatype& op) const;
    virtual Datatype* clone(void) const {
        return new TypePointerRel(*this);
    }
    virtual void encode(Encoder& encoder) const;
    virtual TypePointer* downChain(int8& off, TypePointer*& par, int8& parOff, bool allowArrayWrap,
                                   TypeFactory& typegrp);
...
protected:
    ...
    void setPrototype(TypeFactory* tfact, const PrototypePieces& sig,
                      Datatype* voidtype);                        ///< Establish a function pointer
    ...
    void decodePrototype(Decoder& decoder, bool isConstructor, bool isDestructor,
                         TypeFactory& typegrp); ///< Restore any prototype description
public:
...
public:
    ...
    TypeSpacebase(Architecture* g) : Datatype(0, 1, TYPE_SPACEBASE) {
        spaceid = (AddrSpace*)0;
        glb = g;
        scopeId = 0;
    }
    /// Construct given an address space, scope, and architecture
    TypeSpacebase(AddrSpace* id, uint8 scope, Architecture* g) : Datatype(0, 1, TYPE_SPACEBASE) {
        spaceid = id;
        glb = g;
        scopeId = scope;
    }
...
class TypeFactory {
    ...
    void orderRecurse(vector<Datatype*>& deporder, DatatypeSet& mark,
                      Datatype* ct) const;                    ///< Write out dependency list
    ...
    Datatype* decodeCode(Decoder& decoder, bool isConstructor, bool isDestructor,
                         bool forcecore);                        ///< Restore an element describing a code object
    ...
    void recalcPointerSubmeta(Datatype* base,
                              sub_metatype sub);   ///< Recalculate submeta for pointers to given base data-type
    ...
    void setFields(const vector<TypeField>& fd, const vector<TypeBitField>& bit, TypeStruct* ot, int4 newSize,
                   int4 newAlign, uint4 flags);
    void setFields(const vector<TypeField>& fd, TypeUnion* ot, int4 newSize, int4 newAlign,
                   uint4 flags); ///< Set fields on a TypeUnion
protected:
...
public:
    ...
    TypeUnicode* getProviderUnicode(const string& nm, int4 sz, type_metatype m) {
        return getTypeUnicode(nm, sz, m);
    }
    ...
    TypePointer* getTypePointerStripArray(int4 s, Datatype* pt,
                                          uint4 ws); ///< Construct a pointer data-type, stripping an ARRAY level
    ...
    TypePointer* getTypePointer(int4 s, Datatype* pt, uint4 ws,
                                const string& n);   ///< Construct a named pointer data-type
    ...
    Datatype* getTypedef(Datatype* ct, const string& name, uint8 id,
                         uint4 format); ///< Create a new \e typedef data-type
    TypePointerRel* getTypePointerRel(TypePointer* parentPtr, Datatype* ptrTo,
                                      int4 off); ///< Get pointer offset relative to a container
    ...
    Datatype* getExactPiece(Datatype* ct, int4 offset,
                            int4 size); ///< Get the data-type associated with piece of a structured data-type
...

{
    if (this == &op)
        return 0;
    if (metatype == TYPE_PARTIALUNION)
        return 1; // Prefer partials the least
    if (op.metatype == TYPE_PARTIALUNION)
        return -1;
    if (metatype == TYPE_BOOL)
        return 1; // Prefer bool less than integers
...
inline TypeArray::TypeArray(int4 n, Datatype* ao)
