// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/paramid.cc.
...
// CALL is direct read on input0 (putative/presumptive param flag on params--other inputs).  Special (non-direct) write
// of output. CALLIND same as on CALL.  Special (non-direct) write of output. CALLOTHER is direct read on ALL PARAMETERS
// (input0 and up)--is specified in sleigh.  Direct write if output exists. INDIRECT is least powerful input and output
// of all. MULTIEQUALS is flow through but must test for and not flow through loop paths (whether from param forward our
// return backward directions).
//
        ...
        if (op != ignoreop) {
            ...
            switch (oc) {
                ...
                case CPUI_MULTIEQUAL:
                    // The only op for which there can be a loop in the graph is with the MULTIEQUAL (not for CALL,
                    // etc.). Walk forward only if the path is not part of a loop.
                    if (!op->getParent()->isLoopIn(op->getSlot(vn)))
                        walkforward(state, (PcodeOp*)0, op->getOut());
                    break;
                default:
                    updaterank(DIRECTREAD, state.best);
                    break;
        ...
        case CPUI_MULTIEQUAL:
            ...
            for (int4 slot = 0; slot < op->numInput() && rank != state.terminalrank; slot++)
                if (!op->getParent()->isLoopIn(slot))
                    walkbackward(state, op, op->getIn(slot));
            break;
        default:
...

{
    ...
    if (justproto) { // We only provide info on the recovered prototype
        ...
        for (int4 i = 0; i < num; ++i) {
            ...
            InputParamMeasures.push_back(
                ParamMeasure(param->getAddress(), param->getSize(), param->getType(), ParamMeasure::INPUT));
        ...
        if (!outparam->getAddress().isInvalid()) { // If we don't have a void type
            OutputParamMeasures.push_back(
                ParamMeasure(outparam->getAddress(), outparam->getSize(), outparam->getType(), ParamMeasure::OUTPUT));
        ...
        while (iter != enditer) {
            ...
            ++iter;
            InputParamMeasures.push_back(
                ParamMeasure(invn->getAddr(), invn->getSize(), invn->getType(), ParamMeasure::INPUT));
