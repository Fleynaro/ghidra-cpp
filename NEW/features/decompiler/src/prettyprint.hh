// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/prettyprint.hh.
...
public:
    ...
    /// \param op is the PcodeOp associated with the field (usually PTRSUB or SUBPIECE)
    virtual void tagBitField(const string& name, syntax_highlight hl, const Datatype* ct, int4 id,
                             const PcodeOp* op) = 0;
...
public:
    ...
    /// \param id is an identifier for the field within its structured data-type
    /// \param inOp is the PcodeOp associated with the field (usually PTRSUB or SUBPIECE)
    void tagBitField(const string& name, EmitMarkup::syntax_highlight h, const Datatype* ct, int4 id,
                     const PcodeOp* inOp) {
        tok = name;
        size = tok.size();
        tagtype = bitfield_t;
...
template <typename _type> class circularqueue {
