// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/architecture.hh.
...
public:
    ...
    AddrSpace* getSpaceBySpacebase(const Address& loc,
                                   int4 size) const; ///< Get space associated with a \e spacebase register
    const LanedRegister* getLanedRegister(const Address& loc,
                                          int4 size) const;   ///< Get LanedRegister associated with storage
    ...
    string getScopeDelimiter(void) const {
        return print->getScopeDelimiter();
    } ///< Get the character string separating scope names
#ifdef OPACTION_DEBUG
    void setDebugStream(ostream* s) {
        debugstream = s;
...
#endif
protected:
    void addSpacebase(AddrSpace* basespace, const string& nm, const VarnodeData& ptrdata, int4 truncSize,
                      bool isreversejustified, bool stackGrowth, bool isFormal);
    ...
    void
    addToGlobalScope(const RangeProperties& props); ///< Add a memory range to the set of addresses considered \e global
