// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/constseq.hh.
...
public:
    static const int4
        MINIMUM_SEQUENCE_LENGTH; ///< Minimum number of sequential characters to trigger replacement with CALLOTHER
    ...
    public:
        ...
        bool operator<(const WriteNode& node2) const {
            return op->getSeqNum().getOrder() < node2.op->getSeqNum().getOrder();
        }
    };
...
protected:
    ...
    static bool interfereBetween(PcodeOp* startOp,
                                 PcodeOp* endOp); ///< Check for interfering ops between the two given ops
    bool checkInterference(void); ///< Find maximal set of ops containing the root with no interfering ops in between
    int4 formByteArray(int4 sz, int4 slot, uint8 rootOff,
                       bool bigEndian); ///< Put constant values from COPYs into a single byte array
    uint4
    selectStringCopyFunction(int4& index); ///< Pick either strncpy, wcsncpy, or memcpy function used to copy string
public:
    ArraySequence(Funcdata& fdata, Datatype* ct, PcodeOp* root); ///< Constructor
    bool isValid(void) const {
        return numElements != 0;
    } ///< Return \b true if sequence is found
};
...
class HeapSequence : public ArraySequence {
    ...
    public:
        ...
        void markDuplicate(void) {
            inVn = (Varnode*)0;
        } ///< Note that \b this is a duplicate of another pair
        bool isDuplicate(void) const {
            return (inVn == (Varnode*)0);
        } ///< Return \b true if \b this is marked as a duplicate
    ...
    void removeStoreOps(vector<PcodeOp*>& indirects, vector<IndirectPair>& indirectPairs,
                        PcodeOp* replaceOp); ///< Remove all STORE ops from the basic block
public:
