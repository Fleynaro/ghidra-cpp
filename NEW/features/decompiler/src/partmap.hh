// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/partmap.hh.
...
/// can be changed.
template <typename _linetype, typename _valuetype> class partmap {
public:
...
template <typename _linetype, typename _valuetype>
_valuetype& partmap<_linetype, _valuetype>::clearRange(const _linetype& pnt1, const _linetype& pnt2) {
    split(pnt1);
    split(pnt2);
...
/// \return the corresponding value object
template <typename _linetype, typename _valuetype>
const _valuetype& partmap<_linetype, _valuetype>::bounds(const _linetype& pnt, _linetype& before, _linetype& after,
                                                         int& valid) const {
    if (database.empty()) {
        valid = 3;
        return defaultvalue;
