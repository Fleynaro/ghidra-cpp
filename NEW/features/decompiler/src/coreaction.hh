// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/coreaction.hh.
...
public:
    ActionStackPtrFlow(const string& g, AddrSpace* ss) : Action(0, "stackptrflow", g) {
        stackspace = ss;
    } ///< Constructor
    virtual void reset(Funcdata& data) {
...
class ActionConstantPtr : public Action {
    ...
    static MapEntry* isPointer(AddrSpace* spc, Varnode* vn, PcodeOp* op, int4 slot, Address& rampoint,
...
public:
    ActionDirectWrite(const string& g, bool prop) : Action(0, "directwrite", g) {
...
class ActionMarkExplicit : public Action {
    ...
    static int4 baseExplicit(Varnode* vn,
                             int4 maxref); ///< Make initial determination if a Varnode should be \e explicit
    static int4 multipleInteraction(vector<Varnode*>& multlist); ///< Find multiple descendant chains
    static void processMultiplier(Varnode* vn,
                                  int4 max); ///< For a given multi-descendant Varnode, decide if it should be explicit
...
class ActionLateDoNothing : public Action {
    static bool
    removingCreatesRedundancy(FlowBlock* bl); ///< Does removing the given block create a redundant branch point
public:
...
class ActionConditionalConst : public Action {
    /// \brief Description of a point in control-flow where a Varnode can propagate as a constant down a conditional
    /// branch
    struct ConstPoint {
    ...
    static void findConstCompare(list<ConstPoint>& points, Varnode* boolVn, FlowBlock* bl, bool* blockDom,
                                 bool flipEdge);
    static void pushConstant(list<ConstPoint>& points, PcodeOp* op);
    static void placeMultipleConstants(vector<PcodeOpNode>& phiNodeEdges, vector<int4>& marks, Varnode* constVn,
                                       Funcdata& data);
...
public:
    ActionExtraPopSetup(const string& g, AddrSpace* ss) : Action(rule_onceperfunc, "extrapopsetup", g) {
        stackspace = ss;
    } ///< Constructor
    virtual Action* clone(const ActionGroupList& grouplist) const {
        if (!grouplist.contains(getGroup()))
...
class ActionInputPrototype : public Action {
    ...
    public:
        ...
        InputRef(Address& ad, Datatype* dt) : addr(ad) {
            dataType = dt;
        }                                          ///< Constructor
...
class ActionInferTypes : public Action {
    ...
    static void propagationDebug(Architecture* glb, Varnode* vn, const Datatype* newtype, PcodeOp* op, int4 slot,
                                 Varnode* ptralias);
#endif
