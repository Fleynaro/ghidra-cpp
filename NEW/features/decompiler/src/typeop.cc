// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/typeop.cc.
...
void TypeOp::registerInstructions(vector<TypeOp*>& inst, TypeFactory* tlst, const Translate* trans) {
...
Datatype* TypeOp::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                int4 outslot) {
...
TypeOpCopy::TypeOpCopy(TypeFactory* t)
    : TypeOp(t, CPUI_COPY, "copy")
...
Datatype* TypeOpCopy::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                    int4 outslot) {
...
TypeOpLoad::TypeOpLoad(TypeFactory* t)
    : TypeOp(t, CPUI_LOAD, "load")
...
Datatype* TypeOpLoad::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                    int4 outslot) {
    if ((inslot == 0) || (outslot == 0))
        return (Datatype*)0; // Don't propagate along this edge
    if (invn->isSpacebase())
        return (Datatype*)0;
    Datatype* newtype;
...
TypeOpStore::TypeOpStore(TypeFactory* t)
    : TypeOp(t, CPUI_STORE, "store")
...
Datatype* TypeOpStore::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                     int4 outslot) {
    if ((inslot == 0) || (outslot == 0))
        return (Datatype*)0; // Don't propagate along this edge
    if (invn->isSpacebase())
...
TypeOpBranch::TypeOpBranch(TypeFactory* t)
    : TypeOp(t, CPUI_BRANCH, "goto")
...
TypeOpCbranch::TypeOpCbranch(TypeFactory* t)
    : TypeOp(t, CPUI_CBRANCH, "goto")
...
TypeOpBranchind::TypeOpBranchind(TypeFactory* t)
    : TypeOp(t, CPUI_BRANCHIND, "switch")
...
TypeOpCall::TypeOpCall(TypeFactory* t)
    : TypeOp(t, CPUI_CALL, "call")
...
TypeOpCallind::TypeOpCallind(TypeFactory* t)
    : TypeOp(t, CPUI_CALLIND, "callind")
...
TypeOpCallother::TypeOpCallother(TypeFactory* t)
    : TypeOp(t, CPUI_CALLOTHER, "syscall")
...
TypeOpReturn::TypeOpReturn(TypeFactory* t)
    : TypeOp(t, CPUI_RETURN, "return")
...
TypeOpEqual::TypeOpEqual(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_EQUAL, "==", TYPE_BOOL, TYPE_INT) {
...
Datatype* TypeOpEqual::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                     int4 outslot) {
...
/// \param outslot indicates how the outgoing Varnode is attached to the PcodeOp
/// \return the outgoing data-type or null (to indicate no propagation)
Datatype* TypeOpEqual::propagateAcrossCompare(Datatype* alttype, TypeFactory* typegrp, Varnode* invn, Varnode* outvn,
    ...
    if (invn->isSpacebase()) {
        ...
        if (relPtr->getParent()->getMetatype() == TYPE_STRUCT && relPtr->getByteOffset() >= 0) {
            ...
            newtype =
                typegrp->getTypePointer(relPtr->getSize(), typegrp->getBase(1, TYPE_UNKNOWN), relPtr->getWordSize());
        } else
            newtype = alttype;
    } else
        newtype = alttype;
...
TypeOpNotEqual::TypeOpNotEqual(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_NOTEQUAL, "!=", TYPE_BOOL, TYPE_INT) {
    opflags = PcodeOp::binary | PcodeOp::booloutput | PcodeOp::commutative;
    addlflags = inherits_sign;
...
Datatype* TypeOpNotEqual::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                        int4 outslot) {
    return TypeOpEqual::propagateAcrossCompare(alttype, tlst, invn, outvn, inslot, outslot);
}

TypeOpIntSless::TypeOpIntSless(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_SLESS, "<", TYPE_BOOL, TYPE_INT) {
...
Datatype* TypeOpIntSless::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                        int4 outslot) {
    if ((inslot == -1) || (outslot == -1))
        return (Datatype*)0; // Must propagate input <-> input
    if (alttype->getMetatype() != TYPE_INT)
        return (Datatype*)0; // Only propagate signed things
    return alttype;
}
...
Datatype* TypeOpIntSlessEqual::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                             int4 outslot) {
    if ((inslot == -1) || (outslot == -1))
        return (Datatype*)0; // Must propagate input <-> input
    if (alttype->getMetatype() != TYPE_INT)
...
TypeOpIntLess::TypeOpIntLess(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_LESS, "<", TYPE_BOOL, TYPE_UINT) {
    opflags = PcodeOp::binary | PcodeOp::booloutput;
    addlflags = inherits_sign;
...
Datatype* TypeOpIntLess::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                       int4 outslot) {
    return TypeOpEqual::propagateAcrossCompare(alttype, tlst, invn, outvn, inslot, outslot);
}
...
Datatype* TypeOpIntLessEqual::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                            int4 outslot) {
    return TypeOpEqual::propagateAcrossCompare(alttype, tlst, invn, outvn, inslot, outslot);
}

TypeOpIntZext::TypeOpIntZext(TypeFactory* t) : TypeOpFunc(t, CPUI_INT_ZEXT, "ZEXT", TYPE_UINT, TYPE_UINT) {
...
TypeOpIntSext::TypeOpIntSext(TypeFactory* t) : TypeOpFunc(t, CPUI_INT_SEXT, "SEXT", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::unary;
...
TypeOpIntAdd::TypeOpIntAdd(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_ADD, "+", TYPE_INT, TYPE_INT) {
...
Datatype* TypeOpIntAdd::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                      int4 outslot) {
...

{
    ...
    if (command == 2)
        return (Datatype*)0; // Doesn't look like a good pointer add
    ...
    if (op->getIn(inslot)->isSpacebase()) {
        if (pointer->getPtrTo()->getMetatype() == TYPE_SPACEBASE)
            pointer =
                typegrp->getTypePointer(pointer->getSize(), typegrp->getBase(1, TYPE_UNKNOWN), pointer->getWordSize());
    }
    return pointer;
}
...
TypeOpIntSub::TypeOpIntSub(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_SUB, "-", TYPE_INT, TYPE_INT) {
...
TypeOpIntCarry::TypeOpIntCarry(TypeFactory* t) : TypeOpFunc(t, CPUI_INT_CARRY, "CARRY", TYPE_BOOL, TYPE_UINT) {
...
TypeOpIntScarry::TypeOpIntScarry(TypeFactory* t) : TypeOpFunc(t, CPUI_INT_SCARRY, "SCARRY", TYPE_BOOL, TYPE_INT) {
    opflags = PcodeOp::binary | PcodeOp::commutative | PcodeOp::booloutput;
    addlflags = arithmetic_op;
...
TypeOpIntSborrow::TypeOpIntSborrow(TypeFactory* t) : TypeOpFunc(t, CPUI_INT_SBORROW, "SBORROW", TYPE_BOOL, TYPE_INT) {
    opflags = PcodeOp::binary | PcodeOp::booloutput;
    addlflags = arithmetic_op;
...
TypeOpInt2Comp::TypeOpInt2Comp(TypeFactory* t) : TypeOpUnary(t, CPUI_INT_2COMP, "-", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::unary;
    addlflags = arithmetic_op | inherits_sign;
...
TypeOpIntNegate::TypeOpIntNegate(TypeFactory* t) : TypeOpUnary(t, CPUI_INT_NEGATE, "~", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::unary;
...
TypeOpIntXor::TypeOpIntXor(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_XOR, "^", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::binary | PcodeOp::commutative;
    addlflags = logical_op | inherits_sign;
...
Datatype* TypeOpIntXor::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                      int4 outslot) {
...
TypeOpIntAnd::TypeOpIntAnd(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_AND, "&", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::binary | PcodeOp::commutative;
    addlflags = logical_op | inherits_sign;
...
Datatype* TypeOpIntAnd::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                      int4 outslot) {
    if (!alttype->isEnumType()) {
        if (alttype->getMetatype() != TYPE_FLOAT)
            return (Datatype*)0;
...
TypeOpIntOr::TypeOpIntOr(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_OR, "|", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::binary | PcodeOp::commutative;
    addlflags = logical_op | inherits_sign;
...
Datatype* TypeOpIntOr::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                     int4 outslot) {
    if (!alttype->isEnumType())
        return (Datatype*)0; // Only propagate enums
    Datatype* newtype;
    if (invn->isSpacebase()) {
        AddrSpace* spc = tlst->getArch()->getDefaultDataSpace();
        newtype = tlst->getTypePointer(alttype->getSize(), tlst->getBase(1, TYPE_UNKNOWN), spc->getWordSize());
...
TypeOpIntLeft::TypeOpIntLeft(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_LEFT, "<<", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::binary;
...
TypeOpIntRight::TypeOpIntRight(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_RIGHT, ">>", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::binary;
    addlflags = inherits_sign | inherits_sign_zero | shift_op;
...
TypeOpIntSright::TypeOpIntSright(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_SRIGHT, ">>", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::binary;
    addlflags = inherits_sign | inherits_sign_zero | shift_op;
...
TypeOpIntMult::TypeOpIntMult(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_MULT, "*", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::binary | PcodeOp::commutative;
    addlflags = arithmetic_op | inherits_sign;
...
TypeOpIntDiv::TypeOpIntDiv(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_DIV, "/", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::binary;
    addlflags = arithmetic_op | inherits_sign;
...
TypeOpIntSdiv::TypeOpIntSdiv(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_SDIV, "/", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::binary;
    addlflags = arithmetic_op | inherits_sign;
...
TypeOpIntRem::TypeOpIntRem(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_REM, "%", TYPE_UINT, TYPE_UINT) {
    opflags = PcodeOp::binary;
...
TypeOpIntSrem::TypeOpIntSrem(TypeFactory* t) : TypeOpBinary(t, CPUI_INT_SREM, "%", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::binary;
    addlflags = arithmetic_op | inherits_sign | inherits_sign_zero;
...
TypeOpBoolNegate::TypeOpBoolNegate(TypeFactory* t) : TypeOpUnary(t, CPUI_BOOL_NEGATE, "!", TYPE_BOOL, TYPE_BOOL) {
    opflags = PcodeOp::unary | PcodeOp::booloutput;
    addlflags = logical_op;
    behave = new OpBehaviorBoolNegate();
}

TypeOpBoolXor::TypeOpBoolXor(TypeFactory* t) : TypeOpBinary(t, CPUI_BOOL_XOR, "^^", TYPE_BOOL, TYPE_BOOL) {
    opflags = PcodeOp::binary | PcodeOp::commutative | PcodeOp::booloutput;
    addlflags = logical_op;
    behave = new OpBehaviorBoolXor();
}

TypeOpBoolAnd::TypeOpBoolAnd(TypeFactory* t) : TypeOpBinary(t, CPUI_BOOL_AND, "&&", TYPE_BOOL, TYPE_BOOL) {
    opflags = PcodeOp::binary | PcodeOp::commutative | PcodeOp::booloutput;
    addlflags = logical_op;
    behave = new OpBehaviorBoolAnd();
}

TypeOpBoolOr::TypeOpBoolOr(TypeFactory* t) : TypeOpBinary(t, CPUI_BOOL_OR, "||", TYPE_BOOL, TYPE_BOOL) {
    opflags = PcodeOp::binary | PcodeOp::commutative | PcodeOp::booloutput;
    addlflags = logical_op;
...
TypeOpMulti::TypeOpMulti(TypeFactory* t)
    : TypeOp(t, CPUI_MULTIEQUAL, "?")
...
Datatype* TypeOpMulti::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                     int4 outslot) {
    if ((inslot != -1) && (outslot != -1)) {
        return (Datatype*)0; // Must propagate input <-> output
    }
...
TypeOpIndirect::TypeOpIndirect(TypeFactory* t)
    : TypeOp(t, CPUI_INDIRECT, "[]")
...
Datatype* TypeOpIndirect::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                        int4 outslot) {
...
TypeOpPiece::TypeOpPiece(TypeFactory* t) : TypeOpFunc(t, CPUI_PIECE, "CONCAT", TYPE_UNKNOWN, TYPE_UNKNOWN) {
    opflags = PcodeOp::binary;
...
Datatype* TypeOpPiece::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                     int4 outslot) {
...
TypeOpSubpiece::TypeOpSubpiece(TypeFactory* t) : TypeOpFunc(t, CPUI_SUBPIECE, "SUB", TYPE_UNKNOWN, TYPE_UNKNOWN) {
    opflags = PcodeOp::binary;
...
Datatype* TypeOpSubpiece::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                        int4 outslot) {
    ...
    if (inslot != 0 || outslot != -1)
        return (Datatype*)0; // Propagation must be from in0 to out
    int8 byteOff = computeByteOffsetForComposite(op);
...
TypeOpCast::TypeOpCast(TypeFactory* t)
    : TypeOp(t, CPUI_CAST, "(cast)")
...
TypeOpPtradd::TypeOpPtradd(TypeFactory* t)
    : TypeOp(t, CPUI_PTRADD, "+")
...
Datatype* TypeOpPtradd::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                      int4 outslot) {
...
TypeOpPtrsub::TypeOpPtrsub(TypeFactory* t)
    : TypeOp(t, CPUI_PTRSUB, "->")
...
Datatype* TypeOpPtrsub::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                      int4 outslot) {
    if ((inslot != -1) && (outslot != -1))
        return (Datatype*)0; // Must propagate input <-> output
    type_metatype metain = alttype->getMetatype();
...
TypeOpSegment::TypeOpSegment(TypeFactory* t)
    : TypeOp(t, CPUI_SEGMENTOP, "segmentop")
...
Datatype* TypeOpSegment::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                       int4 outslot) {
...
TypeOpCpoolref::TypeOpCpoolref(TypeFactory* t)
    : TypeOp(t, CPUI_CPOOLREF, "cpoolref")
...
TypeOpNew::TypeOpNew(TypeFactory* t)
    : TypeOp(t, CPUI_NEW, "new")
...
Datatype* TypeOpNew::propagateType(Datatype* alttype, PcodeOp* op, Varnode* invn, Varnode* outvn, int4 inslot,
                                   int4 outslot) {
    if ((inslot != 0) || (outslot != -1))
        return (Datatype*)0;
    Varnode* vn0 = op->getIn(0);
    if (!vn0->isWritten())
        return (Datatype*)0; // Don't propagate
    if (vn0->getDef()->code() != CPUI_CPOOLREF)
        return (Datatype*)0;
...
TypeOpInsert::TypeOpInsert(TypeFactory* t) : TypeOpFunc(t, CPUI_INSERT, "INSERT", TYPE_UNKNOWN, TYPE_INT) {
...
TypeOpZpull::TypeOpZpull(TypeFactory* t) : TypeOpFunc(t, CPUI_ZPULL, "ZPULL", TYPE_UINT, TYPE_INT) {
    opflags = PcodeOp::ternary;
...
TypeOpSpull::TypeOpSpull(TypeFactory* t) : TypeOpFunc(t, CPUI_SPULL, "SPULL", TYPE_INT, TYPE_INT) {
    opflags = PcodeOp::ternary;
...
TypeOpPopcount::TypeOpPopcount(TypeFactory* t) : TypeOpFunc(t, CPUI_POPCOUNT, "POPCOUNT", TYPE_INT, TYPE_UNKNOWN) {
    opflags = PcodeOp::unary;
    behave = new OpBehaviorPopcount();
}

TypeOpLzcount::TypeOpLzcount(TypeFactory* t) : TypeOpFunc(t, CPUI_LZCOUNT, "LZCOUNT", TYPE_INT, TYPE_UNKNOWN) {
    opflags = PcodeOp::unary;
