// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/unionresolve.cc.
...

{
    ...
    switch (trial.op->code()) {
            ...
            if (trial.inslot == 1) {
                ...
                if (meta == TYPE_CODE)
                    ...
                    if (!lastLevel && storePtr->getMetatype() == TYPE_PTR) {
                        Datatype* ptr = typegrp.getTypePointerStripArray(storePtr->getSize(), trial.fitType,
                                                                         ((TypePointer*)storePtr)->getWordSize());
        ...
        case CPUI_PTRADD:
            ...
            } else if (meta == TYPE_ARRAY || meta == TYPE_STRUCT || meta == TYPE_UNION || meta == TYPE_CODE ||
                       meta == TYPE_FLOAT)
                score = -5;
            else
                score = 1;
...
ScoreUnionFields::ScoreUnionFields(Funcdata& fd, Datatype* parentType, PcodeOp* op, int4 slot)
    ...
    if (wordSize == 0 || opc == CPUI_INT_ADD || opc == CPUI_PTRADD || opc == CPUI_PTRSUB || opc == CPUI_STORE ||
        opc == CPUI_LOAD)
    ...
    for (int4 i = 0; i < numFields; ++i) {
        ...
        if (wordSize != 0) {                            // If this is a pointer
            ...
            fieldType = typegrp.getTypePointerStripArray(parentType->getSize(), fieldType,
                                                         wordSize); // Create pointer to field type
        }
