// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/grammar.hh.
...
public:
    ArrayModifier(uint4 fl, int4 as) {
        flags = fl;
        arraysize = as;
    }
    virtual uint4 getType(void) const {
...
struct Enumerator {
    ...
    Enumerator(const string& nm) {
        constantassigned = false;
        enumconstant = nm;
    }
    Enumerator(const string& nm, uintb val) {
