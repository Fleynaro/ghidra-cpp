// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/subflow.hh.
...
public:
    RuleSubvarSext(const string& g) : Rule(g, 0, "subvar_sext") {
        isaggressive = false;
    } ///< Constructor
    virtual Rule* clone(const ActionGroupList& grouplist) const {
        if (!grouplist.contains(getGroup()))
...
class SplitDatatype {
    ...
    public:
        bool find(PcodeOp* op, Datatype* valueType,
                  ResolveCache& resolver);                       ///< Locate root pointer for underlying LOAD or STORE
...
class SubfloatFlow : public TransformManager {
    ...
    public:
        ...
        void incorporateInputSize(int4 sz) {
            maxPrecision = (maxPrecision < sz) ? sz : maxPrecision;
        }
    };
...
class LaneDivide : public TransformManager {
    ...
    void buildBinaryOp(OpCode opc, PcodeOp* op, TransformVar* in0Vars, TransformVar* in1Vars, TransformVar* outVars,
                       int4 numLanes);
