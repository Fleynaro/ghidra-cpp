...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/varnode.cc.
...

{
    ...
    if (f1 != f2)
        return ((f1 - 1) < (f2 - 1)); // -1 forces free varnodes to come last
...
{
    if (loc.getSpace() != op2loc.getSpace())
        return false;
    if (loc.getSpace()->getType() == IPTR_CONSTANT)
        return false;
    uintb a = loc.getOffset();
...

{
    ...
    if (isTypeLock() && (!override))
        return false; // Type is locked
    if ((type == ct) && (isTypeLock() == lock))
...
Varnode::Varnode(int4 s, const Address& m, Datatype* dt) : loc(m) { // Construct a varnode
    size = s;
...

{
    ...
    if (descend.empty())
        return (PcodeOp*)0; // No descendants
    ...
    if (iter != descend.end())
        return (PcodeOp*)0; // More than 1 descendant
    return op;
}
...

{
    ...
    if (!isWritten() || size <= 8)
        return false;
    if (size > 16)
        return false; // Currently only up to 128-bit values
    OpCode opc = def->code();
...

{
    ...
    if (size > sizeof(uintb)) {
        ...
        if (def->code() != CPUI_INT_ZEXT)
            return false;
        if (def->getIn(0)->getSize() > baseSize)
            return false;
        return true;
    }
...

{
    ...
    while ((vn->isWritten()) && (vn->getDef()->code() == CPUI_COPY)) {
        ...
        if (vn == op2)
            return true; // If we hit op2 then this and op2 must be the same
    }
    // Trace op2 to the source of copy chain
    while ((op2->isWritten()) && (op2->getDef()->code() == CPUI_COPY)) {
        op2 = op2->getDef()->getIn(0);
        if (vn == op2)
            return true; // If the source is the same then this and op2 are same
    }
    return false;
}
...

{
    ...
    if (opc == CPUI_SUBPIECE) {
        ...
        if (bigOp->code() != CPUI_MULTIEQUAL)
            return false;
        const PcodeOp* smallOp = vn->getDef();
        if (bigOp->getParent() != smallOp->getParent())
            return false;
...

{
    ...
    while (iter != loc_tree.end()) {
        ...
        if ((op != (PcodeOp*)0) && (op->getAddr() == pc)) {
            if ((uniq == ~((uintm)0)) || (op->getTime() == uniq))
                return vn;
        }
        ++iter;
    }
...

{
    ...
    if (iter != def_tree.end()) {
        ...
        if (vn->isInput() && (vn->getSpace() == loc.getSpace()) && (vn->getOffset() <= loc.getOffset()) &&
...
/// \return the beginning iterator
VarnodeLocSet::const_iterator VarnodeBank::beginLoc(int4 s, const Address& addr, uint4 fl) const {
    VarnodeLocSet::const_iterator iter;
...
/// \param fl is the property restriction
/// \return the ending iterator
VarnodeLocSet::const_iterator VarnodeBank::endLoc(int4 s, const Address& addr, uint4 fl) const {
    VarnodeLocSet::const_iterator iter;
    searchvn.loc = addr;
...
/// \return the beginning iterator
VarnodeLocSet::const_iterator VarnodeBank::beginLoc(int4 s, const Address& addr, const Address& pc, uintm uniq) const
...
/// \param uniq is the sequence number of the PcodeOp or -1 for now sequence number restriction
/// \return the ending iterator
VarnodeLocSet::const_iterator VarnodeBank::endLoc(int4 s, const Address& addr, const Address& pc, uintm uniq) const
