...
public:
    ...
    LoadedPeImage() : storage_(std::make_unique<Storage>()) {}

    /// Transfers ownership of all parsed metadata and image bytes.
    LoadedPeImage(LoadedPeImage&& other) noexcept : storage_(std::move(other.storage_)) {
        if (!other.storage_) {
    ...
    [[nodiscard]] std::expected<FileOffset, AddressError> rva_to_file_offset(Rva rva) const {
        ...
        for (const auto& section : storage_->sections) {
            if (section.file_backed_size != 0 && rva >= section.aligned_virtual_address &&
                rva - section.aligned_virtual_address < section.loaded_size) {
                const auto relative = rva - section.aligned_virtual_address;
                if (relative >= section.file_backed_size) {
                    return std::unexpected(AddressError{AddressErrorCode::not_file_backed, rva, 1,
                                                        "RVA is in a virtual-only section tail"});
                }
    ...
    [[nodiscard]] std::expected<Rva, AddressError> file_offset_to_rva(FileOffset offset) const {
        ...
        for (const auto& section : storage_->sections) {
            if (section.file_backed_size != 0 && offset >= section.raw_offset &&
                ...
                if (relative > std::numeric_limits<Rva>::max() - section.aligned_virtual_address) {
                    return std::unexpected(AddressError{AddressErrorCode::overflow, offset, 1,
                                                        "File offset translation overflows an RVA"});
                }
                return section.aligned_virtual_address + static_cast<Rva>(relative);
            }
        }
        ...
        for (const auto& region : storage_->memory_regions) {
            ...
            if (address >= region.start && delta <= region.size && size <= region.size - delta) {
                ...
                return std::vector<Byte>(storage_->mapped_image.begin() + image_offset,
                                         storage_->mapped_image.begin() + image_offset +
    ...
    [[nodiscard]] std::expected<std::vector<Byte>, MemoryError> read_resource_payload(const ResourceLeaf& leaf) const {
        const auto same_identifier = [](const ResourceIdentifier& left, const ResourceIdentifier& right) {
            return left.named == right.named && left.id == right.id && left.name == right.name;
        };
        const bool is_parsed_leaf =
            storage_->resources &&
            std::any_of(storage_->resources->leaves.begin(), storage_->resources->leaves.end(),
                        [&](const ResourceLeaf& parsed) {
                            return same_identifier(parsed.type, leaf.type) && same_identifier(parsed.name, leaf.name) &&
                                   same_identifier(parsed.language, leaf.language) &&
                                   parsed.data_rva == leaf.data_rva && parsed.size == leaf.size &&
                                   parsed.code_page == leaf.code_page && parsed.reserved == leaf.reserved &&
                                   parsed.file_offset == leaf.file_offset;
                        });
        if (!is_parsed_leaf) {
            return std::unexpected(MemoryError{MemoryErrorCode::outside_image, leaf.data_rva, leaf.size,
                                               "Resource leaf does not belong to the parsed resource tree"});
        }
        if (leaf.size == 0) {
...
/// Forward-declares the exported loader used by LoadedPeImage's friend declaration.
    ...
    [[nodiscard]] std::expected<LoadedPeImage, ParseError> run() {
        try {
            ...
            return parse_failure<LoadedPeImage>(ParseErrorCode::limit_exceeded, 0,
                                                "Unable to retain the PE input bytes");
        }
...
private:
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_headers() {
        ...
        if (storage_.dos.e_magic != 0x5a4d) {
            return parse_failure<void>(ParseErrorCode::invalid_dos_signature, 0,
                                       "The DOS header does not begin with MZ");
        }
        ...
        if (nt_offset_ < 64 || nt_offset_ > reader_.bytes().size() - 4) {
            return parse_failure<void>(ParseErrorCode::invalid_dos_offset, 0x3c,
                                       "DOS e_lfanew points outside the input");
        }
        ...
        auto optional_range =
            reader_.require(optional_offset, storage_.coff.size_of_optional_header, "optional header");
        if (!optional_range) {
            return std::unexpected(optional_range.error());
        }
        auto optional_u16 = [&](std::uint64_t relative,
                                std::string_view name) -> std::expected<std::uint16_t, ParseError> {
            if (relative > storage_.coff.size_of_optional_header ||
                2 > storage_.coff.size_of_optional_header - relative) {
        ...
        auto optional_u32 = [&](std::uint64_t relative,
                                std::string_view name) -> std::expected<std::uint32_t, ParseError> {
            if (relative > storage_.coff.size_of_optional_header ||
                4 > storage_.coff.size_of_optional_header - relative) {
        ...
        auto optional_u64 = [&](std::uint64_t relative,
                                std::string_view name) -> std::expected<std::uint64_t, ParseError> {
            if (relative > storage_.coff.size_of_optional_header ||
                8 > storage_.coff.size_of_optional_header - relative) {
        ...
        const auto alignment_power_of_two = [](std::uint32_t value) {
        ...
        if (storage_.optional.size_of_image == 0 || storage_.optional.size_of_image > options_.maximum_image_size ||
            ...
            storage_.optional.size_of_headers == 0) {
        ...
        for (std::size_t index = 0; index < storage_.coff.number_of_sections; ++index) {
            ...
            const auto aligned_virtual_address = align_up_checked(section.virtual_address, section_alignment_value);
            const auto aligned_virtual_size = align_up_checked(virtual_extent_source, section_alignment_value);
            if (!aligned_virtual_address || !aligned_virtual_size ||
                *aligned_virtual_address > std::numeric_limits<std::uint32_t>::max() ||
            ...
            section.aligned_virtual_address = static_cast<Rva>(*aligned_virtual_address);
            ...
            if (section.virtual_extent != 0 &&
                (section.aligned_virtual_address > storage_.optional.size_of_image ||
                 section.virtual_extent > storage_.optional.size_of_image - section.aligned_virtual_address)) {
            ...
            if (section.loaded_size != 0 &&
                (section.aligned_virtual_address > storage_.optional.size_of_image ||
                 section.loaded_size > storage_.optional.size_of_image - section.aligned_virtual_address)) {
                return parse_failure<void>(ParseErrorCode::invalid_section_range, section_offset + 16,
                                           "Section memory block is outside SizeOfImage");
            }
        ...
        for (std::size_t left = 0; left < storage_.sections.size(); ++left) {
            ...
            if (first.virtual_extent != 0 && first.aligned_virtual_address < storage_.optional.size_of_headers &&
                first.virtual_extent > storage_.optional.size_of_headers - first.aligned_virtual_address) {
            ...
            for (std::size_t right = left + 1; right < storage_.sections.size(); ++right) {
                ...
                const auto first_virtual_end =
                    static_cast<std::uint64_t>(first.aligned_virtual_address) + first.virtual_extent;
                const auto second_virtual_end =
                    static_cast<std::uint64_t>(second.aligned_virtual_address) + second.virtual_extent;
                const bool virtual_overlap =
                    first.virtual_extent != 0 && second.virtual_extent != 0 &&
                    static_cast<std::uint64_t>(first.aligned_virtual_address) < second_virtual_end &&
                    static_cast<std::uint64_t>(second.aligned_virtual_address) < first_virtual_end;
                ...
                const bool raw_overlap = first.raw_size != 0 && second.raw_size != 0 &&
                                         first.raw_offset < second_raw_end && second.raw_offset < first_raw_end;
    ...
    [[nodiscard]] std::expected<void, ParseError> map_image() {
        ...
        storage_.memory_regions.push_back(MemoryRegion{"Headers", storage_.optional.image_base,
                                                       storage_.optional.size_of_headers, true, false, false, true,
                                                       true, std::nullopt});
        for (const auto& section : storage_.sections) {
            if (section.loaded_size == 0) {
                continue;
            }
            auto destination = storage_.mapped_image.begin() + section.aligned_virtual_address;
            ...
            storage_.memory_regions.push_back(MemoryRegion{
                section.name, storage_.optional.image_base + section.aligned_virtual_address, section.loaded_size,
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_directories() {
        const std::array<std::pair<DirectoryIndex, std::expected<void, ParseError> (Parser::*)(const DataDirectory&)>,
                         15>
            parsers = {{
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_architecture(const DataDirectory& directory_value) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_global_pointer(const DataDirectory& directory_value) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_exports(const DataDirectory& directory_value) {
        ...
        for (std::uint32_t index = 0; index < *name_count; ++index) {
            ...
            if (name_delta > std::numeric_limits<Rva>::max() ||
                *names_rva > std::numeric_limits<Rva>::max() - name_delta) {
            ...
            if (ordinal_delta > std::numeric_limits<Rva>::max() ||
                *ordinals_rva > std::numeric_limits<Rva>::max() - ordinal_delta) {
                return parse_failure<void>(ParseErrorCode::invalid_export, *ordinals_rva,
                                           "Export ordinal table overflows");
            }
            ...
            if (*ordinal >= *function_count) {
                return parse_failure<void>(ParseErrorCode::invalid_export,
                                           *ordinals_rva + static_cast<Rva>(ordinal_delta),
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_imports(const DataDirectory& directory_value) {
        constexpr std::uint32_t descriptor_size = 20;
        if (directory_value.rva > std::numeric_limits<Rva>::max() - directory_value.size) {
            return parse_failure<void>(ParseErrorCode::invalid_import, directory_value.rva,
                                       "Import descriptor directory overflows the RVA space");
        }
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_thunks(std::vector<ImportedSymbol>& symbols, Rva int_rva,
        ...
        for (std::uint32_t index = 0; index < maximum_entries; ++index) {
            ...
            const auto iat_value = storage_.optional.pe32_plus
                                       ? mapped_u64(iat_slot)
                                       : mapped_u32(iat_slot).transform(
                                             [](std::uint32_t value) { return static_cast<std::uint64_t>(value); });
            if (!iat_value) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_iat(const DataDirectory& directory_value) {
        ...
        for (std::uint32_t index = 0; index < slot_count; ++index) {
            ...
            if (delta > std::numeric_limits<Rva>::max() ||
                directory_value.rva > std::numeric_limits<Rva>::max() - delta) {
                return parse_failure<void>(ParseErrorCode::invalid_import, directory_value.rva,
                                           "Import address table slot overflows the RVA space");
            }
            const auto slot_rva = directory_value.rva + static_cast<Rva>(delta);
            const auto value = storage_.optional.pe32_plus ? mapped_u64(slot_rva)
                                                           : mapped_u32(slot_rva).transform([](std::uint32_t item) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_relocations(const DataDirectory& directory_value) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_debug(const DataDirectory& directory_value) {
        ...
        for (std::uint32_t offset = 0; offset < directory_value.size; offset += entry_size) {
            ...
            if (!characteristics || !timestamp || !major || !minor || !type || !size || !data_rva || !raw_pointer) {
                return parse_failure<void>(ParseErrorCode::invalid_debug_data, rva,
                                           "Debug directory entry is unmapped");
            }
            ...
            if (*size != 0) {
                ...
                if (entry.type == DebugType::code_view) {
                    ...
                    auto signature =
                        reader_.span(payload_offset, std::min<std::uint32_t>(*size, 4), "CodeView signature");
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_exceptions(const DataDirectory& directory_value) {
        ...
        for (std::uint32_t index = 0; index < count; ++index) {
            const auto delta = static_cast<std::uint64_t>(index) * record_size;
            if (delta > std::numeric_limits<Rva>::max() ||
                directory_value.rva > std::numeric_limits<Rva>::max() - delta) {
                return parse_failure<void>(ParseErrorCode::invalid_directory, directory_value.rva,
                                           "Exception record RVA overflows");
            }
            const auto rva = directory_value.rva + static_cast<Rva>(delta);
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_tls(const DataDirectory& directory_value) {
        ...
        if (directory_value.rva > std::numeric_limits<Rva>::max() - directory_value.size) {
            return parse_failure<void>(ParseErrorCode::invalid_directory, directory_value.rva,
                                       "TLS directory overflows the RVA space");
        }
        ...
        if (*callbacks != 0) {
            ...
            for (std::uint32_t callback_index = 0; callback_index < 4096; ++callback_index) {
                const auto delta = static_cast<std::uint64_t>(callback_index) * (storage_.optional.pe32_plus ? 8U : 4U);
                if (delta > std::numeric_limits<Rva>::max() ||
                    *callback_rva > std::numeric_limits<Rva>::max() - delta) {
                    const auto pointer_rva = *callback_rva;
                    return parse_failure<void>(ParseErrorCode::invalid_directory, pointer_rva,
                                               "TLS callback array overflows");
                }
                const auto pointer_rva = *callback_rva + static_cast<Rva>(delta);
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_load_config(const DataDirectory& directory_value) {
        ...
        auto u16 = [&](std::uint32_t offset) -> std::optional<std::uint16_t> {
            return offset <= available && 2 <= available - offset ? mapped_u16(directory_value.rva + offset)
                                                                  : std::nullopt;
        };
        auto u32 = [&](std::uint32_t offset) -> std::optional<std::uint32_t> {
            return offset <= available && 4 <= available - offset ? mapped_u32(directory_value.rva + offset)
                                                                  : std::nullopt;
        };
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_resources(const DataDirectory& directory_value) {
        ...
        if (directory_value.rva > std::numeric_limits<Rva>::max() - directory_value.size) {
            return parse_failure<void>(ParseErrorCode::invalid_resource, directory_value.rva,
                                       "Resource directory overflows the RVA space");
        }
        ...
        walk = [&](std::uint32_t relative, const Path& path, std::uint32_t depth) -> std::expected<void, ParseError> {
            ...
            if (!named || !ids ||
                static_cast<std::uint32_t>(*named) + *ids > (directory_value.size - relative - 16) / 8) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_security(const DataDirectory& directory_value) {
        std::uint32_t offset = 0;
        while (offset < directory_value.size) {
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_bound_imports(const DataDirectory& directory_value) {
        std::uint32_t offset = 0;
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_delay_imports(const DataDirectory& directory_value) {
        constexpr std::uint32_t descriptor_size = 32;
        if (directory_value.rva > std::numeric_limits<Rva>::max() - directory_value.size) {
            return parse_failure<void>(ParseErrorCode::invalid_directory, directory_value.rva,
                                       "Delay-import directory overflows the RVA space");
        }
        bool terminated = false;
        ...
        for (std::uint32_t offset = 0; directory_value.size - offset >= descriptor_size; offset += descriptor_size) {
            ...
            if (!attributes || !name || !module_handle || !iat || !int_table || !bound_iat || !unload_iat ||
                !timestamp) {
                return parse_failure<void>(ParseErrorCode::invalid_directory, rva,
                                           "Delay-import descriptor is unmapped");
            }
            if (*attributes == 0 && *name == 0 && *module_handle == 0 && *iat == 0 && *int_table == 0 &&
                *bound_iat == 0 && *unload_iat == 0 && *timestamp == 0) {
                terminated = true;
                break;
            }
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_clr(const DataDirectory& directory_value) {
        // Related source:
        // ../../../Ghidra/Features/Base/src/main/java/ghidra/app/util/bin/format/pe/ImageCor20Header.java The Java
        // model exposes the fixed 72-byte IMAGE_COR20_HEADER, while the cb field is the authoritative bound for newer
        // or malformed headers. Never decode fields past either bound.
        if (directory_value.size < 4) {
            return parse_failure<void>(ParseErrorCode::invalid_directory, directory_value.rva,
                                       "CLR directory lacks its cb field");
        }
        auto size = mapped_u32(directory_value.rva);
        if (!size || *size < 72 || directory_value.size < 72 || *size > directory_value.size) {
            return parse_failure<void>(ParseErrorCode::invalid_directory, directory_value.rva,
    ...
    [[nodiscard]] std::expected<void, ParseError> parse_coff_symbols() {
        ...
        const FileOffset string_table_offset =
            storage_.coff.pointer_to_symbol_table +
            static_cast<std::uint64_t>(storage_.coff.number_of_symbols) * symbol_size;
        ...
        auto read_symbol_name = [&](FileOffset offset) -> std::expected<std::string, ParseError> {
            ...
            return read_file_string(reader_, string_table_offset + *string_offset - 4,
                                    *string_table_size - *string_offset, "COFF symbol name");
        };
        ...
        const bool mapped = rva < storage_.optional.size_of_headers ||
                            std::any_of(
                                ...
                                [rva](const Section& section) {
                                    return section.loaded_size != 0 && rva >= section.aligned_virtual_address &&
                                           rva - section.aligned_virtual_address < section.loaded_size;
                                });
    ...
    [[nodiscard]] std::expected<std::string, ParseError> mapped_string(Rva rva, std::uint64_t maximum,
        ...
        return parse_failure<std::string>(ParseErrorCode::truncated, rva,
                                          std::string("Unterminated ") + std::string(what));
    }
...
/// Provides the public byte-span and file-based PE loading entry points.
export class PeLoader {
public:
    ...
    [[nodiscard]] static std::expected<LoadedPeImage, ParseError> load(std::span<const Byte> bytes,
        ...
        try {
            ...
            return parse_failure<LoadedPeImage>(ParseErrorCode::limit_exceeded, 0,
                                                "PE parsing exceeded available memory");
        }
    }
    ...
    [[nodiscard]] static std::expected<LoadedPeImage, ParseError> load_file(const std::filesystem::path& path,
        ...
        if (!input) {
            return parse_failure<LoadedPeImage>(ParseErrorCode::io_failure, 0,
                                                "Unable to open PE file: " + path.string());
        }
        ...
        if (size > std::numeric_limits<std::size_t>::max()) {
            return parse_failure<LoadedPeImage>(ParseErrorCode::limit_exceeded, 0,
                                                "PE file is too large for this process");
        }
