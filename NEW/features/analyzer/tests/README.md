...
Delta reports are copied into typed C++ constants in the test module; the
bare `x86-64.sla` name is resolved by the shared Sleigh runtime specification
directory, so the test does not need an SLA-specific compile definition.
