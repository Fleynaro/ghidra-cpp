...
## Test Scenario
Two exported code symbols and one exported data marker are imported without pre-disassembly; the script compares instruction and function presence before and after analysis.
...
## Expected Results
Code export locations may gain instructions/functions, while the data marker must not be disassembled as code.
...
- [`run_ghidra.py`](run_ghidra.py) deliberately avoids pre-disassembly, enables Disassemble Entry Points, and compares selected symbol state.
...
Validation completed with MSVC 2022 x64 compilation/linking, project save/reopen through `project.openProgram(...)`, before/after target-only analysis, and report inspection. The generated report contains the two exported code symbols; the data marker is not treated as an entry point.
