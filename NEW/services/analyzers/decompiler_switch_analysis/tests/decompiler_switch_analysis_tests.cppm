...
TEST(DecompilerSwitchAnalysis, ParsesControlFlowAddresses) {
    const auto addresses =
        recode::analyzer::decompiler_control_flow_addresses("block 0x140001019 -> 0x14000101b; block 0x140001021");
...
TEST(DecompilerSwitchAnalysis, RecoversFixtureSwitchTargets) {
    auto context = recode::analyzer::tests::load_fixture("decompiler_switch_analysis");
    ...
    recode::analyzer::DecompilerSwitchAnalysisAnalyzer analyzer;
    ...
    const std::set<recode::analyzer::Address> expected{0x14000101bULL, 0x140001021ULL, 0x140001027ULL, 0x14000102dULL,
                                                       0x140001033ULL, 0x140001039ULL, 0x14000103fULL, 0x140001045ULL};
    std::set<recode::analyzer::Address> actual;
    for (const auto& reference : context.references())
        if (reference.source == 0x140001019ULL && reference.kind == recode::analyzer::ReferenceKind::computed_jump)
