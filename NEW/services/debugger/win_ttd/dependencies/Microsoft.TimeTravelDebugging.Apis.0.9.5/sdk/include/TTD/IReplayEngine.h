...
struct Module {
    ...
    friend constexpr auto AsComparisonTuple(Module const& m) {
        return std::tie(m.Address, m.Timestamp, m.Checksum, m.Size);
    }

    friend constexpr bool operator==(Module const& a, Module const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(Module const& a, Module const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
    }
...
inline size_t ModuleToString(_In_ Module const& module, _Out_writes_z_(size) wchar_t* const pBuff,
                             _In_ size_t const size) {
    return _snwprintf_s(pBuff, size, _TRUNCATE, L"Module %ls at address 0X%llX with size %llu", module.pName,
...
struct ModuleInstance {
    ...
    friend constexpr auto AsComparisonTuple(ModuleInstance const& m) {
        return std::tie(m.LoadTime, m.pModule);
    }

    friend constexpr bool operator==(ModuleInstance const& a, ModuleInstance const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(ModuleInstance const& a, ModuleInstance const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
...
[[deprecated]] inline char const* GetDebugEventName(_In_ DebugEvent const event) {
    switch (event) {
        case DebugEvent::Read:
            return "Read";
        case DebugEvent::Write:
            return "Write";
        case DebugEvent::SingleInstruction:
            return "SingleInstruction";
        case DebugEvent::InstructionBytes:
            return "InstructionBytes";
        case DebugEvent::GeneralExecutionState:
            return "GeneralExecutionState";
        case DebugEvent::Message:
            return "Message";
        case DebugEvent::ValidationDerailment:
            return "ValidationDerailment";
        case DebugEvent::MissingDataDerailment:
            return "MissingDataDerailment";
        case DebugEvent::IncorrectPacketDerailment:
            return "IncorrectPacketDerailment";
        case DebugEvent::CountedPacketOutOfOrder:
            return "CountedPacketOutOfOrder";
        case DebugEvent::EmulatorDerailment:
            return "EmulatorDerailment";
        default:
            return "<Unknown>";
    }
}
...
using ClientLoggingCallback
    [[deprecated]] = void(__stdcall*)(_In_ LoggingInformation const&, _In_ DebugEvent, _In_opt_z_ wchar_t const*);
#pragma warning(pop)
...
inline char const* GetRecordingTypeName(_In_ RecordingType const type) {
    switch (type) {
        case RecordingType::Invalid:
            return "Invalid";
        case RecordingType::Full:
            return "Full";
        case RecordingType::Selective:
            return "Selective";
...
constexpr StepCount ToStepCount(_In_ InstructionCount const count) {
    return static_cast<StepCount>(static_cast<uint64_t>(count));
}

constexpr StepCount operator+(_In_ StepCount a, _In_ uint64_t b) {
    return static_cast<StepCount>(static_cast<uint64_t>(a) + b);
}
constexpr StepCount operator-(_In_ StepCount a, _In_ uint64_t b) {
    return static_cast<StepCount>(static_cast<uint64_t>(a) - b);
}
...
constexpr uint64_t operator%(_In_ StepCount a, _In_ uint64_t b) {
    return static_cast<uint64_t>(a) % b;
}
constexpr StepCount operator+(_In_ StepCount a, _In_ StepCount b) {
...
constexpr bool operator==(_In_ StepCount a, _In_ uint64_t b) {
    return static_cast<uint64_t>(a) == b;
}
constexpr bool operator!=(_In_ StepCount a, _In_ uint64_t b) {
    return static_cast<uint64_t>(a) != b;
...
inline StepCount& operator+=(_Inout_ StepCount& a, _In_ int64_t b) {
    return a = static_cast<StepCount>(static_cast<uint64_t>(a) + b);
}
inline StepCount& operator-=(_Inout_ StepCount& a, _In_ int64_t b) {
    return a = static_cast<StepCount>(static_cast<uint64_t>(a) - b);
}
inline StepCount& operator+=(_Inout_ StepCount& a, _In_ StepCount b) {
    return a += static_cast<uint64_t>(b);
}
inline StepCount& operator-=(_Inout_ StepCount& a, _In_ StepCount b) {
    return a -= static_cast<uint64_t>(b);
...
constexpr StepCount operator+(_In_ StepCount a, _In_ InstructionCount b) {
    return a + static_cast<uint64_t>(b);
}
constexpr StepCount operator-(_In_ StepCount a, _In_ InstructionCount b) {
    return a - static_cast<uint64_t>(b);
}
constexpr StepCount& operator+=(_Inout_ StepCount& a, _In_ InstructionCount b) {
...
constexpr StepCount operator+(_In_ InstructionCount a, _In_ StepCount b) {
    return b + static_cast<uint64_t>(a);
}

constexpr bool operator==(_In_ StepCount a, _In_ InstructionCount b) {
...
constexpr bool operator==(_In_ InstructionCount a, _In_ StepCount b) {
    return static_cast<uint64_t>(a) == static_cast<uint64_t>(b);
}
constexpr bool operator!=(_In_ InstructionCount a, _In_ StepCount b) {
    return static_cast<uint64_t>(a) != static_cast<uint64_t>(b);
...
struct Position {
    ...
    constexpr bool IsValid() const {
        return Sequence != SequenceId::Invalid;
    }

    inline Position& operator+=(int64_t increment) {
        Steps += increment;
        return *this;
    }
    inline Position& operator+=(StepCount increment) {
        Steps += increment;
        return *this;
    }
    ...
    friend constexpr auto AsComparisonTuple(Position const& p) {
        return std::tie(p.Sequence, p.Steps);
    }

    friend constexpr bool operator==(Position const& a, Position const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(Position const& a, Position const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
...
constexpr Position operator+(_In_ Position const& position, StepCount increment) {
    return position + static_cast<uint64_t>(increment);
}
constexpr Position operator-(_In_ Position const& position, StepCount increment) {
    return position + static_cast<uint64_t>(increment);
}
...
struct PositionRange {
    ...
    friend constexpr auto AsComparisonTuple(PositionRange const& r) {
        return std::tie(r.Min, r.Max);
    }

    friend constexpr bool operator==(PositionRange const& a, PositionRange const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(PositionRange const& a, PositionRange const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
...
inline size_t PositionRangeToString(_In_ PositionRange const& range, _Out_writes_z_(size) wchar_t* const pBuff,
                                    _In_ size_t const size) {
    return _snwprintf_s(pBuff, size, _TRUNCATE, L"[%llX:%llX, %llX:%llX]", range.Min.Sequence, range.Min.Steps,
                        range.Max.Sequence, range.Max.Steps);
}
...
constexpr char const* GetGapKindName(_In_ GapKind const kind) {
    switch (kind) {
        case GapKind::NoGap:
            return "NoGap";
        case GapKind::ContextSwitch:
            return "ContextSwitch";
        case GapKind::Unrecorded:
            return "Unrecorded";
        case GapKind::Large:
            return "Large";
        default:
            return "<Unknown GapKind>";
    }
}
...
enum class GapKindMask : uint32_t {
    ...
    None = 0,
    All = NoGap | ContextSwitch | Unrecorded | Large,
};
...
enum GapEventType : uint8_t {
    ...
    PotentialAtomicCollision, // just finished an atomic operation. TODO: We currently don't use this, but we should, to
                              // provide faithful ordering of atomics.
    ...
    PauseEmulation, // paused emulation, sort of like a syscall but without accruing an instruction. For instance, to
                    // skip a function.
...
constexpr char const* GetGapEventTypeName(_In_ GapEventType const kind) {
    switch (kind) {
        case GapEventType::SyntheticSequence:
            return "SyntheticSequence";
        case GapEventType::CodeCacheFlush:
            return "CodeCacheFlush";
        case GapEventType::PreAtomicOperation:
            return "PreAtomicOperation";
        case GapEventType::PotentialAtomicCollision:
            return "PotentialAtomicCollision";
        case GapEventType::EtwEvent:
            return "EtwEvent";
        case GapEventType::DebugBreak:
            return "DebugBreak";
        case GapEventType::FastFail:
            return "FastFail";
        case GapEventType::KernelCall:
            return "KernelCall";
        case GapEventType::SyntheticFallback:
            return "SyntheticFallback";
        case GapEventType::ExceptionDispatch:
            return "ExceptionDispatch";
        case GapEventType::UnknownInstruction:
            return "UnknownInstruction";
        case GapEventType::ThreadSuspended:
            return "ThreadSuspended";
        case GapEventType::SListRollback:
            return "SListRollback";
        case GapEventType::SyncPoint:
            return "SyncPoint";
        case GapEventType::PauseEmulation:
            return "PauseEmulation";
        case GapEventType::StopEmulation:
            return "StopEmulation";
        case GapEventType::Throttled:
            return "Throttled";
        default:
            return "<Unknown GapEventType>";
    }
}
...
constexpr GapEventMask ConvertGapEventTypeToMask(_In_ GapEventType const kind) {
    DBG_ASSERT_MSG(static_cast<uint8_t>(kind) < sizeof(GapEventMask) * 8,
                   "Ensure that all GapEventTypes fit in GapEventMask");
...
enum class GapEventMask : uint32_t {
    ...
    None = 0,
    All = SyntheticSequence | CodeCacheFlush | PreAtomicOperation | PotentialAtomicCollision | EtwEvent | DebugBreak |
        FastFail | KernelCall | SyntheticFallback | ExceptionDispatch | UnknownInstruction | ThreadSuspended |
...
enum class EventType : uint8_t {
    ...
    Exception, // An exception matching the ExceptionMask was encountered. TODO: Eliminate this, the caller can use the
               // event list and then set a limit Position instead.
...
constexpr char const* GetEventTypeName(_In_ EventType const type) {
    switch (type) {
        case EventType::MemoryWatchpoint:
            return "MemoryWatchpoint";
        case EventType::PositionWatchpoint:
            return "PositionWatchpoint";
...
constexpr char const* GetDataAccessTypeName(_In_ DataAccessType const type) {
    switch (type) {
        ...
        case DataAccessType::Write:
            return "Write";
        case DataAccessType::Execute:
            return "Execute";
        case DataAccessType::CodeFetch:
            return "CodeFetch";
        case DataAccessType::Overwrite:
            return "Overwrite";
        case DataAccessType::DataMismatch:
            return "DataMismatch";
        case DataAccessType::NewData:
            return "NewData";
        case DataAccessType::RedundantData:
            return "RedundantData";
        default:
            return "<Unknown DataAccessType>";
    }
}
...
constexpr wchar_t const* GetDataAccessTypeNameW(_In_ DataAccessType const type) {
    switch (type) {
        ...
        case DataAccessType::Overwrite:
            return L"Overwrite";
        case DataAccessType::DataMismatch:
            return L"DataMismatch";
        case DataAccessType::NewData:
            return L"NewData";
        case DataAccessType::RedundantData:
            return L"RedundantData";
        default:
            return L"<Unknown DataAccessType>";
    }
}
...
constexpr DataAccessMask ConvertDataAccessTypeToMask(_In_ DataAccessType const type) {
    DBG_ASSERT_MSG(static_cast<uint8_t>(type) < sizeof(DataAccessMask) * 8,
                   "Ensure that all DataAccessTypes fit in DataAccessMask");
...
struct MemoryWatchpointData {
    ...
    friend constexpr auto AsComparisonTuple(MemoryWatchpointData const& w) {
        return std::tie(w.Address, w.Size, w.AccessMask, w.ThreadId);
    }

    friend constexpr bool operator==(MemoryWatchpointData const& a, MemoryWatchpointData const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(MemoryWatchpointData const& a, MemoryWatchpointData const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
...
struct PositionWatchpointData {
    ...
    friend constexpr auto AsComparisonTuple(PositionWatchpointData const& w) {
        return std::tie(w.Positions, w.ThreadId);
    }

    friend constexpr bool operator==(PositionWatchpointData const& a, PositionWatchpointData const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(PositionWatchpointData const& a, PositionWatchpointData const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
...
constexpr wchar_t const* GetExceptionTypeName(_In_ ExceptionType const type) {
    switch (type) {
        case ExceptionType::Hardware:
            return L"Hardware";
        case ExceptionType::Software:
            return L"Software";
        case ExceptionType::CPlusPlus:
            return L"CPlusPlus";
        case ExceptionType::DebugPrint:
            return L"DebugPrint";
        default:
            return L"<Unknown ExceptionType>";
    }
}
...
constexpr ExceptionMask ConvertExceptionTypeToMask(_In_ ExceptionType const type) {
    DBG_ASSERT_MSG(static_cast<uint8_t>(type) < sizeof(ExceptionMask) * 8,
                   "Ensure that all ExceptionTypes fit in ExceptionMask");
...
enum class ExceptionMask : uint32_t {
    ...
    None = 0,
    All = Hardware | Software | CPlusPlus | DebugPrint,
};
...
enum ReplayFlags : uint32_t {
    ...
    None = 0,
    Default = 0,
    All = ReplayOnlyCurrentThread | ReplayAllSegmentsWithoutFiltering | ReplaySegmentsSequentially,
};
...
struct ThreadInfo {
    ...
    /*implicit*/ constexpr operator UniqueThreadId() const {
        return UniqueId;
    }
...
struct ActiveThreadInfo {
    ...
    // TODO: Turn this into a position range, right now LastValidPosition is exclusive, not inclusive, so it's not just
    // a naming change
    Position CurrentPosition;
    Position LastValidPosition;

    constexpr ThreadInfo const* operator->() const {
        return pThread;
    }

    friend constexpr auto AsComparisonTuple(ActiveThreadInfo const& t) {
        return std::tie(t->UniqueId, t.CurrentPosition);
    }
...
inline size_t ExceptionEventToString(_In_ ExceptionEvent const& exception, _Out_writes_z_(size) wchar_t* const pBuff,
                                     _In_ size_t const size) {
    return _snwprintf_s(pBuff, size, _TRUNCATE, L"Exception 0x%08X of type %s at PC: 0X%llX", exception.Code,
                        GetExceptionTypeName(exception.Type), exception.ProgramCounter);
}
...
struct ThreadCreatedEvent {
    ...
    ThreadCreatedEvent(_In_ Position const& position, _In_ ThreadInfo const* pThreadInfo)
        : Position(position), pThreadInfo(pThreadInfo) {}

    Position Position;
    ThreadInfo const* pThreadInfo;
};

inline size_t ThreadCreatedEventToString(_In_ ThreadCreatedEvent const& event, _Out_writes_z_(size) wchar_t* pBuff,
                                         _In_ size_t size) {
    return _snwprintf_s(pBuff, size, _TRUNCATE, L"Thread UID: %3u TID: 0x%04X created at %llX:%llX",
                        event.pThreadInfo->UniqueId, event.pThreadInfo->Id, event.Position.Sequence,
...
inline size_t ThreadTerminatedEventToString(_In_ ThreadTerminatedEvent const& event,
                                            _Out_writes_z_(size) wchar_t* pBuff, _In_ size_t size) {
    return _snwprintf_s(pBuff, size, _TRUNCATE, L"Thread UID: %3u TID: 0x%04X terminated at %llX:%llX",
                        event.pThreadInfo->UniqueId, event.pThreadInfo->Id, event.Position.Sequence,
                        event.Position.Steps);
}

inline wchar_t const* GetModuleBaseName(_In_z_ wchar_t const* const pModuleFullName,
                                        _In_ size_t const pModuleNameLength) noexcept {
    ...
    return std::find_first_of(std::make_reverse_iterator(last), std::make_reverse_iterator(first),
...
inline size_t ModuleLoadedEventToString(_In_ ModuleLoadedEvent const& moduleEvent,
                                        _Out_writes_z_(size) wchar_t* const pBuff, _In_ size_t const size) {
    // Note that written doesn't contain the count of the null terminator
    auto const written = _snwprintf_s(pBuff, size, _TRUNCATE, L"Module %ls Loaded at position: ",
                                      GetModuleBaseName(moduleEvent.pModule->pName, moduleEvent.pModule->NameLength));
    if (written <= 0) {
...
inline size_t ModuleUnloadedEventToString(_In_ ModuleUnloadedEvent const& moduleEvent,
                                          _Out_writes_z_(size) wchar_t* const pBuff, _In_ size_t const size) {
    // Note that written doesn't contain the count of the null terminator
    auto const written = _snwprintf_s(pBuff, size, _TRUNCATE, L"Module %ls Unloaded at position: ",
                                      GetModuleBaseName(moduleEvent.pModule->pName, moduleEvent.pModule->NameLength));
    if (written <= 0) {
        return 0u; // An error occured
    }
...
struct RecordClient {
    ...
    friend constexpr bool operator==(RecordClient const& a, RecordClient const& b) {
        return a.Id == b.Id;
    }
    friend constexpr bool operator!=(RecordClient const& a, RecordClient const& b) {
        return a.Id != b.Id;
...
inline size_t RecordClientToString(RecordClient const& client, _Out_writes_z_(size) wchar_t* const pBuff,
                                   size_t const size) {
    // Note that written doesn't contain the count of the null terminator
    auto const written = _snwprintf_s(
        pBuff, size, _TRUNCATE,
        L"Record client %u with GUID {%08lX-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X} and lifetime: ", client.Id,
        client.ClientGuid.Data1, client.ClientGuid.Data2, client.ClientGuid.Data3, client.ClientGuid.Data4[0],
        client.ClientGuid.Data4[1], client.ClientGuid.Data4[2], client.ClientGuid.Data4[3], client.ClientGuid.Data4[4],
        client.ClientGuid.Data4[5], client.ClientGuid.Data4[6], client.ClientGuid.Data4[7]);
    if (written <= 0) {
        return 0u; // An error occured
    }
...
inline size_t CustomEventToString(CustomEvent const& event, _Out_writes_z_(size) wchar_t* const pBuff,
                                  size_t const size) {
    // Note that written doesn't contain the count of the null terminator
    auto const written =
        _snwprintf_s(pBuff, size, _TRUNCATE, L"Custom event from client %u on thread %3u (0x%04X) at position: ",
                     event.pRecordClient->Id, event.pThreadInfo->UniqueId, event.pThreadInfo->Id);
    if (written <= 0) {
        return 0u; // An error occured
    }
...
struct Activity {
    ...
    friend constexpr auto AsComparisonTuple(Activity const& a) {
        return std::tie(*a.pRecordClient, a.Id);
    }

    friend constexpr bool operator==(Activity const& a, Activity const& b) {
        return AsComparisonTuple(a) == AsComparisonTuple(b);
    }
    friend constexpr bool operator!=(Activity const& a, Activity const& b) {
        return AsComparisonTuple(a) != AsComparisonTuple(b);
...
inline size_t ActivityToString(Activity const& activity, _Out_writes_z_(size) wchar_t* const pBuff, size_t const size) {
    // Note that written doesn't contain the count of the null terminator
    auto const written = _snwprintf_s(pBuff, size, _TRUNCATE, L"Activity %u from client %u with lifetime: ",
                                      activity.Id, activity.pRecordClient->Id);
    if (written <= 0) {
        return 0u; // An error occured
    }
...
struct Island {
    ...
    friend constexpr bool operator<(Island const& a, Island const& b) {
        return a.Lifetime.Min < b.Lifetime.Min;
    }
    friend constexpr bool operator>(Island const& a, Island const& b) {
        return a.Lifetime.Min > b.Lifetime.Min;
    }
...
inline size_t IslandToString(Island const& island, _Out_writes_z_(size) wchar_t* const pBuff, size_t const size) {
    // Note that written doesn't contain the count of the null terminator
    auto const written = _snwprintf_s(pBuff, size, _TRUNCATE,
                                      L"Island on activity %u from client %u on thread %3u (0x%04X) with lifetime: ",
                                      island.pActivity->Id, island.pActivity->pRecordClient->Id,
                                      island.pThreadInfo->UniqueId, island.pThreadInfo->Id);
    if (written <= 0) {
        return 0u; // An error occured
    }
...
constexpr wchar_t const* GetIndexStatusName(IndexStatus const status) noexcept {
    switch (status) {
        case IndexStatus::IndexFileLoaded:
            return L"Loaded";
        case IndexStatus::IndexFileNotPresent:
            return L"NotPresent";
        case IndexStatus::IndexFileUnloadable:
            return L"Unloadable";
        default:
...
enum class IndexBuildFlags : uint32_t {
    ...
    All = DeleteExistingUnloadableIndexFile | TemporaryIndexFile | MakeSelfContained
};
...
using IndexBuildProgressCallback = void(__stdcall*)(_In_opt_ void const* pCallerContext,
...
template <typename Interface> struct Deleter {
...
// No matter which policy is used, results shall be consistent as long as the current position (and thread) don't
// change.
...
constexpr wchar_t const* GetQueryMemoryPolicyName(_In_ QueryMemoryPolicy const type) {
    switch (type) {
        case QueryMemoryPolicy::Default:
            return L"Default";
        case QueryMemoryPolicy::ThreadLocal:
            return L"ThreadLocal";
        case QueryMemoryPolicy::GloballyConservative:
            return L"GloballyConservative";
        case QueryMemoryPolicy::GloballyAggressive:
            return L"GloballyAggressive";
        case QueryMemoryPolicy::InFragmentAggressive:
            return L"InFragmentAggressive";
        default:
            return L"<Unknown QueryMemoryPolicy>";
    }
}
...
public:
    ...
    virtual MemoryBufferWithRanges QueryMemoryBufferWithRanges(_In_ GuestAddress, _In_ BufferView,
                                                               _In_range_(1, SIZE_MAX) size_t maxRanges,
                                                               _Out_writes_(maxRanges) MemoryRange*) const noexcept = 0;
...
public:
    ...
    // Returns a single range of contiguous memory bytes stored internally,
    // taken from a single position in the trace, where the buffer is valid until the next memory query on this cursor.
    virtual MemoryRange QueryMemoryRange(_In_ GuestAddress,
                                         _In_ QueryMemoryPolicy = QueryMemoryPolicy::Default) const noexcept = 0;

    // Fills as much of the provided buffer as possible, and returns the resulting valid buffer.
    virtual MemoryBuffer QueryMemoryBuffer(_In_ GuestAddress, _In_ BufferView,
                                           _In_ QueryMemoryPolicy = QueryMemoryPolicy::Default) const noexcept = 0;

    // Fills as much of the provided buffer as possible, and returns he resulting valid buffer,
    // along with a list of the corresponding ranges.
    virtual MemoryBufferWithRanges
    QueryMemoryBufferWithRanges(_In_ GuestAddress, _In_ BufferView, _In_range_(1, SIZE_MAX) size_t maxRanges,
    ...
    template <typename Interface = IReplayEngineView> Interface* GetReplayEngine() const noexcept {
        return static_cast<Interface*>(UnsafeGetReplayEngine(__uuidof(Interface)));
    }
    ...
    template <typename Interface> auto AsInterface() noexcept {
        return static_cast<Interface*>(UnsafeAsInterface(__uuidof(Interface)));
    }
    template <typename Interface> auto AsInterface() const noexcept {
        return static_cast<Interface const*>(UnsafeAsInterface(__uuidof(Interface)));
    }
    ...
    // Allows the caller to observe and reject (result == false) or accept (result != false) a particular watchpoint
    // hit. ICursorView is not to be used. Most calls might be unsafe when performed from within the callback.
    // IThreadView allows the callback to safely perform limited thread-local queries for registers, memory and state.
    typedef bool __fastcall MemoryWatchpointCallback(_In_ uintptr_t context, _In_ MemoryWatchpointResult const&,
                                                     _In_ IThreadView const*);

    // Allows the caller to observe and reject (result == false) or accept (result != false) a particular watchpoint
    // hit. ICursorView is not to be used. Most calls might be unsafe when performed from within the callback.
    // IThreadView allows the callback to safely perform limited thread-local queries for registers, memory and state.
    typedef bool __fastcall PositionWatchpointCallback(_In_ uintptr_t context, _In_ Position const&,
                                                       _In_ IThreadView const*);

    // Allows the caller to observe and reject (result == false) or accept (result != false) a particular gap event
    // stop. ICursorView is not to be used. Most calls might be unsafe when performed from within the callback.
    // IThreadView allows the callback to safely perform limited thread-local queries for registers, memory and state.
    ...
    // When synthetic == true, it means that the instruction was emulated at record time, but added to the file as a
    // fallback, to allow the use of replay engines that don't know how to emulate it, like some x64 instructions that
    // don't exist in x86. The address and size parameters indicate where the bytes of the instruction can be found (via
    // a memory query). ICursorView is not to be used. Most calls might be unsafe when performed from within the
    // callback. IThreadView allows the callback to safely perform limited thread-local queries for registers, memory
    // and state.
    typedef void __fastcall FallbackCallback(_In_ uintptr_t context, _In_ bool synthetic,
                                             _In_ GuestAddress instructionAddress, _In_ size_t instructionSize,
                                             _In_ IThreadView const*);

    // Callback on every call / iretq / return instruction
    // call: guestInstructionAddress = calling to; guestFallThroughInstructionAddress = address of function after the
    // call ret: guestInstructionAddress = returning to; guestFallThroughInstructionAddress = nullptr Note: the current
    // position points to the call or ret instruction ICursorView is not to be used. Most calls might be unsafe when
    // performed from within the callback. IThreadView allows the callback to safely perform limited thread-local
    // queries for registers, memory and state.
    typedef void __fastcall CallReturnCallback(_In_ uintptr_t context, _In_ GuestAddress guestInstructionAddress,
                                               _In_ GuestAddress guestFallThroughInstructionAddress,
                                               _In_ IThreadView const*);

    // Callback on every jump instruction to an indirect target
    // targetAddress: address that is being jumped to
    typedef void __fastcall IndirectJumpCallback(_In_ uintptr_t context, _In_ GuestAddress targetAddress,
                                                 _In_ IThreadView const*);
    ...
    // ICursorView is not to be used. Most calls might be unsafe when performed from within the callback.
    // IThreadView allows the callback to safely perform limited thread-local queries for registers, memory and state.
    typedef void __fastcall RegisterChangedCallback(uintptr_t context, uint8_t regId,
                                                    _In_reads_bytes_(dataSizeInBytes) void const* pOldData,
                                                    _In_reads_bytes_opt_(dataSizeInBytes) void const* pNewData,
                                                    size_t dataSizeInBytes, _In_ IThreadView const*);

    virtual void SetMemoryWatchpointCallback(_In_opt_ MemoryWatchpointCallback*, _In_ uintptr_t context) noexcept = 0;
    virtual void SetPositionWatchpointCallback(_In_opt_ PositionWatchpointCallback*,
                                               _In_ uintptr_t context) noexcept = 0;
    virtual void SetGapEventCallback(_In_opt_ GapEventCallback*, _In_ uintptr_t context) noexcept = 0;
    virtual void SetThreadContinuityBreakCallback(_In_opt_ ThreadContinuityBreakCallback*,
                                                  _In_ uintptr_t context) noexcept = 0;
    ...
    ReplayResult ReplayForward(_In_ StepCount stepCount = StepCount::Max) {
        return ReplayForward(Position::Max, stepCount);
    }
    ReplayResult ReplayBackward(_In_ StepCount stepCount = StepCount::Max) {
        return ReplayBackward(Position::Min, stepCount);
    }
    ...
    virtual size_t GetInternalData(InternalDataId, _Out_writes_bytes_(bufferSizeInBytes) void* pBuffer,
                                   size_t bufferSizeInBytes) const noexcept = 0;

    // Convenience adapter to retrieve an internal blob of data of a specific type.
    template <typename T> T GetInternalData() const noexcept {
        T result{};
        GetInternalData(T::InternalDataId, &result, sizeof(result));
        return result;
    }
    ...
    [[deprecated]] bool AddWatchpoint(_In_ MemoryWatchpointData const& data) noexcept {
        return AddMemoryWatchpoint(data);
    }
    [[deprecated]] bool RemoveWatchpoint(_In_ MemoryWatchpointData const& data) noexcept {
        return RemoveMemoryWatchpoint(data);
    }

    [[deprecated]] void SetWatchpointCallback(_In_opt_ MemoryWatchpointCallback* callback,
                                              _In_ uintptr_t context) noexcept {
        return SetMemoryWatchpointCallback(callback, context);
    }
    template <typename Func> void SetWatchpointCallback(Func& func) noexcept {
        return SetMemoryWatchpointCallback(func);
    }
...
template <typename Func> void ICursorView::SetMemoryWatchpointCallback(Func& func) noexcept {
    SetMemoryWatchpointCallback(
        [](uintptr_t context, MemoryWatchpointResult const& watchpointResult, IThreadView const* pThreadView) -> bool {
            return (*reinterpret_cast<Func*>(context))(watchpointResult, pThreadView);
        },
        reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetPositionWatchpointCallback(Func& func) noexcept {
    SetPositionWatchpointCallback(
        [](uintptr_t context, Position const& watchpointPosition, IThreadView const* pThreadView) -> bool {
            return (*reinterpret_cast<Func*>(context))(watchpointPosition, pThreadView);
        },
        reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetGapEventCallback(Func& func) noexcept {
    SetGapEventCallback(
        [](uintptr_t context, GapKind kind, GapEventType event, IThreadView const* pThreadView) -> bool {
            return (*reinterpret_cast<Func*>(context))(kind, event, pThreadView);
        },
        reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetThreadContinuityBreakCallback(Func& func) noexcept {
    SetThreadContinuityBreakCallback([](uintptr_t context) { return (*reinterpret_cast<Func*>(context))(); },
                                     reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetReplayProgressCallback(Func& func) noexcept {
    SetReplayProgressCallback(
        [](uintptr_t context, _In_ Position const& position) { return (*reinterpret_cast<Func*>(context))(position); },
        reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetFallbackCallback(Func& func) noexcept {
    SetFallbackCallback(
        [](uintptr_t context, _In_ bool synthetic, _In_ GuestAddress instructionAddress, _In_ size_t instructionSize,
...
template <typename Func> void ICursorView::SetCallReturnCallback(Func& func) noexcept {
    SetCallReturnCallback(
        [](uintptr_t context, _In_ GuestAddress guestInstructionAddress,
           _In_ GuestAddress guestFallThroughInstructionAddress, _In_ IThreadView const* pThreadView) {
            return (*reinterpret_cast<Func*>(context))(guestInstructionAddress, guestFallThroughInstructionAddress,
                                                       pThreadView);
        },
        reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetIndirectJumpCallback(Func& func) noexcept {
    SetIndirectJumpCallback(
        [](uintptr_t context, GuestAddress targetAddress, _In_ IThreadView const* pThreadView) {
            return (*reinterpret_cast<Func*>(context))(targetAddress, pThreadView);
        },
        reinterpret_cast<uintptr_t>(&func));
}

template <typename Func> void ICursorView::SetRegisterChangedCallback(Func& func) noexcept {
    SetRegisterChangedCallback(
        [](uintptr_t context, uint8_t regId, _In_reads_bytes_(dataSizeInBytes) void const* pOldData,
           _In_reads_bytes_opt_(dataSizeInBytes) void const* pNewData, size_t dataSizeInBytes,
           _In_ IThreadView const* pThreadView) {
...
inline MemoryBuffer QueryMemoryBuffer(_In_ ICursorView const* pCursor, _In_ GuestAddress address,
                                      _In_ BufferView bufferView) {
    return pCursor->QueryMemoryBuffer(address, bufferView);
}

inline MemoryBuffer QueryMemoryBuffer(_In_ IThreadView const* pThreadView, _In_ GuestAddress address,
                                      _In_ BufferView bufferView) {
...
public:
    ...
    virtual IndexStatus BuildIndex(_In_ IndexBuildProgressCallback reportProgress, _In_opt_ void const* pCallerContext,
                                   _In_ IndexBuildFlags flags = IndexBuildFlags::None) noexcept = 0;

    virtual IndexStatus GetIndexStatus() const noexcept = 0;

    virtual IndexFileStats GetIndexFileStats() const noexcept = 0;

    virtual void RegisterDebugModeAndLogging(DebugModeType const debugMode,
                                             _In_opt_ ErrorReporting* const pErrorReporting) noexcept = 0;

    [[deprecated]] void RegisterErrorAndWarningCallbacks(_In_ void const*, _In_ void const*, _In_ DebugModeType mode,
                                                         _In_opt_ ErrorReporting* pErrorReporting) noexcept {
...
inline constexpr auto ThreadTerminatedEvents(IReplayEngineView const* pReplayEngineView) noexcept {
    return std::span(pReplayEngineView->GetThreadTerminatedEventList(),
                     pReplayEngineView->GetThreadTerminatedEventCount());
}
...
public:
    ...
    virtual bool LoadFile(_In_z_ wchar_t const* pFileName,
                          _In_opt_z_ wchar_t const* pCompanionName = nullptr) noexcept = 0;
