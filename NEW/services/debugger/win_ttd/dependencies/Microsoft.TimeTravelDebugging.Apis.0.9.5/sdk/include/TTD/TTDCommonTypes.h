...
constexpr InstructionCount operator+(InstructionCount a, uint64_t b) {
    return static_cast<InstructionCount>(static_cast<uint64_t>(a) + b);
}
constexpr InstructionCount operator-(InstructionCount a, uint64_t b) {
    return static_cast<InstructionCount>(static_cast<uint64_t>(a) - b);
}
...
constexpr uint64_t operator%(InstructionCount a, uint64_t b) {
    return static_cast<uint64_t>(a) % b;
}
constexpr InstructionCount operator+(InstructionCount a, InstructionCount b) {
...
constexpr bool operator==(InstructionCount a, uint64_t b) {
    return static_cast<uint64_t>(a) == b;
}
constexpr bool operator!=(InstructionCount a, uint64_t b) {
    return static_cast<uint64_t>(a) != b;
...
inline InstructionCount& operator+=(_Inout_ InstructionCount& a, int64_t b) {
    return a = static_cast<InstructionCount>(static_cast<uint64_t>(a) + b);
}
inline InstructionCount& operator-=(_Inout_ InstructionCount& a, int64_t b) {
    return a = static_cast<InstructionCount>(static_cast<uint64_t>(a) - b);
}
inline InstructionCount& operator+=(_Inout_ InstructionCount& a, InstructionCount b) {
    return a += static_cast<uint64_t>(b);
}
inline InstructionCount& operator-=(_Inout_ InstructionCount& a, InstructionCount b) {
    return a -= static_cast<uint64_t>(b);
...
enum class ThreadRecordingState : uint8_t {
    ...
    Paused =
        2, // Inside of a Start/Stop recording sequence, but recording has been temporarily paused by calling the API.
    Throttled =
        3, // Inside of a Start/Stop recording sequence, but recording has been halted by the throttling mechanism.
};
