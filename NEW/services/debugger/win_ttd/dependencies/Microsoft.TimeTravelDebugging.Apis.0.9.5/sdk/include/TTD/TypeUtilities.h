...
__forceinline TResult __fastcall SafeNumericCastOrZero(TArgument argument) {
...
__forceinline TArgument __fastcall ValidateLimitOrZero(TArgument argument, TLimit limit) {
    if (argument > limit) {
...
template <typename T, typename V> __forceinline T* __fastcall PointerOffsetInBytes(_In_ T* ptr, V offset) {
...
__forceinline T* __fastcall
PointerOffsetInBytes(_In_ typename std::conditional<std::is_const<T>::value, void const, void>::type* ptr, V offset) {
    return reinterpret_cast<T*>(reinterpret_cast<uintptr_t>(ptr) + static_cast<uintptr_t>(offset));
}
