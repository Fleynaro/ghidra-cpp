...
template <typename Interface> using Unique = std::unique_ptr<Interface, Deleter<Interface>>;

using UniqueTraceList = Unique<ITraceList>;
using UniqueReplayEngine = Unique<IReplayEngine>;
using UniqueCursor = Unique<ICursor>;

extern "C" uint32_t __cdecl CreateReplayEngine(_Out_ IReplayEngine*& pReplayEngine,
                                               _In_ GUID const& engineGuidCreateReplayEngine) noexcept;

inline std::pair<UniqueReplayEngine, uint32_t>
MakeReplayEngine(_In_ GUID const& engineGuid = __uuidof(IReplayEngineView)) noexcept {
...
extern "C" uint32_t __cdecl CreateTraceList(_Out_ ITraceList*& pTraceList, _In_ GUID const& engineGuid) noexcept;

inline std::pair<UniqueTraceList, uint32_t>
MakeTraceList(_In_ GUID const& engineGuid = __uuidof(ITraceListView)) noexcept {
