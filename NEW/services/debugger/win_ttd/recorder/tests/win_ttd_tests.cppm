...
export module recode.service.debugger.win_ttd.recorder.tests;

import std;
import recode.core;
import recode.service.debugger.win_ttd.recorder;

namespace recorder = recode::services::debugger::win_ttd::recorder;
...
TEST(WinTtdRecorder, BuildsTtdLaunchCommand) {
    recode::core::trace_recording::RecordingRequest request{
...
TEST(WinTtdRecorder, ReportsUnsupportedPlatformWhenRecording) {
    ...
    recode::core::contracts::OperationContext context{};
    recode::core::trace_recording::RecordingRequest request{.program = "missing-program.exe",
                                                            .output_trace = "missing.run"};
    auto task = recorder_instance.record(std::move(request), context);
    const auto result = task.get();
#ifndef _WIN32
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().code, recode::core::DiagnosticCode::unsupported);
#else
...
TEST(WinTtdRecorder, RejectsUnsupportedRecordingOptions) {
    recorder::WinTtdRecorder recorder_instance;
    recode::core::trace_recording::RecordingRequest request{.program = "target.exe", .output_trace = "target.run"};
...
TEST(WinTtdRecorder, RejectsNonRunOutput) {
    recorder::WinTtdRecorder recorder_instance;
    recode::core::trace_recording::RecordingRequest request{.program = "target.exe", .output_trace = "target.ttd"};
    const auto result = recorder_instance.record(std::move(request), {}).get();
    ASSERT_FALSE(result);
    EXPECT_EQ(result.error().code, recode::core::DiagnosticCode::invalid_argument);
}

/// Verifies recording never overwrites an existing trace artifact.
TEST(WinTtdRecorder, RejectsExistingOutput) {
    const auto output = std::filesystem::temp_directory_path() / "recode-existing.run";
    std::ofstream{output} << "existing";
    recorder::WinTtdRecorder recorder_instance;
    recode::core::trace_recording::RecordingRequest request{.program = "target.exe", .output_trace = output};
    const auto result = recorder_instance.record(std::move(request), {}).get();
    std::error_code cleanup_error;
    std::filesystem::remove(output, cleanup_error);
    ASSERT_FALSE(result);
    EXPECT_EQ(result.error().code, recode::core::DiagnosticCode::conflict);
}
...
TEST(WinTtdRecorder, RejectsMissingOutputDirectory) {
    const auto output = std::filesystem::temp_directory_path() / "recode-missing-directory" / "trace.run";
    recorder::WinTtdRecorder recorder_instance;
    recode::core::trace_recording::RecordingRequest request{.program = "target.exe", .output_trace = output};
    const auto result = recorder_instance.record(std::move(request), {}).get();
    ASSERT_FALSE(result);
...
TEST(WinTtdRecorder, ReportsMissingExecutable) {
    ...
    const auto output = std::filesystem::temp_directory_path() / "recode-missing-ttd.run";
    recorder::WinTtdRecorder recorder_instance;
    recode::core::trace_recording::RecordingRequest request{.program = "target.exe", .output_trace = output};
    const auto result = recorder_instance.record(std::move(request), {}).get();
    if (previous != nullptr)
    ...
    std::filesystem::remove(output, cleanup_error);
    ASSERT_FALSE(result);
    EXPECT_EQ(result.error().code, recode::core::DiagnosticCode::resource_unavailable);
#endif
}
...
TEST(WinTtdRecorder, CancelsOptedInLongRecording) {
    ...
    const auto output = std::filesystem::temp_directory_path() / "recode-ttd-cancel-test.run";
    std::error_code cleanup_error;
    std::filesystem::remove(output, cleanup_error);
...
TEST(WinTtdRecorder, RecordsProvidedProgram) {
    ...
    const auto output = std::filesystem::temp_directory_path() / "recode-ttd-recorder-test.run";
    std::error_code cleanup_error;
    std::filesystem::remove(output, cleanup_error);
    const auto recorder_service = recorder::create_win_ttd_recorder();
    ASSERT_TRUE(recorder_service) << recorder_service.error().message;
