...
The real backend has been compiled and exercised against the pinned Microsoft.TimeTravelDebugging.Apis 0.9.5 inputs and runtime DLLs owned by this service. Run `NEW\build.bat ttd_setup` to restore/update dependencies, then `NEW\build.bat ttd_replay` to require the real backend for an already prepared trace. For a complete workflow, use `NEW\build.bat ttd_all`; it provisions missing dependencies, requires the prebuilt multi-thread debuggee in `tests/data`, records a temporary `.run`, and runs the full CTest suite through the real recorder and Replay API. It never rebuilds the fixture executable automatically. `TTD_APIS_PACKAGE_DIR` and `TTD_RUNTIME_DIR` remain explicit overrides for external installations.
