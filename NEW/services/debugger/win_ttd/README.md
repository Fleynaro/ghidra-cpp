...
- [`tests/data`](tests/data) owns the multi-thread recording debuggee used by `ttd_all` and trace-analysis tests; it is independent of the live-debugger fixture tree.
...
- [`setup_dependencies.bat`](setup_dependencies.bat) is the provisioning implementation; the recommended single entry point is `build.bat ttd_setup`, which invokes it through the CMake target `recode_win_ttd_setup`.
...
The real backend has been compiled and exercised against the pinned Microsoft.TimeTravelDebugging.Apis 0.9.5 inputs and runtime DLLs owned by this service. Run `build.bat ttd_setup` to restore/update dependencies, then `build.bat ttd_replay` to require the real backend for an already prepared trace. For a complete workflow, use `build.bat ttd_all`; it provisions missing dependencies, requires the prebuilt multi-thread debuggee in `tests/data`, records a temporary `.run`, and runs the full CTest suite through the real recorder and Replay API. It never rebuilds the fixture executable automatically. `TTD_APIS_PACKAGE_DIR` and `TTD_RUNTIME_DIR` remain explicit overrides for external installations.

To reproduce the dependency setup without any external repository path:

```powershell
build.bat ttd_setup
$env:TTD_APIS_PACKAGE_DIR = (Resolve-Path services\debugger\win_ttd\dependencies\Microsoft.TimeTravelDebugging.Apis.0.9.5).Path
$env:TTD_RUNTIME_DIR = (Resolve-Path services\debugger\win_ttd\dependencies\runtime\x64).Path
build.bat ttd_replay
```

The normal `build.bat all` remains the non-recording full build. `ttd_all` is intentionally explicit because it launches `ttd.exe` and records a temporary process execution.
