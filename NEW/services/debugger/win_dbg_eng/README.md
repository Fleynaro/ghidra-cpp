...
- [`CMakeLists.txt`](CMakeLists.txt) builds the backend as `ReCode::WinDbgEng` and links the Windows-provided `dbgeng` library only for MSVC builds.
...
- The backend depends only on [`../../../core`](../../../core/README.md) and does not depend on `runtime`.
