...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/printc.cc.
...
/// References the C capability singleton from a public member so clients
/// linking the static library retain this translation unit.
void PrintCCapability::ensureRegistration(void)

{
    (void)printCCapability;
}

/// References the C capability singleton so static-library clients retain
/// the translation unit whose static initializer registers the printer.
void forcePrintCLanguageRegistration(void)

{
    PrintCCapability::ensureRegistration();
}

/// \param g is the Architecture owning this c-language emitter
/// \param nm is the name assigned to this emitter
PrintC::PrintC(Architecture* g, const string& nm)
    : PrintLanguage(g, nm)
...

{
    ...
    for (;;) {
        ...
        if (ct->getMetatype() == TYPE_PTR)
            ...
            ct = ctarray->getBase();
            push_integer(ctarray->numElements(), 4, false, syntax, (const Varnode*)0, (const PcodeOp*)0, 0);
        } else if (ct->getMetatype() == TYPE_CODE) {
...

{
    if (op->doesSpecialPrinting()) { // Special printing means it is a field extraction
        ...
        if (ct->isPieceStructured()) {
            ...
            const TypeField* field =
                ct->findTruncation(byteOff, op->getOut()->getSize(), op, 1, offset); // Use artificial slot
    ...
    if (castStrategy->isSubpieceCast(op->getOut()->getHighTypeDefFacing(), op->getIn(0)->getHighTypeReadFacing(op),
...

{
    ...
    if (expr.loadOp != (const PcodeOp*)0) {
        ...
        pushAtom(
            Atom(expr.bitfield->name, bitfieldtoken, EmitMarkup::no_color, expr.theStruct, expr.bitfield->ident, op));
    } else {
        pushOp(&object_member, op);
...
void PrintC::push_integer(uintb val, int4 sz, bool sign, tagtype tag, const Varnode* vn, const PcodeOp* op,
                          uint4 displayFormat) {
    bool print_negsign;
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void PrintC::pushBoolConstant(uintb val, const TypeBase* ct, tagtype tag, const Varnode* vn, const PcodeOp* op) {
    if (val != 0)
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void PrintC::pushEnumConstant(uintb val, const TypeEnum* ct, tagtype tag, const Varnode* vn, const PcodeOp* op) {
...
bool PrintC::pushPtrCodeConstant(uintb val, const TypePointer* ct, const Varnode* vn, const PcodeOp* op) {
...
void PrintC::pushConstant(uintb val, const Datatype* ct, tagtype tag, const Varnode* vn, const PcodeOp* op,
                          uint4 displayFormat) {
    Datatype* subtype;
...
void PrintC::pushUnnamedLocation(const Address& addr, const Varnode* vn, const PcodeOp* op) {
    ostringstream s;
...
void PrintC::pushPartialSymbol(const Symbol* sym, int4 off, int4 sz, const Varnode* vn, const PcodeOp* op, int4 slot,
                               bool allowCast) {
...
void PrintC::pushMismatchSymbol(const Symbol* sym, int4 off, int4 sz, const Varnode* vn, const PcodeOp* op) {
    if (off == 0) {
...
void PrintC::emitPrototypeOutput(const FuncProto* proto, const Funcdata* fd) {
    PcodeOp* op;
    Varnode* vn;
...
void PrintC::emitGotoStatement(const FlowBlock* bl, const FlowBlock* exp_bl, uint4 type)
...

{
    ...
    try {
        commsorter.setupFunctionList(instr_comment_type | head_comment_type, fd, *fd->getArch()->commentdb,
                                     option_unplaced);
...

{
    ...
    if (mainType == FlowBlock::t_ifgoto) {
        emit->spaces(1);
        emitGotoStatement(condBlock, ((const BlockIfGoto*)bl)->getGotoTarget(),
                          ((const BlockIfGoto*)bl)->getGotoType());
    } else {
        setMod(no_branch);
        ...
        if (mainType == FlowBlock::t_ifelse) {
            ...
            // Since the first body does not exit, we don't need an "else" and curly braces for the body on the
            // alternate path
