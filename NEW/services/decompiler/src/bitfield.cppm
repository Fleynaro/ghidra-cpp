...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/bitfield.cc.
...
const vector<uint4> BitFieldInsertTransform::allowedFinalWrites = {
    CPUI_COPY,           CPUI_INT_EQUAL,   CPUI_INT_NOTEQUAL,    CPUI_INT_SLESS, CPUI_INT_SLESSEQUAL, CPUI_INT_LESS,
    CPUI_INT_LESSEQUAL,  CPUI_INT_ZEXT,    CPUI_INT_SEXT,        CPUI_INT_ADD,   CPUI_INT_CARRY,      CPUI_INT_SCARRY,
    CPUI_INT_XOR,        CPUI_INT_AND,     CPUI_INT_OR,          CPUI_INT_LEFT,  CPUI_INT_RIGHT,      CPUI_INT_SRIGHT,
    CPUI_INT_MULT,       CPUI_BOOL_NEGATE, CPUI_BOOL_XOR,        CPUI_BOOL_AND,  CPUI_BOOL_OR,        CPUI_FLOAT_EQUAL,
    CPUI_FLOAT_NOTEQUAL, CPUI_FLOAT_LESS,  CPUI_FLOAT_LESSEQUAL, CPUI_FLOAT_NAN, CPUI_SUBPIECE};
...

{
    ...
    if (finalWriteOp->code() != CPUI_STORE) {
        ...
        BitRange curRange(initialOffset, mappedVn->getSize(), state.origLeastSigBit, state.bitsField.numBits,
                          isBigEndian);
...

{
    ...
    if (pos != state.bitsField.leastSigBit)
        return false;
    if (numbits != state.bitsField.numBits)
        return false;
...

{
    if (finalWriteOp->code() == CPUI_STORE) {
        ...
        if (loadOp->code() != CPUI_LOAD)
            return false;
        if (!pointerEquality(loadOp->getIn(1), finalWriteOp->getIn(1)))
            return false;
        if (loadOp->getParent() != finalWriteOp->getParent())
            return false;
    } else {
        if (mappedVn == vn)
            return false;
...

{
    ...
    if (dt->getSize() != state.node->getSize())
        dt = (Datatype*)0;
    insertList.emplace_back(state.node, dt, state.origLeastSigBit, state.field->bits.numBits,
                            state.bitsField.leastSigBit);
    state.node = (Varnode*)0;
}
...

{
    if (!op->isCall())
        return false;
    if (finalWriteOp->code() == CPUI_STORE)
        return false; // If value is being STOREd, original value is not a call
    if (state.bitsField.leastSigBit != state.origLeastSigBit)
        return false;
    if (mappedVn->isAddrTied())
        return false; // If value is address tied, cannot have different storage
    if (originalValue != (Varnode*)0)
        return false; // Already found an original value
    ...
    if (dt->getMetatype() == TYPE_STRUCT) {
        ...
        return false;
    if (dt != parentStruct)
        return false; // Check if data-type matches
...

{
    ...
    for (iter = vn->beginDescend(); iter != vn->endDescend(); ++iter) {
        PcodeOp* op = *iter;
        if (op->code() == CPUI_STORE && op->doesSpecialPrinting())
            continue;
...

{
    ...
    for (; iter != rec.vn->endDescend(); ++iter) {
        ...
        if (op->code() != CPUI_INSERT) {
            ...
            if (op == (PcodeOp*)0 || op->code() != CPUI_INSERT)
                continue;
        }
        ...
        if (op->getIn(2)->getOffset() != immedOp->getIn(2)->getOffset())
            continue;
        if (op->getIn(3)->getOffset() != immedOp->getIn(3)->getOffset())
            continue;
        if (finalWriteOp->code() == CPUI_STORE) {
            PcodeOp* store1 = op->getOut()->loneDescend();
            if (store1 == (PcodeOp*)0)
                continue;
            if (store1->code() != CPUI_STORE)
                continue;
            PcodeOp* store2 = immedOp->getOut()->loneDescend();
            if (store2 == (PcodeOp*)0)
                continue;
            if (store2->code() != CPUI_STORE)
                continue;
            if (store1->getParent() != store2->getParent())
                continue;
            if (!pointerEquality(store1->getIn(1), store2->getIn(1)))
                continue;
...

{
    ...
    while (iter != biter) {
        ...
        if (insertOp->code() != CPUI_INSERT)
            return false; // Unknown value
        uintb insertMask = InsertExpression::getRangeMask(insertOp);
        if ((insertMask & mask) != 0)
            return false; // Writing bits that are supposed to be original value
    }
    return true;
}
...

{
    ...
    while (iter != biter) {
        ...
        if (vn->getAddr() != originalValue->getAddr())
            continue;
        if (vn->getSize() != originalValue->getSize())
            continue;
        if (!vn->isWritten())
            return false; // Unknown value
        PcodeOp* insertOp = vn->getDef();
...
{
    if (originalValue == (Varnode*)0)
        return true; // Not using original value bits
...

{
    ...
    if (finalWriteOp->code() == CPUI_STORE) {
        ...
        for (iter = insertList.begin(); iter != insertList.end(); ++iter) {
            ...
            if (currentStore == (PcodeOp*)0) {
                ...
                if (loadModel != (PcodeOp*)0) {
                    PcodeOp* loadOp =
                        func->newOp(2, loadModel->getAddr()); // Create new LOAD for each additional INSERT
        ...
        PcodeOp* insertOp =
            setInsertInputs(finalWriteOp, *iter); // Redefine finalWriteOp as INSERT, preserving original output
...
{
    if (bitField.byteSize > sizeof(uintb))
        return false;
...

{
    ...
    if (state.bitsField.numBits == newRange.numBits) {
        bool newSignExt =
            state.isSignExtended; // Any sign extension is preserved, since we only truncate and whole field is present
        workList.emplace_back(state, newRange, op->getOut(), newSignExt);
...

{
    ...
    if (needMaskCheck) {
        ...
        mask -= 1;
        if ((mask & state.node->getNZMask()) == mask)
...
list<BitFieldPullTransform::PullRecord>::iterator
BitFieldPullTransform::testCompareGroup(list<PullRecord>::iterator iter)
...

{
    ...
    if (!dt->hasBitfields())
        return 0;
    Varnode* vn = op->getIn(2);
    if (vn->isWritten() && vn->getDef()->code() == CPUI_INSERT)
        return 0;
...
{
    oplist.insert(oplist.end(), BitFieldInsertTransform::allowedFinalWrites.begin(),
                  BitFieldInsertTransform::allowedFinalWrites.end());
...

{
    ...
    for (iter = outvn->beginDescend(); iter != outvn->endDescend(); ++iter) {
        ...
        if (opc != CPUI_INT_EQUAL && opc != CPUI_INT_NOTEQUAL)
            continue;
        if (!readOp->getIn(1)->constantMatch(0))
            continue;
...

{
    ...
    if (!leftcvn->isConstant())
        return 0;
    Varnode* rightcvn = rightOp->getIn(1);
    if (!rightcvn->isConstant())
        return 0;
    ...
    if (leftshift + bitsize > containerSize)
...

{
    ...
    if (!zvn->constantMatch(0))
        return 0;
    int4 bitsize = (int4)pullOp->getIn(2)->getOffset();
    if (bitsize != 1)
        return 0;
    Varnode* vn = compOp->getIn(0);
    if (vn->loneDescend() != compOp)
        return 0;
    if (vn->isAddrTied())
        return 0;
    if (pullOp->code() == CPUI_SPULL)
...

{
    ...
    if (bitsize >
        insertOp->getIn(1)->getSize() * 8 - lsa) // Shifts cancel unless bitsize exceeds number of bits preserved
        return 0;
...
///
/// The op feeding the INSERT can be any operation where more significant bits of the input do not affect the less
/// significant bits.
/// \param data is the function
...
{
    if (baseOp->getOut()->loneDescend() != insertOp)
