...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/cast.cc.
...

{
    ...
    type_metatype meta = dt->getMetatype();
    if (meta != TYPE_UINT && meta != TYPE_INT && meta != TYPE_UNKNOWN && meta != TYPE_PARTIALSTRUCT &&
        meta != TYPE_PARTIALUNION)
        return false;
...

{
    ...
    int4 natural;
    if (meta == TYPE_UINT || meta == TYPE_BOOL || meta == TYPE_UNKNOWN || meta == TYPE_PARTIALSTRUCT ||
        meta == TYPE_PARTIALUNION)
...
Datatype* CastStrategyC::castStandard(Datatype* reqtype, Datatype* curtype, bool care_uint_int,
                                      bool care_ptr_uint) const

{ // Generic casting rules that apply for most ops
    if (curtype == reqtype)
        return (Datatype*)0; // Types are equal, no cast required
    ...
    if (curbase == reqbase)
        return (Datatype*)0; // Different typedefs could point to the same type
...

{
    ...
    if (outmeta != TYPE_INT && outmeta != TYPE_UINT && outmeta != TYPE_UNKNOWN && outmeta != TYPE_PTR &&
        outmeta != TYPE_FLOAT)
        return false;
    if (inmeta == TYPE_PTR) {
        if (outmeta == TYPE_PTR) {
...
Datatype* CastStrategyJava::castStandard(Datatype* reqtype, Datatype* curtype, bool care_uint_int,
                                         bool care_ptr_uint) const
