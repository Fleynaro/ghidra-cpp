...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/prettyprint.cc.
        ...
        case TokenSplit::tokenbreak:
            if (tok.getSize() > spaceremain) {
                if (tok.getTag() == TokenSplit::line_t) // Absolute indent
                    ...
                    if ((tok.getNumSpaces() <= spaceremain) && (val - spaceremain < 10)) {
...
void EmitPrettyPrint::tagBitField(const string& name, syntax_highlight hl, const Datatype* ct, int4 id,
                                  const PcodeOp* op)
