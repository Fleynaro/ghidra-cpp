...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/double.cc.
...
{
    if (!h->isPrecisHi())
        return false; // Check for mark, in order to have quick -false- in most cases
        ...
        if (op->code() == CPUI_SUBPIECE) {
            ...
            while (iter != enditer) {
                ...
                if (tmphi->getSize() + l->getSize() != w->getSize())
                    continue;
                if (tmpop->getIn(1)->getOffset() != (uintb)l->getSize())
                    continue;
                // There could conceivably be more than one, but this shouldn't happen with CSE
                initAll(w, l, tmphi);
                return true;
...

{
    if (whole == (Varnode*)0) {
        ...
        if (subhi->code() != CPUI_SUBPIECE)
            return false;
        if (subhi->getIn(1)->getOffset() != wholesize - hi->getSize())
            return false;
        Varnode* putativeWhole = subhi->getIn(0);
        if (putativeWhole->getSize() != wholesize)
            return false;
        if (!lo->isWritten())
            return false;
...
{
    PcodeOp* lastop;
    if (hi != (Varnode*)0 && hi->isConstant())
        return false; // If one but not both is constant
    if (lo->isConstant())
        return false;
...

{
    ...
    if (op1->code() != CPUI_INT_ADD)
        return false;
    if (!op1->getIn(1)->isConstant())
        return false;
    uintb c1 = op1->getIn(1)->getOffset();

    if (op1->getIn(0) != op2->getIn(0))
        return false;
    return ((c1 + size1) == c2);
}
...
bool SplitVarnode::testContiguousPointers(PcodeOp* most, PcodeOp* least, PcodeOp*& first, PcodeOp*& second,
                                          AddrSpace*& spc)
...

{
    ...
    while (iter != enditer) {
        ...
        while (iter2 != enditer2) {
            ...
            if (hicpy->getAddr() != addr)
                continue;
            if (hiop->getParent() != loop->getParent())
                continue;
...

{
    ...
    if (existop == (PcodeOp*)0)
        return existop; // If we can find no such point return false;
...
void SplitVarnode::createShiftOp(Funcdata& data, SplitVarnode& out, SplitVarnode& in, Varnode* sa, PcodeOp* existop,
                                 OpCode opc)
...
/// \param opc is the opcode of the operation
void SplitVarnode::replaceBoolOp(Funcdata& data, PcodeOp* boolop, SplitVarnode& in1, SplitVarnode& in2, OpCode opc)
...
void SplitVarnode::createBoolOp(Funcdata& data, PcodeOp* cbranch, SplitVarnode& in1, SplitVarnode& in2, OpCode opc)
...
/// \param existop is the precalculated PcodeOp where the output whole Varnode must exist
void SplitVarnode::createPhiOp(Funcdata& data, SplitVarnode& out, vector<SplitVarnode>& inlist, PcodeOp* existop)
...
void SplitVarnode::replaceCopyForce(Funcdata& data, const Address& addr, SplitVarnode& in, PcodeOp* copylo,
                                    PcodeOp* copyhi)
    ...
    for (int4 i = 0; i < 3; ++i) {
        ...
        for (int4 j = 0; j < 2; ++j) {
            if (i == 2) { // hi2 is an implied 0
                ...
                if (!hizext2->isWritten())
                    continue;
                zextop = hizext2->getDef();
                hi2 = hizext1;
            }
            if (!checkForCarry(zextop))
...

{
    ...
    for (int4 i = 0; i < 2; ++i) {
        ...
        if (!hineg2->isWritten())
            continue;
        if (!hineg1->getDef()->verifyMultNegOne())
            continue;
        if (!hineg2->getDef()->verifyMultNegOne())
            continue;
        ...
        for (int4 j = 0; j < 2; ++j) {
            ...
            while (iter2 != enditer2) {
                ...
                if (!tmpvn->isWritten())
                    continue;
                negop = tmpvn->getDef();
                if (!negop->verifyMultNegOne())
                    continue;
                if (negop->getIn(0) != lo2)
                    continue;
                reslo = loadd->getOut();
                return true;
            }
...

{
    ...
    if (res == 0) { // We found a matching lo presis operation
        ...
        if ((lo2 == lo1) || (lo2 == hi1) || (hi2 == hi1) || (hi2 == lo1))
            return false; // No manipulation of itself
        if (lo2 == hi2)
            return false;
        return true;
    }
            ...
            while (iter3 != enditer3) {
                ...
                if ((hibool->code() == CPUI_CBRANCH) && (lobool->code() == CPUI_CBRANCH)) {
                    ...
                    if ((hibooltrue == lobool->getParent()) && // hi is checked first then lo
                        (hiboolfalse == loboolfalse) && SplitVarnode::otherwiseEmpty(lobool)) {
                        if (SplitVarnode::prepareBoolOp(in1, in2, hibool)) {
                            setonlow = true;
                            SplitVarnode::createBoolOp(data, hibool, in1, in2,
                                                       notequalformhi ? CPUI_INT_NOTEQUAL : CPUI_INT_EQUAL);
                            ...
                            return true;
                               ...
                               (hiboolfalse == loboolfalse) && SplitVarnode::otherwiseEmpty(hibool)) {
                        if (SplitVarnode::prepareBoolOp(in1, in2, lobool)) {
                            setonlow = false;
                            SplitVarnode::createBoolOp(data, lobool, in1, in2,
                                                       notequalformlo ? CPUI_INT_NOTEQUAL : CPUI_INT_EQUAL);
...

{
    ...
    if (!smallc->isConstant())
        return false;
    if (smallc->getOffset() != allonesval)
        return false;
    return true;
}
...

{
    ...
    if (in2.exceedsConstPrecision())
        return false;
    if (!SplitVarnode::prepareBoolOp(in, in2, compareop))
        return false;
...
{ // Assuming -lobl- is the block containing the low precision test of a double precision lessthan
    ...
    lolessbl = lobl;
    if (lolessbl->sizeIn() != 1)
        return false;
    if (lolessbl->sizeOut() != 2)
        return false;
...

{
    ...
    if (lolessbool == (PcodeOp*)0)
        return false;
    if (lolessbool->code() != CPUI_CBRANCH)
        return false;
    hieqbool = hieqbl->lastOp();
    if (hieqbool == (PcodeOp*)0)
        return false;
    if (hieqbool->code() != CPUI_CBRANCH)
        return false;
    hilessbool = hilessbl->lastOp();
    if (hilessbool == (PcodeOp*)0)
        return false;
    if (hilessbool->code() != CPUI_CBRANCH)
        return false;
    ...
    switch (loless->code()) { // Only unsigned forms
        ...
        case CPUI_INT_LESSEQUAL:
            ...
            break;
        case CPUI_INT_EQUAL:
            if (!loless->getIn(1)->isConstant())
                return false;
            if (loless->getIn(1)->getOffset() != 0)
                return false;
...

{
    ...
    if (vnhie2->isConstant()) {
        ...
        if (vnhie2->getSize() == in.getSize()) {
            ...
            if (midlessform) {
                if (midlessequal) {
                    if (lopart != calc_mask(in.getLo()->getSize()))
                        return false;
                } else {
                    if (lopart != 0)
                        return false;
...

{
    ...
    if ((hilesstrue == lolesstrue) && (hieqfalse == lolessfalse) && (hilessfalse == hieqbl) && (hieqtrue == lolessbl)) {
...
{ // Given the less than comparison for the lo piece and an input varnode explicitly marked as isPrecisLo
    ...
    if (loop == (PcodeOp*)0)
        return false;
    if (!mapBlocksFromLow(loop->getParent()))
        return false;
    if (!mapOpsFromBlocks())
        return false;
    if (!checkSignedness())
...
{ // Assume reshi, reslo are filled in, fill in other ops and varnodes
    ...
    if (midshift->code() != CPUI_INT_RIGHT)
        return false; // Must be unsigned RIGHT
    hishift = midhi->getDef();
    if (hishift->code() != CPUI_INT_LEFT)
        return false;

    if (lo != loshift->getIn(0))
        return false;
    if (hi != hishift->getIn(0))
        return false;
    if (lo != midshift->getIn(0))
        return false;
...
{ // Assume reshi, reslo are filled in, fill in other ops and varnodes
    ...
    if (midshift->code() != CPUI_INT_LEFT)
        return false;
...
{ // Verify that big is (some form of) a zero extension of small
    ...
    if (small->isConstant()) {
        if (!big->isConstant())
            return false;
        if (big->getOffset() == small->getOffset())
            return true;
        return false;
    }
    if (!big->isWritten())
...
{ // Given we have labelled lo1/hi1 lo2/hi2, make sure midtmp is formed properly
    ...
    if (subhi->getIn(1)->getOffset() != lo1->getSize())
        return false;
    ...
    while (iter != enditer) {
        ...
        if (lo2->isConstant()) {
            if ((!vn1->isConstant() || (vn1->getOffset() != lo2->getOffset())) &&
                ...
                continue;
        } else if ((op->getIn(0) != lo2) && (op->getIn(1) != lo2))
            continue;
        reslo = op->getOut();
        return true;
    }
...
{
    if (!mapResHiSmallConst(rhi))
        return false;
    if (!findLoFromInSmallConst())
        return false;
    if (!verifyLo())
        return false;
    if (!findResLo())
...

{
    ...
    while (iter != enditer) {
        lophi = *iter;
        ++iter;
        if (lophi->code() != CPUI_MULTIEQUAL)
            continue;
        if (lophi->getParent() != blbase)
            continue;
        if (lophi->getIn(inslot) != lobase)
            continue;
        return true;
    }
    return false;
...
{ // Verify the basic double precision indirect form and fill out the pieces
    ...
    lo = l;
    indhi = ind;
    if (indhi->getIn(1)->getSpace()->getType() != IPTR_IOP)
        return false;
    affector = PcodeOp::getOpFromConst(indhi->getIn(1)->getAddr());
    if (affector->isDead())
        return false;
    reshi = indhi->getOut();
    if (reshi->getSpace()->getType() == IPTR_INTERNAL)
    ...
    while (iter != enditer) {
        ...
        if (indlo->code() != CPUI_INDIRECT)
            continue;
        if (indlo->getIn(1)->getSpace()->getType() != IPTR_IOP)
            continue;
        if (affector != PcodeOp::getOpFromConst(indlo->getIn(1)->getAddr()))
            continue; // hi and lo must be affected by same op
        reslo = indlo->getOut();
        if (reslo->getSpace()->getType() == IPTR_INTERNAL)
            return false; // Indirect must not be through a temporary
...

{
    ...
    for (iter = whole->beginDescend(); iter != whole->endDescend(); ++iter) {
        ...
        if (offset != vnlo->getSize())
            continue;
        Varnode* vnhi = subpieceOpHi->getOut();
        if (!vnhi->isPrecisHi())
            continue;
        if (vnhi->getSize() != whole->getSize() - offset)
            continue;
        ...
        for (iter2 = vnhi->beginDescend(); iter2 != vnhi->endDescend(); ++iter2) {
            ...
            if (SplitVarnode::testContiguousPointers(storeOp2, op, storelo, storehi, spc)) {
                ...
                if (!testIndirectUse(storelo, storehi, indirects))
                    continue;
...

{
    ...
    for (int4 i = 0; i < indirects.size(); ++i) {
        ...
        for (iter = outvn->beginDescend(); iter != outvn->endDescend(); ++iter) {
            PcodeOp* op = *iter;
            usecount += 1;
            if (op->getParent() != op1->getParent())
                continue;
            if (op->getSeqNum().getOrder() < op1->getSeqNum().getOrder())
                continue;
            if (op->getSeqNum().getOrder() > op2->getSeqNum().getOrder())
                continue;
