...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/condexe.cc.
        ...
        if (bl == iblock) {
            ...
            if (readop->code() == CPUI_MULTIEQUAL) {
                rvn = getMultiequalRead(op, readop, slot);
                       ...
                       slot > 0) { // Cannot replace input of RETURN directly, create COPY to hold input
                PcodeOp* newcopyop = fd->newOp(1, readop->getAddr());
                fd->opSetOpcode(newcopyop, CPUI_COPY);
                Varnode* outvn = fd->newVarnodeOut(vn->getSize(), vn->getAddr(),
                                                   newcopyop); // Preserve the Varnode's storage address
...

{
    ...
    if (op->code() == CPUI_MULTIEQUAL) {
        ...
        if (vn != (Varnode*)0) {
            ...
            if (hasnodescend &&
                (!heritageyes[vn->getSpace()->getIndex()])) // Check if heritage is performed for this varnode's space
                return false;
        }
    }
    return true;
