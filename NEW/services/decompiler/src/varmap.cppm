...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/varmap.cc.
...

{
    ...
    if (b->sstart + b->size - 1 <= sstart + size - 1)
        return true;
    return false;
}
...

{
    ...
    if (!didReconcile) {
        if (isTypeLock()) {
            if (b->isTypeLock())
                throw LowlevelError("Overlapping forced variable types : " + type->getName() + "   " +
                                    b->type->getName());
            if (start != b->start)
...
ScopeLocal::ScopeLocal(uint8 id, AddrSpace* spc, Funcdata* fd, Architecture* g)
    : ScopeInternal(id, fd->getName(), g)
...

{
    ...
    if (retOp == (PcodeOp*)0 || retOp->numInput() < 2)
        return;
    Varnode* vn = retOp->getIn(1);
    if (vn->getSpace() != space)
        return;
    ...
    if (iter != alias.end()) {
        ...
        if (*iter <= (vn->getOffset() + vn->getSize() - 1))
...

{
    ...
    if (maxParamOffset < minParamOffset)
        return true; // If no min/max, then we have no know stack parameters
...
string ScopeLocal::buildVariableName(const Address& addr, const Address& pc, Datatype* ct, int4& index,
                                     uint4 flags) const {
    if (((flags & (Varnode::addrtied | Varnode::persist)) == Varnode::addrtied) && addr.getSpace() == space) {
...

{
    ...
    if (spacebase == (Varnode*)0)
        return; // No possible alias
...
MapState::MapState(AddrSpace* spc, const RangeList& rn, const RangeList& pm, Datatype* dt) : range(rn), paramRange(pm) {
    spaceid = spc;
    defaultType = dt;
    paramRangeHit = false;
...

{
    if (ct->getMetatype() == TYPE_PARTIALSTRUCT) {
        ...
        if (ct->getMetatype() == TYPE_STRUCT &&
            tps->getOffset() == 0) {                     // If initial fields of TYPE_STRUCT are moved here
...

{
    ...
    while (riter != iterend) {
        ...
        switch (op->code()) {
            ...
            case CPUI_COPY:
                addFixedType(vn->getOffset(), vn->getType(), op->getIn(0)->isConstant() ? RangeHint::copy_constant : 0,
                             types);
                break;
            default:
                addFixedType(vn->getOffset(), vn->getType(), 0, types);
...

{
    ...
    while (iter != enditer) {
        ...
        Address addr = vn->getAddr();
        if (addr.getSpace() != space)
            continue;
        // Only allow offsets which can be parameters
        if (!fd->getFuncProto().getParamRange().inRange(addr, 1))
            continue;
...

{
    ...
    if (entry == (SymbolEntry*)0)
        return;
    if (entry->isDynamic()) {
        dynRecommend.emplace_back(entry->getFirstUseAddress(), ((DynamicEntry*)entry)->getHash(), sym->getName(),
                                  sym->getId());
    } else {
        ...
        nameRecommend.emplace_back(((MapEntry*)entry)->getAddr(), usepoint, entry->getSize(), sym->getName(),
                                   sym->getId());
    }
