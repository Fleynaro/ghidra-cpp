...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/printlanguage.cc.
...

{
    ...
    switch (topToken->type) {
        ...
        case OpToken::binary:
            ...
            if (topToken->precedence < op2->precedence)
                return false;
            if (topToken->associative && (topToken == op2))
                return false;
            ...
            if ((op2->type == OpToken::postsurround) && (stage == 0))
                return false;
            return true;
        case OpToken::unary_prefix:
            ...
            if ((op2->type == OpToken::unary_prefix) || (op2->type == OpToken::presurround))
                return false;
            return true;
        case OpToken::postsurround:
            ...
            if ((op2->type == OpToken::postsurround) || (op2->type == OpToken::binary))
                return false;
            //    if (associative && (this == &op2)) return false;
            return true;
...

{
    ...
    emit->tagComment(commentstart, EmitMarkup::comment_color, spc, off);
    int4 pos = 0;
    while (pos < text.size()) {
        char tok = text[pos++];
        if ((tok == ' ') || (tok == '\t')) {
            ...
            while (pos < text.size()) {
                ...
                pos += 1;
                if (tok == '}')
                    break; // Search for brace ending the annotation
            }
