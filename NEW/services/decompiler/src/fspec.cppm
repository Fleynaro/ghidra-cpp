...
// Port provenance: mechanically copied from Ghidra/Features/Decompiler/src/decompile/cpp/fspec.cc.
...
{
    if ((type != TYPECLASS_GENERAL) && (op2.type != type))
        return false;
    if (spaceid != op2.spaceid)
        return false;
    if (op2.addressbase < addressbase)
        return false;
    if ((op2.addressbase + op2.size - 1) > (addressbase + size - 1))
        return false;
...
{
    if (spaceid != addr.getSpace())
        return false;
    if (addressbase < addr.getOffset())
        return false;
...

{
    ...
    Address entry(spaceid, addressbase);
    if (addr.overlap(0, entry, size) < 0)
        return false;
    if (endaddr.overlap(0, entry, size) < 0)
        return false;
    if (alignment == 0) {
        // Ordinary endian containment
        res.space = spaceid;
...
{
    if ((flags & (smallsize_zext | smallsize_sext | smallsize_inttype)) == 0)
        return CPUI_COPY;
    if (alignment != 0) {
        if (sz >= alignment)
            return CPUI_COPY;
    ...
    if (justifiedContain(addr, sz) != 0)
        return CPUI_COPY; // (addr,sz) is not justified properly to allow an extension
...
/// \param m is the space manager
/// \return the address of the new parameter (or an invalid address)
Address ParamEntry::getAddrBySlot(int4& slotnum, int4 sz, int4 typeAlign, bool justifyRight,
                                  const AddrSpaceManager* m) const

{
    ...
    if (alignment == 0) { // If not an aligned entry (allowing multiple slots)
        if (slotnum != 0)
            return res; // Can only allocate slot 0
        if (sz > size)
            return res;                      // Check on maximum size
...
uint4 ParamListStandard::assignAddressFallback(type_class resource, Datatype* tp, bool matchExact, vector<int4>& status,
    ...
    for (iter = entry.begin(); iter != entry.end(); ++iter) {
        ...
        if (param.addr.isInvalid())
            continue; // If -tp- doesn't fit an invalid address is returned
        if (curEntry.isExclusion()) {
...
void ParamListStandard::assignMap(const PrototypePieces& proto, TypeFactory& typefactory,
    ...
    for (int4 i = 0; i < hitlist.size(); ++i) {
        ...
        if (curentry == (const ParamEntry*)0) {
            ...
            for (int4 j = 0; j < active->getNumTrials(); ++j) {
                ...
                if (paramtrial.getEntry() != curentry)
                    continue;
                int4 slot = curentry->getSlot(paramtrial.getAddress(), 0) - curentry->getGroup();
                int4 endslot =
                    curentry->getSlot(paramtrial.getAddress(), paramtrial.getSize() - 1) - curentry->getGroup();
...

{
    ...
    for (int4 i = trialStart; i < numTrials; ++i) { // Mark entries intersecting the group set as definitely not used
        if (i == activeTrial)
            continue; // The trial NOT to mark
        ParamTrial& othertrial(active->getTrial(i));
        if (othertrial.isDefinitelyNotUsed())
            continue;
        if (!othertrial.getEntry()->groupOverlap(*activeEntry))
            break;
...

{
    ...
    for (int4 i = groupStart; i < numTrials; ++i) {
        ...
        if (grp != group)
            break;
        if (entry->getAllGroups().size() > 1)
            continue; // Covering multiple slots automatically give low score
        int4 score = 0;
...

{
    ...
    for (int4 i = start; i < stop; ++i) {
        ParamTrial& trial(active->getTrial(i));
        if (trial.isDefinitelyNotUsed())
            continue; // Already know not used
        if (!trial.isActive()) {
...
void ParamListStandard::parsePentry(Decoder& decoder, vector<EffectRecord>& effectlist, int4 groupid, bool normalstack,
...
/// \param normalstack is \b true if the parameters should be allocated from the front of the range
/// \param splitFloat is \b true if floating-point parameters are in their own resource section
void ParamListStandard::parseGroup(Decoder& decoder, vector<EffectRecord>& effectlist, int4 groupid, bool normalstack,
                                   bool splitFloat) {
...

{
    ...
    if (entryHi->getGroup() == entryLo->getGroup()) {
        ...
        list<ParamEntry>::const_iterator iter;
        for (iter = entry.begin(); iter != entry.end(); ++iter) {
            if ((*iter).getSize() < sizesum)
                continue;
            if ((*iter).justifiedContain(loaddr, losize) != 0)
                continue;
            if ((*iter).justifiedContain(hiaddr, hisize) != losize)
                continue;
            return true;
        }
    }
...

{
    ...
    if (entryNum == (const ParamEntry*)0)
        return false;
...
void ParamListRegisterOut::assignMap(const PrototypePieces& proto, TypeFactory& typefactory,
                                     vector<ParameterPieces>& res) const
...
void ParamListStandardOut::assignMap(const PrototypePieces& proto, TypeFactory& typefactory,
                                     vector<ParameterPieces>& res) const
...

{
    ...
    for (iter = entry.begin(); iter != entry.end(); ++iter) {
        const ParamEntry* curentry = &(*iter);
        if (firstOnly && !curentry->isFirstInClass() && curentry->isExclusion() &&
            curentry->getAllGroups().size() == 1) {
        ...
        for (k = 0; k < active->getNumTrials(); ++k) {
            ...
            if (paramtrial.getEntry() == (const ParamEntry*)0)
                continue;
            if (offmatch != paramtrial.getOffset())
                break;
            if (((offmatch == 0) && curentry->isParamCheckLow()) ||
                ...
                if (paramtrial.isRemFormed())
                    break; // Formed as a remainder of dual div/rem operation
                if (paramtrial.isIndCreateFormed())
                    break; // Formed indirectly by call
            }
...

{
    for (int4 i = 0; i < trial.size(); ++i) {
        if (addr.overlap(0, trial[i].getAddress(), trial[i].getSize()) >= 0)
            return i;
        if (sz <= 1)
            return -1;
        Address endaddr = addr + (sz - 1);
        if (endaddr.overlap(0, trial[i].getAddress(), trial[i].getSize()) >= 0)
            return i;
    }
    return -1;
}
...
void ProtoModel::assignParameterStorage(const PrototypePieces& proto, vector<ParameterPieces>& res,
...

{
    ...
    if (addr.getSpace()->getType() == IPTR_INTERNAL)
    ...
    if (iter == efflist.begin())
        return EffectRecord::unknown_effect; // Can't go back one
    --iter;
...
int4 ProtoModel::lookupRecord(const vector<EffectRecord>& efflist, int4 listSize, const Address& addr, int4 size) {
    if (listSize == 0)
        return -1;
    EffectRecord cur(addr, size);
...
{
    if (model->glb != glb)
        throw LowlevelError("Mismatched architecture");
    if ((model->input->getType() != ParamList::p_standard) && (model->input->getType() != ParamList::p_register))
...

{
    ...
    for (vector<VarnodeData>::const_iterator iter = likelytrash.begin(); iter != likelytrash.end(); ++iter) {
        ...
        if (binary_search(iter1, iter2, cur))
...

{
    ...
    for (int4 i = 1; i < res.size(); ++i) {
        if ((res[i].flags & ParameterPieces::hiddenretparm) != 0)
            continue;
        return res[i].addr;
    }
    return Address();
...
{
    if (!model->isCompatible(op2.model))
        return false;
    ...
    if (injectid != op2.injectid)
        return false;
    if ((flags & (is_inline | no_return)) != (op2.flags & (is_inline | no_return)))
        return false;
    if (effectlist.size() != op2.effectlist.size())
        return false;
    for (int4 i = 0; i < effectlist.size(); ++i)
        if (effectlist[i] != op2.effectlist[i])
            return false;

    if (likelytrash.size() != op2.likelytrash.size())
        return false;
    for (int4 i = 0; i < likelytrash.size(); ++i)
        if (likelytrash[i] != op2.likelytrash[i])
            return false;
    return true;
}
...
FuncCallSpecs::FuncCallSpecs(PcodeOp* call_op) : FuncProto(), activeinput(true), activeoutput(true) {
...

{
    ...
    if (!tmpvn->isSpacebasePlaceholder())
        return (Varnode*)0;
    if (!tmpvn->isWritten())
        return (Varnode*)0;
    ...
    if (!newoutput.empty()) {
        ...
        for (int4 i = 0; i < newoutput.size(); ++i) {
            ...
            if (oldOut->getSize() < param->getSize()) {
                ...
                int4 overlap =
                    oldOut->getAddr().justifiedContain(oldOut->getSize(), param->getAddress(), param->getSize(), false);
                VarnodeData vardata;
...

{
    if (isInputActive())
        return false;
    if (slot1 >= activeinput.getNumTrials())
    ...
    if (ishislot) { // slot1 looks like the high slot
        ...
        if (hislot->getSize() != vn1->getSize())
            return false;
        if (loslot->getSize() != vn2->getSize())
            return false;
    } else {
        loslot = &activeinput.getTrialForInputVarnode(slot1);
        hislot = &activeinput.getTrialForInputVarnode(slot1 + 1);
        if (loslot->getSize() != vn1->getSize())
            return false;
        if (hislot->getSize() != vn2->getSize())
            return false;
    }
...

{
    ...
    if (!isCompatible(restrictedProto))
        return false;
    if (restrictedProto.isDotdotdot() && (!isinputactive))
        return false;
...

{
    ...
    if (activeoutput.getNumTrials() == 0)
    ...
    if (activeoutput.getNumTrials() == 1) { // We have a single, properly justified output
        ...
        //     if (finaloutvn->getSize() != curtrial.getSize()) { // If the varnode does not exactly match the original
        //     trial
        ...
        if (finaloutvn == (Varnode*)0) {
            Address joinaddr = data.getArch()->constructJoinAddress(data.getArch()->translate, hivn->getAddr(),
                                                                    hivn->getSize(), lovn->getAddr(), lovn->getSize());
...

{
    ...
    for (i = 0; i < copyList.size(); ++i) {
        if (!copyList[i]->entryaddress.isInvalid())
            break;
    ...
    for (; i < copyList.size(); ++i) {
        if (copyList[i]->entryaddress == lastAddr)
            continue;
        num = i - lastChange;
