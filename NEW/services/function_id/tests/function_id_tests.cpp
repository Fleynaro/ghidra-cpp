...
/// Warms the immutable database cache once so acceptance tests do not serialize
/// independent cold parses across test cases.
void warm_original_databases() {
    static std::once_flag warmed;
    std::call_once(warmed, [] {
        const std::array names{"vs2012_x64.fidb", "vs2015_x64.fidb", "vs2017_x64.fidb", "vsOlder_x64.fidb"};
        std::vector<std::future<std::expected<fid::Database, fid::Error>>> pending;
        pending.reserve(names.size());
        for (const auto name : names) {
            const auto path = std::filesystem::path(FUNCTION_ID_TEST_DIR) / name;
...
void expect_fixture_matches(std::string_view hex, std::initializer_list<std::string_view> database_names,
    ...
    std::vector<std::future<std::expected<fid::Database, fid::Error>>> pending;
    pending.reserve(database_names.size());
    for (const auto database_name : database_names) {
        const auto path = std::filesystem::path(FUNCTION_ID_TEST_DIR) / database_name;
        pending.push_back(std::async(std::launch::async, [path] { return fid::Database::open(path); }));
    }
    for (auto& future : pending) {
        auto result = future.get();
        ASSERT_TRUE(result.has_value()) << result.error().message;
        auto database = std::move(*result);
        const auto database_result = database.identify(context, program, 0.0F);
        ASSERT_TRUE(database_result.has_value()) << database_result.error().message;
        names.insert(names.end(), database_result->names.begin(), database_result->names.end());
    }
